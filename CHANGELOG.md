...
- Audit §10 item 8 filed `H18` as "the `rollup` template emits Optimistic commitments against a
  deployment that registers only the ZK verifier". Reconciling the two showed a larger structure: the
  accept rule was implemented three times — `SettlementManagerContract.IsProofTypeCompatible` (the
  enforcer), `LocalHostOperatorStatus.IsSecurityLevelPairedWithProofType` (a named entry in the
  operator-readiness failure list) and `ValidateChainConfigCommand` (the CLI an operator runs before
  registering a chain). The two off-chain copies were copies *of each other*, and they disagreed with
  the contract on `Optimistic+Multisig`, `Sidechain+None` and `Settled+None` — all three writable in a
  `chain.config.json`, all three passing `validate`, all three faulting in `submitBatch`
  (`SettlementManagerContract.cs:370`) before the verifier is ever consulted. The CLI stated the rule as
  four independent `if`s on the level and named none of them `Settled`, so a `Settled` chain validated
  silently at *any* proof type.
- The template line the finding pointed at was not the cause. `Optimistic+Optimistic` is legal on-chain,
  so no correct version of the accept table would have warned about the flagship template either. The
  knowledge that was actually missing is a second, orthogonal axis — which verifier routes a deployment
  registers — and no layer in the repo encoded it: `LiveDeployCommand.cs:866-869` registers only
  `ProofType.Zk` and then calls `lockGovernance`, one-way. The defect was therefore two tables that
  disagreed about legality *and* one legal-but-unserved axis nobody tracked.
- The table is now asked rather than copied. `IsProofTypeCompatible` became
  `[Safe] public static` (`SettlementManagerContract.cs:403-425`) with an unchanged body — the same
  function already enforces at `:370` and re-checks at `:523`, so the read cannot drift from the fault
  path — and `src/Neo.L2.Abstractions/Models/ProofRouting.cs` is the only off-chain copy. Both
  hand-written tables are deleted; the operator-status heuristic (`:584`) and `validate` (`:100-108`)
  delegate to it. `validate` now emits two distinct warnings instead of one: `accepts only proofType=…`
  for a pair the contract rejects, and `… has no verifier route in the shipped production bundle` for a
  legal pair whose route the deployer freezes without.
- Contract and mirror are checked against a **third** reference neither one can see:
  `tests/Shared/ProofRoutingExpectations.cs` is compiled into every test assembly through
  `tests/Directory.Build.props`, `UT_SettlementManager_ProofRouting` pushes all 36 pairs (levels
  `0..5`, proof types including `4` and `255`) through the deployed NEF, and `UT_ProofRouting` checks
  `ProofRouting` against the same list. It is a compiled-in file, not a project reference, because
  `NeoHub.Contracts.VmTests.csproj` deliberately carries no `<ProjectReference>` at all — referencing
  `Neo.L2.Abstractions` would resolve `$(NeoCorePath)\Neo\Neo.csproj` next to
  `Neo.SmartContract.Testing`'s own `Neo`.
- **Operator-visible change:** `rollup` now emits `proofType: Zk` under its `securityLevel: Optimistic`
  floor (over-delivery is legal and is the only pairing a shipped hub can both accept and verify), and
  `sidechain` emits `Multisig` instead of `None` — no layer accepts `None`: `WriteVerifier` rejects proof
  type `0` (`VerifierRegistryContract.cs:233`) and `ProofWitnessSerializers` refuses to build a `None`
  artifact, so a `None` config cannot produce a batch anywhere. The three shipped-config guards
  (per-template `create-chain`, per-template `new-l2`, the `samples/*.config.json` walk) now share one
  class-aware `ShippedConfigWarningPolicy` — a contradiction is never allowed, and `sidechain` /
  `privacy-sidechain` must still print their documented caveat and print nothing else. Two catalog
  guards in `UT_ListTemplatesCommand` assert every template's pair is legal and every non-sidechain
  template names a route the deployer registers. Five fixtures that paired `Multisig` with
  `securityLevel: "Optimistic"` described a chain `SettlementManager` would fault and are re-paired to
  `Settled`, rather than the table being loosened to accommodate them.
- Docs corrected against the code: `doc.md` §3.2 now states the accept table and lists
  `isProofTypeCompatible` (it previously stated neither), and `doc.md:169`'s `securityLevel` comment
  gained `4 validium` to match §12 of the same file. `docs/launching-an-l2.md`'s template table had
  drifted to `rollup` = L1 DA + Optimistic and `sidechain` = External + None, and its optimistic-rollup
  section opened on the false premise that the shipped template runs `proofType: Optimistic`; both are
  fixed with their `docs/zh/` mirrors, and `docs/zh/specification/08-neohub-contracts.md` §8.6's
  four-route `VerifierRegistry` table — including a `ProofType.Gateway` route the registry cannot hold —
  now separates "registerable" from "registered by the bundle". Both published template tables are
  hand-typed copies of `TemplateCatalog`, so `LaunchingGuide_TemplateTable_MatchesTheCatalog` and its
  Chinese twin now parse them and compare every cell against the catalog; drifting one `proofType`
  column produced exactly one failure, which is the negative control that the guard bites.
- **Not closed, deliberately:** `doc.md` §7.5 stage 0/1's `Multisig` and `Optimistic` on-chain verifiers
  remain unimplemented — of the 26 `contracts/NeoHub.*` projects only `ContractZkVerifierContract.cs:302`
  satisfies the registry's `verify(commitmentBytes)` interface — so those two routes are
  operator-supplied by construction and the `sidechain` caveat is description, not decoration.
  `ShippedConfigWarningPolicy` is the tripwire that says delete it when they land. Also left as found:
  `tools/Neo.L2.Devnet/Program.cs:385,403` builds Multisig commitments regardless of the config's
  `proofType`, and no shipped template emits `SecurityLevel.Settled` despite its four legal pairings.
- Tests: full solution 38 assemblies / **2,921 tests** / 0 failed / 5 skipped (eleven new
  `[TestMethod]` methods over item 7's 2,910), `Neo.Plugins.L2Settlement.UnitTests` 168,
  `Neo.L2.IntegrationTests` 40, `Neo.Stack.Cli.UnitTests` 195, and the 36-pair VM check runs compiled
  bytecode rather than a re-read of the source. The re-emitted `SettlementManager` NEF/manifest passes
  the authoritative gate: `NEO_N4_REQUIRE_FRESH_MANIFESTS=1 dotnet test
  tests/Neo.Hub.Deploy.UnitTests` is 115/115. `dotnet format --verify-no-changes` clean.
- Audit: §4 H18's status block and §10 item 8, mirrored into
...
- Close Multisig/Optimistic/Zk ops composition-root gap for Zk Open:
  ...
  - `Open_StartMetricsHttp_ReadyzOk` (`startMetricsHttp: true` → `/readyz` + `/healthprobe`,
    pins `SupportsLocalDaReader=false`)
  - `Open_OptimisticSettlementConfig_FailsClosed` (mode-only Open rejects Optimistic config)
- Wire/ABI unchanged. SoftSeal multi-cycle remains Multisig/Opt local-DA; Zk SoftSeal /
  prove-batch / real production DA remain funded.
...
- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (trevigintuple
  ...
  **twenty-third** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, trevigintuple
  deposits, trevigintuple outbox, trevigintuple FI/inbound known, Finalized RPC batch 1+2, and
  twenty-third withdrawal/message proofs still present.
- After twenty-second recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-twenty-third-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealTwentyThirdPoisonRecoverRetention`.
...
- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (duovigintuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **twenty-second** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, duovigintuple
  deposits, duovigintuple outbox, duovigintuple FI/inbound known, Finalized RPC batch 1+2, and
  twenty-second withdrawal/message proofs still present.
- After twenty-first recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-twenty-second-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealTwentySecondPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after twenty-first-recover twenty-second outbound + FI/RPC — 2026-07-22

- Multisig/Optimistic unit + E2E SoftSeal: after twenty-first recover + twenty-second deposit,
  pin twenty-second withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=22`),
  twenty-second FI/inbound nonces (known=22), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-twenty-first-recover-twenty-second-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterTwentyFirstRecoverTwentySecondOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.
...
- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (unvigintuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **twenty-first** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, unvigintuple
  deposits, unvigintuple outbox, unvigintuple FI/inbound known, Finalized RPC batch 1+2, and
  twenty-first withdrawal/message proofs still present.
- After twentieth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-twenty-first-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealTwentyFirstPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after twentieth-recover twenty-first outbound + FI/RPC — 2026-07-22

- Multisig/Optimistic unit + E2E SoftSeal: after twentieth recover + twenty-first deposit,
  pin twenty-first withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=21`),
  twenty-first FI/inbound nonces (known=21), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-twentieth-recover-twenty-first-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterTwentiethRecoverTwentyFirstOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after twentieth-recover DA + twenty-first offline deposit — 2026-07-22

- Multisig/Optimistic unit + E2E SoftSeal: after twentieth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a twenty-first offline
  deposit (nonce 21, `IncludedInBatch=2`, `ConsumedDepositCount=21`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 20, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-twentieth-recover-da-deposit.json` +
  `soft-seal-after-twentieth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterTwentiethRecoverDaAndTwentyFirstDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Tested — Neo N3 testnet session21 reverify + SharedBridge deposit n22 — 2026-07-22
...
- Live SharedBridge Deposit **nonce 22** HALT (`0x2f4a8ab2…498a`, 0.001 GAS,
...
- Chain `20260716` still active (securityLevel=3, daMode=0); TokenRegistry GAS/NEO
  ...
  registry pointer.
- Evidence: `docs/audit/testnet-deployment-20260722-session21-reverify.json`,
  `docs/audit/testnet-evidence-status-2026-07-22-session21.json`.
...
### Changed — SoftSeal twentieth poison→recover multi-state retention — 2026-07-22

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (viguple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **twentieth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, viguple deposits,
  viguple outbox, viguple FI/inbound known, Finalized RPC batch 1+2, and twentieth
  withdrawal/message proofs still present.
- After nineteenth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-twentieth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealTwentiethPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after nineteenth-recover twentieth outbound + FI/RPC — 2026-07-22

- Multisig/Optimistic unit + E2E SoftSeal: after nineteenth recover + twentieth deposit,
  pin twentieth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=20`),
  twentieth FI/inbound nonces (known=20), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-nineteenth-recover-twentieth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterNineteenthRecoverTwentiethOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after nineteenth-recover DA + twentieth offline deposit — 2026-07-22

- Multisig/Optimistic unit + E2E SoftSeal: after nineteenth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a twentieth offline
  deposit (nonce 20, `IncludedInBatch=2`, `ConsumedDepositCount=20`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 19, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-nineteenth-recover-da-deposit.json` +
  `soft-seal-after-nineteenth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterNineteenthRecoverDaAndTwentiethDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal nineteenth poison→recover multi-state retention — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (novemdecuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **nineteenth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, novemdecuple deposits,
  novemdecuple outbox, novemdecuple FI/inbound known, Finalized RPC batch 1+2, and nineteenth
  withdrawal/message proofs still present.
- After eighteenth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-nineteenth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealNineteenthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after eighteenth-recover nineteenth outbound + FI/RPC — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after eighteenth recover + nineteenth deposit,
  pin nineteenth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=19`),
  nineteenth FI/inbound nonces (known=19), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-eighteenth-recover-nineteenth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterEighteenthRecoverNineteenthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after eighteenth-recover DA + nineteenth offline deposit — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after eighteenth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a nineteenth offline
  deposit (nonce 19, `IncludedInBatch=2`, `ConsumedDepositCount=19`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 18, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-eighteenth-recover-da-deposit.json` +
  `soft-seal-after-eighteenth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterEighteenthRecoverDaAndNineteenthDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal eighteenth poison→recover multi-state retention — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (octodecuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **eighteenth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, octodecuple deposits,
  octodecuple outbox, octodecuple FI/inbound known, Finalized RPC batch 1+2, and eighteenth
  withdrawal/message proofs still present.
- After seventeenth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-eighteenth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealEighteenthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after seventeenth-recover eighteenth outbound + FI/RPC — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after seventeenth recover + eighteenth deposit,
  pin eighteenth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=18`),
  eighteenth FI/inbound nonces (known=18), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-seventeenth-recover-eighteenth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterSeventeenthRecoverEighteenthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after seventeenth-recover DA + eighteenth offline deposit — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after seventeenth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process an eighteenth offline
  deposit (nonce 18, `IncludedInBatch=2`, `ConsumedDepositCount=18`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 17, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-seventeenth-recover-da-deposit.json` +
  `soft-seal-after-seventeenth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterSeventeenthRecoverDaAndEighteenthDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal seventeenth poison→recover multi-state retention — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (septendecuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **seventeenth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, septendecuple deposits,
  septendecuple outbox, septendecuple FI/inbound known, Finalized RPC batch 1+2, and seventeenth
  withdrawal/message proofs still present.
- After sixteenth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-seventeenth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealSeventeenthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after sixteenth-recover seventeenth outbound + FI/RPC — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after sixteenth recover + seventeenth deposit,
  pin seventeenth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=17`),
  seventeenth FI/inbound nonces (known=17), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-sixteenth-recover-seventeenth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterSixteenthRecoverSeventeenthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after sixteenth-recover DA + seventeenth offline deposit — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after sixteenth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a seventeenth offline
  deposit (nonce 17, `IncludedInBatch=2`, `ConsumedDepositCount=17`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 16, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-sixteenth-recover-da-deposit.json` +
  `soft-seal-after-sixteenth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterSixteenthRecoverDaAndSeventeenthDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal sixteenth poison→recover multi-state retention — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (sexdecuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **sixteenth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, sexdecuple deposits,
  sexdecuple outbox, sexdecuple FI/inbound known, Finalized RPC batch 1+2, and sixteenth
  withdrawal/message proofs still present.
- After fifteenth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-sixteenth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealSixteenthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.
...
- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (quindecuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **fifteenth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, quindecuple deposits,
  quindecuple outbox, quindecuple FI/inbound known, Finalized RPC batch 1+2, and fifteenth
  withdrawal/message proofs still present.
- After fourteenth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-fifteenth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealFifteenthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.
...
- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (quattuordecuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **fourteenth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, quattuordecuple deposits,
  quattuordecuple outbox, quattuordecuple FI/inbound known, Finalized RPC batch 1+2, and fourteenth
  withdrawal/message proofs still present.
- After thirteenth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-fourteenth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealFourteenthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after thirteenth-recover fourteenth outbound + FI/RPC — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after thirteenth recover + fourteenth deposit,
  pin fourteenth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=14`),
  fourteenth FI/inbound nonces (known=14), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-thirteenth-recover-fourteenth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterThirteenthRecoverFourteenthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after thirteenth-recover DA + fourteenth offline deposit — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after thirteenth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a fourteenth offline
  deposit (nonce 14, `IncludedInBatch=2`, `ConsumedDepositCount=14`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 13, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-thirteenth-recover-da-deposit.json` +
  `soft-seal-after-thirteenth-recover-host.prom`. E2E helper
...
- Multisig/Optimistic unit + E2E SoftSeal: after twelfth recover + thirteenth deposit,
  pin thirteenth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=13`),
  thirteenth FI/inbound nonces (known=13), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-twelfth-recover-thirteenth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterTwelfthRecoverThirteenthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after twelfth-recover DA + thirteenth offline deposit — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after twelfth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a thirteenth offline
  deposit (nonce 13, `IncludedInBatch=2`, `ConsumedDepositCount=13`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 12, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-twelfth-recover-da-deposit.json` +
  `soft-seal-after-twelfth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterTwelfthRecoverDaAndThirteenthDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal twelfth poison→recover multi-state retention — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (duodecuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **twelfth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, duodecuple deposits,
  duodecuple outbox, duodecuple FI/inbound known, Finalized RPC batch 1+2, and twelfth
  withdrawal/message proofs still present.
- After eleventh recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-twelfth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealTwelfthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after eleventh-recover twelfth outbound + FI/RPC — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after eleventh recover + twelfth deposit,
  pin twelfth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=12`),
  twelfth FI/inbound nonces (known=12), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-eleventh-recover-twelfth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterEleventhRecoverTwelfthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after eleventh-recover DA + twelfth offline deposit — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after eleventh poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a twelfth offline
  deposit (nonce 12, `IncludedInBatch=2`, `ConsumedDepositCount=12`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 11, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-eleventh-recover-da-deposit.json` +
  `soft-seal-after-eleventh-recover-host.prom`. E2E helper
  `AssertSoftSealAfterEleventhRecoverDaAndTwelfthDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal eleventh poison→recover multi-state retention — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (undecuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **eleventh** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, undecuple deposits,
  undecuple outbox, undecuple FI/inbound known, Finalized RPC batch 1+2, and eleventh
  withdrawal/message proofs still present.
- After tenth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-eleventh-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealEleventhPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after tenth-recover eleventh outbound + FI/RPC — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after tenth recover + eleventh deposit,
  pin eleventh withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=11`),
  eleventh FI/inbound nonces (known=11), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-tenth-recover-eleventh-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterTenthRecoverEleventhOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after tenth-recover DA + eleventh offline deposit — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after tenth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process an eleventh offline
  deposit (nonce 11, `IncludedInBatch=2`, `ConsumedDepositCount=11`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 10, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-tenth-recover-da-deposit.json` +
  `soft-seal-after-tenth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterTenthRecoverDaAndEleventhDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal tenth poison→recover multi-state retention — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (decuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **tenth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, decuple deposits,
  decuple outbox, decuple FI/inbound known, Finalized RPC batch 1+2, and tenth
  withdrawal/message proofs still present.
- After ninth recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-tenth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealTenthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after ninth-recover tenth outbound + FI/RPC — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after ninth recover + tenth deposit,
  pin tenth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=10`),
  tenth FI/inbound nonces (known=10), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-ninth-recover-tenth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterNinthRecoverTenthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after ninth-recover DA + tenth offline deposit — 2026-07-21

- Multisig/Optimistic unit + E2E SoftSeal: after ninth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a tenth offline
  deposit (nonce 10, `IncludedInBatch=2`, `ConsumedDepositCount=10`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 9, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-ninth-recover-da-deposit.json` +
  `soft-seal-after-ninth-recover-host.prom`. E2E helper
...
- Multisig/Optimistic unit + E2E SoftSeal: after eighth recover + ninth deposit,
  pin ninth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=9`),
  ninth FI/inbound nonces (known=9), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-eighth-recover-ninth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterEighthRecoverNinthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after eighth-recover DA + ninth offline deposit — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after eighth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a ninth offline
  deposit (nonce 9, `IncludedInBatch=2`, `ConsumedDepositCount=9`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 8, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-eighth-recover-da-deposit.json` +
  `soft-seal-after-eighth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterEighthRecoverDaAndNinthDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal eighth poison→recover multi-state retention — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after full soft multi-batch path (octuple
  deposit/outbox/FI-inbound + RPC proofs), re-escalate mock L1 failures until
  **eighth** `Poisoned`, then RecoverPoisonedBatch; pins pending≥2, tip=2, octuple deposits,
  octuple outbox, octuple FI/inbound known, Finalized RPC batch 1+2, and eighth
  withdrawal/message proofs still present.
- After seventh recover, RetryCount resets — tests loop Reconcile/SubmitNext until
  Poisoned. Durable `soft-seal-eighth-poison-recover.json` + status/probe.
  E2E helper `AssertSoftSealEighthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.
...
- CLI `--fraud-replay-domain` must be on-chain **raw wire** bytes
  (`0xf1e2d3c4…1100`); report `ToString` may display the reversed form.
- Live SharedBridge Deposit **nonce 20** HALT (`0xa8503477…8532`, 0.001 GAS,
  `WitnessScope.Global`, Transfer + DepositEnqueued); `getDeposit` confirmed.
- Chain `20260716` still active (securityLevel=3, daMode=0); TokenRegistry GAS/NEO
  active; ForcedInclusion production-ready; fixed bridge still independent of legacy
  registry pointer.
- Evidence: `docs/audit/testnet-deployment-20260720-session19-reverify.json`,
  `docs/audit/testnet-evidence-status-2026-07-20-session19.json`.
- WIF only via process env `NEO_N4_TESTNET_WIF` (not committed). Funded gates still open:
  L1 settle, Zk prove-batch, production DA, 2-of-2 bridge retarget, full Neo.CLI stack.
...
- Multisig/Optimistic unit + E2E SoftSeal: after sixth recover + seventh deposit,
  pin seventh withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=7`),
  seventh FI/inbound nonces (known=7), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-sixth-recover-seventh-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterSixthRecoverSeventhOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after sixth-recover DA + seventh offline deposit — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after sixth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a seventh offline
  deposit (nonce 7, `IncludedInBatch=2`, `ConsumedDepositCount=7`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 6, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-sixth-recover-da-deposit.json` +
  `soft-seal-after-sixth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterSixthRecoverDaAndSeventhDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal sixth poison→recover sextuple-state retention — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after fifth recover + sixth deposit/outbox/FI,
  re-escalate mock L1 failures until **sixth** `Poisoned`, then `RecoverPoisonedBatch`
  (wrong-hash fail-closed) and pin sextuple soft multi-batch state survives: pending≥2,
  tip=2, `ConsumedDepositCount=6`, outbox=6, FI/inbound known=6, RPC Finalized batch1+2,
  sixth withdrawal + message proofs retained while settle returns to `Retrying`.
- Durable `soft-seal-sixth-poison-recover.json` + status/probe. E2E helper
  `AssertSoftSealSixthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after fifth-recover sixth outbound + FI/RPC — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after fifth recover + sixth deposit,
  pin sixth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=6`),
  sixth FI/inbound nonces (known=6), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-fifth-recover-sixth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterFifthRecoverSixthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after fifth-recover DA + sixth offline deposit — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after fifth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a sixth offline
  deposit (nonce 6, `IncludedInBatch=2`, `ConsumedDepositCount=6`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 5, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-fifth-recover-da-deposit.json` +
  `soft-seal-after-fifth-recover-host.prom`. E2E helper
...
- Multisig/Optimistic unit + E2E SoftSeal: after fourth recover + fifth deposit,
  pin fifth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=5`),
  fifth FI/inbound nonces (known=5), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-fourth-recover-fifth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterFourthRecoverFifthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after fourth-recover DA + fifth offline deposit — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after fourth poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a fifth offline
  deposit (nonce 5, `IncludedInBatch=2`, `ConsumedDepositCount=5`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 4, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-fourth-recover-da-deposit.json` +
  `soft-seal-after-fourth-recover-host.prom`. E2E helper
  `AssertSoftSealAfterFourthRecoverDaAndFifthDeposit`.
- No wire/ABI change. L1 deposit scan / production DA / settle remain funded.

### Changed — SoftSeal fourth poison→recover quadruple-state retention — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after third recover + fourth deposit/outbox/FI,
  re-escalate mock L1 failures until **fourth** `Poisoned`, then `RecoverPoisonedBatch`
  (wrong-hash fail-closed) and pin quadruple soft multi-batch state survives: pending≥2,
  tip=2, `ConsumedDepositCount=4`, outbox=4, FI/inbound known=4, RPC Finalized batch1+2,
  fourth withdrawal + message proofs retained while settle returns to `Retrying`.
- Durable `soft-seal-fourth-poison-recover.json` + status/probe. E2E helper
  `AssertSoftSealFourthPoisonRecoverRetention`.
- No wire/ABI change. L1 settle for multi-pending batches remains a funded gate.

### Changed — SoftSeal after third-recover fourth outbound + FI/RPC — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after third recover + fourth deposit,
  pin fourth withdrawal seal + L2→L1 outbox enqueue (`MessageOutboxL2ToL1Count=4`),
  fourth FI/inbound nonces (known=4), and RPC withdrawal/message/router proofs
  while settle remains `Retrying` with multi-batch pending≥2 and tip=2.
- Durable `soft-seal-after-third-recover-fourth-outbound.json` +
  `-rpc.json` + status/probe. E2E helper
  `AssertSoftSealAfterThirdRecoverFourthOutboundAndFi`.
- No wire/ABI change. L1 claim / FI drain / settle remain funded.

### Changed — SoftSeal after third-recover DA + fourth offline deposit — 2026-07-20

- Multisig/Optimistic unit + E2E SoftSeal: after third poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a fourth offline
  deposit (nonce 4, `IncludedInBatch=2`, `ConsumedDepositCount=4`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 3, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-third-recover-da-deposit.json` +
  `soft-seal-after-third-recover-host.prom`. E2E helper
...
- CLI `--fraud-replay-domain` must be on-chain **raw wire** bytes
  (`0xf1e2d3c4…1100`); report `ToString` may display the reversed form.
- Live SharedBridge Deposit **nonce 19** HALT (`0x54f21a9f…b614`, 0.001 GAS,
  `WitnessScope.Global`, Transfer + DepositEnqueued); `getDeposit` confirmed.
- Chain `20260716` still active (securityLevel=3, daMode=0); TokenRegistry GAS/NEO
  active; ForcedInclusion production-ready; fixed bridge still independent of legacy
  registry pointer.
- Evidence: `docs/audit/testnet-deployment-20260720-session18-reverify.json`,
  `docs/audit/testnet-evidence-status-2026-07-20-session18.json`.
- WIF only via process env `NEO_N4_TESTNET_WIF` (not committed). Funded gates still open:
  L1 settle, Zk prove-batch, production DA, 2-of-2 bridge retarget, full Neo.CLI stack.
...
- Multisig/Optimistic unit + E2E SoftSeal: after second poison→recover, re-publish
  local DA for sealed batches 1+2 (reader round-trip), process a third offline
  deposit (nonce 3, `IncludedInBatch=2`, `ConsumedDepositCount=3`) while settle
  remains `Retrying` with pending≥2, outbox/FI/inbound known still 2, passport
  complete; host Prometheus scrape + status/probe durable files.
- Durable `soft-seal-after-second-recover-da-deposit.json` +
  `soft-seal-after-second-recover-host.prom`. E2E helper
...
- CLI `--fraud-replay-domain` must be on-chain **raw wire** bytes
  (`0xf1e2d3c4…1100`); report `ToString` may display the reversed form.
- Live SharedBridge Deposit **nonce 18** HALT (`0xf73e3c4b…6fad`, 0.001 GAS,
  `WitnessScope.Global`, Transfer + DepositEnqueued); `getDeposit` confirmed.
- Chain `20260716` still active (securityLevel=3, daMode=0); TokenRegistry GAS/NEO
  active; ForcedInclusion production-ready; fixed bridge still independent of legacy
  registry pointer.
- Evidence: `docs/audit/testnet-deployment-20260720-session17-reverify.json`,
  `docs/audit/testnet-evidence-status-2026-07-20-session17.json`.
- WIF only via process env `NEO_N4_TESTNET_WIF` (not committed). Funded gates still open:
  L1 settle, Zk prove-batch, production DA, 2-of-2 bridge retarget, full Neo.CLI stack.
...
- Live SharedBridge Deposit **nonce 16** HALT (`0x3dd27041…fdfa`, 0.001 GAS,
  `WitnessScope.Global`, Transfer + DepositEnqueued); `getDeposit` confirmed.
- Chain `20260716` still active (securityLevel=3, daMode=0); TokenRegistry GAS/NEO
  active; ForcedInclusion production-ready; fixed bridge still independent of legacy
  registry pointer.
- Evidence: `docs/audit/testnet-deployment-20260719-session15-reverify.json`,
  `docs/audit/testnet-evidence-status-2026-07-19-session15.json`.
- WIF only via process env `NEO_N4_TESTNET_WIF` (not committed). Funded gates still open:
  L1 settle, Zk prove-batch, production DA, 2-of-2 bridge retarget, full Neo.CLI stack.
...
- CLI `--fraud-replay-domain` must be on-chain **raw wire** bytes
  ...
  reversed hex fails postdeploy `RegisterPermissionlessFraudProfile` with
  `verifier replay domain mismatch`.
- Live SharedBridge Deposit **nonce 15** HALT (`0xcf7f81f8…0e9f`, 0.001 GAS,
  `WitnessScope.Global`, Transfer + DepositEnqueued); `getDeposit` confirmed.
- Chain `20260716` still active (securityLevel=3, daMode=0); TokenRegistry GAS/NEO
  active; ForcedInclusion production-ready; fixed bridge still independent of legacy
  registry pointer.
- Evidence: `docs/audit/testnet-deployment-20260719-session14-reverify.json`,
  `docs/audit/testnet-evidence-status-2026-07-19-session14.json`.
- WIF only via process env `NEO_N4_TESTNET_WIF` (not committed). Funded gates still open:
  L1 settle, Zk prove-batch, production DA, 2-of-2 bridge retarget, full Neo.CLI stack.

### Changed — Gateway operator status adds metrics soft config (probe parity) — 2026-07-19

- `GatewayHostOperatorStatus` / `GatewayHostOperatorStatusDocument` /
  `FormatOperatorStatusJson` / `/operatorstatus` include
  `MetricsConfiguredPort`, `MetricsBindAddress`, and
  `MetricsMaxConcurrentConnections` (same soft config already on the compact
  health probe) so full status JSON can distinguish configured vs bound metrics
  HTTP endpoints without a separate probe.
- E2E Multisig+Gateway asserts status/probe metrics soft-config parity +
  status JSON keys; init-l2 host status tips name the same fields.
...
- CLI `--fraud-replay-domain` must be on-chain **raw wire** bytes
  (`0xf1e2d3c4…1100`); report `ToString` may display the reversed form.
- Live SharedBridge Deposit **nonce 13** HALT (`0x698df46f…2c0b`, 0.001 GAS,
  `WitnessScope.Global`, Transfer + DepositEnqueued); `getDeposit` confirmed.
- Chain `20260716` still active; TokenRegistry GAS/NEO active; ForcedInclusion production-ready.
- Evidence: `docs/audit/testnet-deployment-20260719-session12-reverify.json`,
  `docs/audit/testnet-evidence-status-2026-07-19-session12.json`.
- WIF only via process env `NEO_N4_TESTNET_WIF` (not committed). Funded gates still open:
  L1 settle, Zk prove-batch, production DA, 2-of-2 bridge retarget, full Neo.CLI stack.
...
- Multisig/Optimistic/Zk `GetHealthProbeAsync` returns `LocalHostHealthProbeDocument`;
  `WriteHealthProbeAsync` serializes it (passport / pipeline / metrics / settlement).
- `GatewayHostComposition.GetHealthProbe` returns `GatewayHostHealthProbeDocument`;
  `WriteHealthProbeAsync` serializes it (passport / outbox / publication).
- Wireproduction notes + init-l2 tips; unit/integration coverage. No wire/ABI change.
- L1 settle / publication confirmation remain funded gates.
...
- CLI `--fraud-replay-domain` must be on-chain **raw wire** bytes
  (`0xf1e2d3c4…1100`); report `ToString` may display the reversed form.
- Live SharedBridge Deposit **nonce 11** HALT (`0xcb2656d2…eb36`, 0.001 GAS,
  `WitnessScope.Global`, Transfer + DepositEnqueued); `getDeposit` confirmed.
- Chain `20260716` still active; TokenRegistry GAS/NEO active; ForcedInclusion production-ready.
- Evidence: `docs/audit/testnet-deployment-20260718-session10-reverify.json`,
  `docs/audit/testnet-evidence-status-2026-07-18-session10.json`.
- WIF only via process env `NEO_N4_TESTNET_WIF` (not committed). Funded gates still open:
  L1 settle, Zk prove-batch, production DA, 2-of-2 bridge retarget, full Neo.CLI stack.
