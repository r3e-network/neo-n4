...
| 4     | NeoVM2 / RISC-V ZK Validity Proof         | ✅ **End-to-end ZK validity proofs work.** `bridge/neo-zkvm-guest/` ships the Rust crate that `cargo prove build` compiles to a real RISC-V succinct zkVM ELF. `bridge/neo-zkvm-host/` loads it + exposes three production functions: `execute()` (zkVM run, no proof), `prove()` (real CPU proof generation — returns proof bytes + verifying-key bytes for on-chain submission), `verify()` (pre-submission proof check). Operator CLI `prove-batch --prove <hex>` writes the artifact pair to disk. Verified end-to-end by `tests/end_to_end.rs`: the default test runs the zkVM execute path (~42s); two `#[ignore]`-gated tests exercise real proof + verify, plus a tampered-hash negative test (~4 min wall time on CPU). RISC-V execution: `Neo.L2.Executor.RiscV` P/Invokes into `external/neo-riscv-vm`. Operator install: `sp1up` (one-time toolchain bootstrap). |
...
- **Real SP1 prover linkage** — fully wired through `bridge/neo-zkvm-host/` (Rust): `prove()` returns real proof + verifying-key bytes for on-chain submission and `verify()` confirms them off-chain before settlement; both exercised end-to-end by the `prove_and_verify_real_zk_proof` `#[ignore]`-gated test. The older C# `Neo.L2.Proving.Sp1` P/Invoke path (`--features real-prover` on `bridge/neo-zkvm-bridge`) remains as an alternative integration. Either path requires the SP1 toolchain (`sp1up`) installed.
