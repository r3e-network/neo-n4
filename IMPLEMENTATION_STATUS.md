...
| 1     | NeoHub v0 + Shared Bridge                 | ✅ All 26 NeoHub projects compile; deploy planner emits 24 production steps, including the concrete immutable L2 payout adapter; advisory structural verifier is excluded |
...
| 5     | Neo Gateway (proof aggregation)           | 🟡 Binding, durable outbox, Secp256r1/Merkle round provers, and a fail-closed SP1 6.2.1 recursive guest/host ship. The proof-bound RPC path submits exact finalized constituent references to `SettlementManager`, which reconstructs both roots, advances non-revertible per-chain watermarks, and atomically calls `MessageRouter`; external audit and executed real-proof deployment evidence remain required before the Gateway can be called a production validity-proof path |
...
| `Neo.L2.Executor`         | `SPEC.md` + fixed 105-byte `Receipt`, pluggable `ITransactionExecutor` / `IPostStateRootOracle` / `IL1MessageProcessor`, `ReferenceBatchExecutor`, **`ExecutionStateTransaction` + canonical execution effects V1 + `KeyedStateStore` + `KeyedStateRootOracle`** |
...
| Crate                       | Role                                             |
| --------------------------- | ------------------------------------------------ |
| `neo-zkvm-host`             | Rust binary (sp1-sdk 6.2.1): `prove-batch daemon --watch <dir> --gateway-sidecars <dir>` emits terminal batch proofs and tuple-bound compressed child sidecars. Also exposes `execute()` / `prove()` / `prove_compressed()` / `verify()`. |
...
| `Neo.L2.ExternalBridge.UnitTests`    | 3     | exact canonical payout field preservation, duplicate/conflicting mapped-asset queue identity, prepared-transaction retry reload, and bounded poison transition |
...
| `Neo.L2.Executor.UnitTests`          | 66    | empty/single/many, ordering, determinism, canonical output binding, **KeyedStateStore + oracle, persistence reopen pins, transaction-overlay rollback/conflict checks, canonical effects V1 ordering/full-field/event-state pins, 105-byte receipt encoding** |
| `Neo.L2.ForcedInclusion.UnitTests`   | 29    | nonce ordering, replay, overdue detection, encoded transaction hash validation, L1-confirmed consumption, **persistence reopen pins** |
...
| `Neo.Plugins.L2Settlement.UnitTests` | 64    | canonical artifact bytes reach prover, ZK fail-closed policy, DA/result/public-input cross-checks, strict contiguous durable reconciliation, bounded retry/poison/restart/operator recovery, no-loss and no-bypass checks, validated continuous batch/block/state checkpoint recovery, process-crash recovery across proof/broadcast/observation/finality/consume windows, transaction-aware duplicate suppression, restored success/failure/concurrency/throwing-metrics coverage, explicit legacy control, production configuration/constructor/forced-pair/ownership/disposal coverage |
...
- **Recursive-ZK Gateway operations** — the SP1 6.2.1 guest/daemon and strict queue protocol
  ship in-repo. Operators still supply canonical compressed batch-proof sidecars, proving
  hardware, execute the ignored real-recursion release gate for the exact build manifest,
  and retain deployment/audit evidence; `test-only-vk` builds cannot prove.
