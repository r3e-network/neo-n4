...
| `Neo.Plugins.L2Prover.UnitTests`     | 17    | `L2ProverPlugin` lifecycle, ProofType resolution, **CreateFromChainDirectory / CreateMultisigWiredFromChainDirectory**, Wire dispatch Multisig/Zk/Optimistic/None |
...
| Host WireProduction | **code-complete**: batch/settlement/bridge/prover/metrics/gateway factories + local DA + RPC proofs + gateway outbox + L1 inbox bundle (`L1InboxFromChainDirectory` / `WireL1InboxFromChainDirectory` + per-type `Create*FromChainDirectory`) + `WireProductionFromLayout` + Multisig (`CreateMultisigWiredFromChainDirectory`) / Optimistic provers; Zk: `OpenStateFromChainDirectory` + Sp1 stack (still needs funded executor binary + VK + production DA); Gateway: `CreateMerkleDurable` / `CreateMultisigDurable` / `CreateSp1Durable` + `Sp1GatewayProofProver.OpenFromChainDirectory` + `ProofBoundRpcGlobalRootPublisher.OpenFromChainDirectory` then `ConfigureGlobalRootPublication` (gateway daemon + funded L1 still operator-supplied) |
...
- **Settlement and operator signer custody** — `L2SettlementPlugin.WireProduction` closes the
  ...
  `L2ProverPlugin.CreateMultisigWiredFromChainDirectory(chainDir, signers)`.
