...
| In-process deposit injection (devnet) | `RpcSharedBridgeDepositSource` + `RpcSharedBridgeDepositScanner` ship in `src/Neo.L2.Bridge/`. Production wiring scans finalized `DepositEnqueued` logs, fetches `GetDeposit`, decodes the unsigned-LE SharedBridge record into `DepositPayload` + native-bridge-bound `CrossChainMessage`, and drains into the batcher L1 inbox. | `ISharedBridgeDepositSource` |
