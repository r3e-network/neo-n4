...
| Default deployment automation | `RestrictedExecutionFraudVerifier` accepts empty deploy data only for legacy governance-only v3 compatibility; v4 then fails closed. A permissionless restricted deployment must pass `[SettlementManager, replayDomain]` and call `RegisterPermissionlessFraudProfile`. The existing deploy planner has not been expanded in this change. | Operator wiring required |
...
| `Neo.L2.Executor.UnitTests`          | 66    | empty/single/many, ordering, determinism, **KeyedStateStore + oracle, persistence reopen pins, transaction-overlay rollback/conflict checks, canonical effects V1 ordering/full-field/event-state pins, 105-byte receipt encoding** |
