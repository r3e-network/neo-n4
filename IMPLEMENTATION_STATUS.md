...
| `Neo.Plugins.L2Gateway`      | `BinaryTreeAggregator` with pluggable `IRoundProver` (default `PassThroughRoundProver`); `PassThroughAggregator` for flat aggregation; emits `l2.gateway.aggregations/batches_aggregated/aggregation_rounds/aggregation_latency_ms`; **`RetryingGatewayProofProver`** provides the proving retry seam — `TimeoutException` → idempotent resubmit with exponential backoff, fail-closed (`InvalidDataException`/`ArgumentException`) errors never retried |
...
### Design-only and remaining gaps

These documents capture validated designs without adding speculative configuration or implementation:

| Document | Scope | Status |
| --- | --- | --- |
| [`docs/l2-fee-pricing-design.md`](docs/l2-fee-pricing-design.md) | Committee-governed L2 fee pricing surface (`BaseFee` + optional L1 DA data-cost pass-through), complementing the existing `L2FeeContract` distribution and `L2PaymasterContract` | **Design only**; requires fee-policy/spec review before implementation |
| [`docs/l2-auto-recovery-design.md`](docs/l2-auto-recovery-design.md) | Detection→action orchestration over existing health/metrics signals and the implemented DA/prover recovery seams | **Design only**; requires policy/config spec before implementation |

Remaining work is not an exhaustive two-item checklist. `RollupHubContract.EnqueueForcedTransaction` already supplies a consolidated on-chain forced-inclusion entry point; absence of the historical standalone project is not evidence that the capability is missing. The inspected consolidated ABI is incompatible with the legacy RPC path: RollupHub emits three-field `ForcedTransactionEnqueued`, while the scanner expects four-field `ForcedTxEnqueued`; the source/finalizer require `getEntry`, `isConsumed`, and proof-based `consume`, whereas RollupHub stores transaction hashes and advances a queue by count. Its first nonce is zero, which the finalizer rejects. SharedBridge stores raw message payloads while the RPC decoder expects a canonical 61-byte header; root publication exists but does not establish destination-chain delivery. Resolving these requires a coordinated contract/client ABI and storage migration proposal under doc.md §10/§15.4, not a duplicate standalone contract or an event rename. The DA failover and prover retry decorators are caller-injected building blocks, not a deployed recovery system; production reader/profile binding, host wiring and live recovery drills remain acceptance gates.
...
## Local RISC-V / neo-zkvm verification

Latest local re-verify (2026-07-22, commit `fecb15a` lineage): [`docs/audit/riscv-zkvm-local-verify-2026-07-22.json`](./docs/audit/riscv-zkvm-local-verify-2026-07-22.json). Repeat with `scripts/ci/verify_riscv_zkvm_local.sh`. **Code-complete:** PolkaVM RealNative 31/31 + Sp1Stateful executor boundary + guest host tests. **Funded/env still open:** SP1 Docker ELF rebuild, real-CPU prove, gateway recursive proof, PolkaVM validity prover, NEF/VK deployment evidence.

## Public testnet evidence (NeoHub L1)

| Field | Value |
|-------|-------|
| Status | **Current for NeoHub L1 + registerChain + TokenRegistry + ForcedInclusion + fixed SharedBridge Deposit** (2026-07-22 session21) |
...
| Contracts | 24 original bundle re-verified (24/24 reuse + smoke 2026-07-22 session21); SharedBridge **fixed** redeployed as new hash |
