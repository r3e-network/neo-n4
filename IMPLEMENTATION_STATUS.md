...
| In-process deposit injection (devnet) | `RpcSharedBridgeDepositSource` + `RpcSharedBridgeDepositScanner` ship in `src/Neo.L2.Bridge/`; `InMemorySharedBridgeDepositSource` covers tests/devnet. Production wiring scans finalized `DepositEnqueued` logs, fetches `GetDeposit`, decodes the unsigned-LE SharedBridge record into `DepositPayload` + native-bridge-bound `CrossChainMessage`, and drains into the batcher L1 inbox via `L1MessageDrain.Combine` / `L2BridgePlugin.DrainSharedBridgeDeposits`. | `ISharedBridgeDepositSource` |
...
| `Neo.Plugins.L2Prover.UnitTests`     | 11    | `L2ProverPlugin` lifecycle, ProofType resolution, **Wire dispatch coverage for all branches: Zk-with-prover / Zk-without (helpful error) / Optimistic (points at L2SettlementPlugin) / None (points at Multisig+Optimistic+Zk)** |
