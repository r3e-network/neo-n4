...
| `PassThroughRoundProver` is a deterministic reference combiner; `MultisigRoundProver` and `MerklePathRoundProver` are the two shipped production implementations | SP1 Compress / Halo2 fold / Risc0 fold when recursive-ZK aggregation is required | `IRoundProver` |
...
| `Neo.Plugins.L2Rpc`          | 10 RPC handlers (doc.md §14.1) + `IL2RpcStore` (`InMemoryL2RpcStore` with optional `IL2KeyValueStore` for withdrawal/message proofs); per-method `l2.rpc.calls/latency_ms/failures` tagged by `method` |
...
| `Neo.Plugins.L2Settlement.UnitTests` | 36    | canonical artifact bytes reach prover, ZK fail-closed policy, DA/result/public-input cross-checks, ordered durable reconciliation, process-crash recovery across proof/broadcast/observation/finality/consume windows, transaction-aware retry, restored success/failure/concurrency/throwing-metrics coverage, explicit legacy control |
| `Neo.L2.Settlement.Rpc.UnitTests`    | 50    | JSON-RPC envelope, stack parsing, signer, batch lifecycle, and transaction-status reconciliation for pending/confirmed/unknown submissions |
...
- **Recursive-ZK Gateway folding** — `MultisigRoundProver` and
  `MerklePathRoundProver` are production implementations; recursive aggregation
  requires an operator-supplied SP1 Compress, Halo2, or Risc0 `IRoundProver`.
- **dBFT validator-set activation** — `SequencerRegistry` and
  `Neo.L2.Sequencer` provide the per-chain validator set; production node startup
  must activate that set in the r3e `DBFTPlugin` integration rather than the
