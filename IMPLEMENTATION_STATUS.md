...
| In-process deposit injection (devnet) | `RpcSharedBridgeDepositSource` + scanner + `InMemorySharedBridgeDepositSource`. Full lifecycle: Scan → **Drain (reserve)** → batch seal → **ConfirmConsumed** (or **ReleaseReservations** on persist failure). `L2BatchPlugin.WireL1MessageInbox` is the production composition root for deposits ± MessageRouter; `L2SettlementPlugin.Wire` / `WireProduction` attach the same inbox before the sealed-batch sink. Unit evidence covers seal-confirm, persist-fail release/retry, and settlement wiring fail-closed. | `ISharedBridgeDepositSource` |
