...
| 4     | NeoVM2 / RISC-V ZK Validity Proof         | ✅ **End-to-end zkVM proving works.** `bridge/neo-zkvm-guest/` ships the Rust crate that `cargo prove build` compiles to a real RISC-V succinct zkVM ELF. `bridge/neo-zkvm-host/` loads it + runs through SP1's actual zkVM (verified by `tests/end_to_end.rs` — 42s real cryptographic proving, asserts zkVM output matches host execution byte-for-byte). RISC-V execution: `Neo.L2.Executor.RiscV` P/Invokes into `external/neo-riscv-vm`. Operator install: `sp1up` (one-time toolchain bootstrap). |
