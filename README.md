...
| Foreign-chain integrations | **4** | `watchers/neo-bridge-watcher-eth/` (Rust crate, secp256k1+SHA256, byte-for-byte parity with the C# encoder, 24 tests) · `watchers/neo-bridge-watcher-tron/` (thin re-export of the Eth crate with Tron chain-ids 0xE0000010..12, 7 tests) · `watchers/neo-bridge-watcher-sol/` (ed25519-dalek-backed `Ed25519FileSigner` + Solana chain-ids 0xE0000020..22, 9 tests; same curve-agnostic `Signer` trait dispatches to `CryptoLib.VerifyWithEd25519` on-chain) · `external/foreign-contracts/eth/NeoExternalBridgeRouter.sol` (393-line Solidity router; 13 Foundry tests with real `vm.sign` + `ecrecover`) |
