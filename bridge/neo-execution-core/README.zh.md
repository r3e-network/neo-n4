...
这些图是 `neo-execution-core` 自己目录下的 crate 专属学习资料，用来说明它在 Neo N4 中的位置、自己负责的技术边界、内部工作流，以及数据如何流经它。
...
- **下游使用者:** neo-zkvm-guest、neo-zkvm-host、网关服务
- **扫描到的源码文件:** 6
- **扫描到的公开符号:** 15
- **扫描到的 Rust 测试:** 5
...
- **本 crate 负责:** 校验批次结构、执行确定性状态转换、提交新状态根
- **本 crate 消费:** L2 批次、前序状态根、执行参数
- **本 crate 产出:** 执行轨迹、新状态根、公开证明输入
- **主要被谁使用:** neo-zkvm-guest、neo-zkvm-host、网关服务
...
| 类型 | 代表符号 |
| --- | --- |
| 类型 | BatchResult <br> VmExecutionReceipt <br> ExecutionError <br> BatchRequest +1 |
| 函数 | execute_batch_with <br> merkle_root <br> hash256 <br> hash_receipt +4 |
| Trait | 未扫描到公开符号 |
| 常量 | BATCH_WIRE_VERSION <br> DEFAULT_PER_TX_GAS_LIMIT |
