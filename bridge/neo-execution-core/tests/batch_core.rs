use neo_execution_core::{
    DEFAULT_PER_TX_GAS_LIMIT, ExecutionError, VmExecutionReceipt, execute_batch_with, hash256,
    merkle_root,
};
...
fn hash_public_inputs_golden_value() {
    ...
    let expected: [u8; 32] = [
        0x6B, 0xF4, 0x2E, 0x3D, 0xE8, 0x1D, 0x57, 0x10, 0x09, 0x23, 0xEE, 0xBA, 0x1F, 0xC5, 0x6A,
        0x61, 0xC7, 0x6D, 0x3A, 0x98, 0xDF, 0x36, 0xA4, 0x52, 0xF1, 0xA1, 0x71, 0xD5, 0xDF, 0x6F,
        0xED, 0xCA,
    ];
    assert_eq!(
        result, expected,
        "hash_public_inputs golden value must match C# byte-for-byte"
    );
}

#[test]
fn hash_receipt_is_deterministic() {
    use neo_execution_core::{VmExecutionReceipt, hash_receipt};

    let tx_hash = [0xAA; 32];
    let receipt = VmExecutionReceipt {
        state: 0,
        gas_consumed: 42,
        output_hash: [0xBB; 32],
    };
    ...
    let receipt_fault = VmExecutionReceipt {
        state: 1,
        ..receipt
    };
    assert_ne!(hash_receipt(&tx_hash, receipt_fault), h1);

    let receipt_gas = VmExecutionReceipt {
        gas_consumed: 43,
        ..receipt
    };
...
fn fold_state_root_is_order_dependent() {
    use neo_execution_core::{VmExecutionReceipt, fold_state_root, hash_receipt};
    ...
    let root_b = [0x44; 32];
    assert_ne!(
        fold_state_root(&root_a, &receipt_hash),
        fold_state_root(&root_b, &receipt_hash)
    );
}

#[test]
fn apply_l1_message_is_deterministic() {
    use neo_execution_core::{L1Message, apply_l1_message};

    let root = [0x55; 32];
    let msg = L1Message {
        bytes: b"test-l1-msg".to_vec(),
    };
...
fn reject_oversized_per_element() {
    use neo_execution_core::{ExecutionError, parse_batch_request};
...
fn gas_exceeded_error_propagated() {
    use neo_execution_core::{ExecutionError, VmExecutionReceipt, execute_batch_with};
...
fn batch_with_l1_messages_and_transactions() {
    use neo_execution_core::{VmExecutionReceipt, execute_batch_with, hash256};
...
fn hash_public_inputs_all_roots_change_output() {
    ...
    let baseline = hash_public_inputs(
        1, 1, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero,
    );

    // Changing any single root should change the output
    assert_ne!(
        hash_public_inputs(
            1, 1, &a, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero
        ),
        baseline
    );
    assert_ne!(
        hash_public_inputs(
            1, 1, &zero, &a, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero
        ),
        baseline
    );
    assert_ne!(
        hash_public_inputs(
            1, 1, &zero, &zero, &a, &zero, &zero, &zero, &zero, &zero, &zero, &zero
        ),
        baseline
    );
    assert_ne!(
        hash_public_inputs(
            1, 1, &zero, &zero, &zero, &a, &zero, &zero, &zero, &zero, &zero, &zero
        ),
        baseline
    );
    ...
    assert_ne!(
        hash_public_inputs(
            2, 1, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero
        ),
        baseline
    );
    assert_ne!(
        hash_public_inputs(
            1, 2, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero, &zero
        ),
        baseline
    );
}
