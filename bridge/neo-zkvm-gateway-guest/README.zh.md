...
- 当已证明的 constituent 消息根树确实为零时，零 `globalMessageRoot` 是合法规范值；
  发布存在性必须单独记录，禁止从根值推断。
...
生产发布工件必须通过 `neo-zkvm-gateway-host` 生成。`batch_vk_manifest.rs` 是唯一编译
进本 guest 的验证键；独立的 `vk_manifest.rs` 固定 host 校验的 SP1 版本、batch/Gateway
验证密钥与两个 guest ELF 摘要，避免 Gateway ELF 对自己的输出哈希/VK 形成自引用；
host 构建会重新推导并逐项比较。
