下面是一份**完整系统架构设计**，目标是把当前 `github.com/neo-project/neo` master branch 的 Neo 4 core 作为 **Neo 4 L2 chain 的执行内核**，同时借鉴 ZKsync Elastic Chain 的 shared bridge、chain registry、proof aggregation、native interoperability 思路，构建一个类似 **Neo Elastic Network** 的多 L2 网络。
...
每条 L2 使用 `neo-project/neo` master branch 作为执行内核，但必须新增 **L2 mode**。
...
`neo-project/neo` 是完整节点实现，包括 P2P、RPC、plugins、wallet、mempool、DB 等。真正 L2 proof 不应该证明整个 node，而应该证明 deterministic state transition function：
...
```text
Deliverables:
1. fork / configure neo-project/neo master
2. new Network ID
3. L2 genesis config
...
在 `neo-project/neo` 基础上，不建议把所有 L2 逻辑直接塞进 core。更好的方式是核心抽象 + 插件/模块扩展。
