下面是一份**完整系统架构设计**，目标是以 `github.com/r3e-network/neo` 的 `r3e/neo-n4-core` 分支作为 **Neo 4 L2 chain 的执行内核**。该分支从 `github.com/neo-project/neo` master 同步并承载 N4 所需的 native contract、ChainMode 和执行内核改动；同时借鉴 ZKsync Elastic Chain 的 shared bridge、chain registry、proof aggregation、native interoperability 思路，构建一个类似 **Neo Elastic Network** 的多 L2 网络。
...
每条 L2 使用 `r3e-network/neo` 的 `r3e/neo-n4-core` 分支作为执行内核，该分支跟踪 `neo-project/neo` master，但必须新增 **L2 mode**。
...
`r3e-network/neo` 的 `r3e/neo-n4-core` 分支基于 `neo-project/neo` 完整节点实现，包括 P2P、RPC、plugins、wallet、mempool、DB 等。真正 L2 proof 不应该证明整个 node，而应该证明 deterministic state transition function：
...
```text
Deliverables:
1. fork / configure `r3e-network/neo` branch `r3e/neo-n4-core` from `neo-project/neo` master
2. new Network ID
3. L2 genesis config
