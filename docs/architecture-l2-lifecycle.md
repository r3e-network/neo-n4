...
- `TokenRegistry` stores both L1 and L2 decimals. Platform mappings pin NEO at
  0→8, GAS at 8→8, USDT/USDC at 6→6, and BTC at 8→8.
...
| Native SP1 executor | Runs the digest-pinned shared guest runtime, validates a complete transition, then replays and atomically commits state only after the immutable proof artifact is durable | `neo-zkvm-executor` + `Sp1StatefulBatchExecutor` |
...
`ApplicationEngineTransactionExecutor` remains a compatibility/reference path.
...
For the production SP1 profile, complete pre-state `NEO4STW1` plus `NEO4EXEC` enter the
pinned native executor; validated `NEO4EXR1` atomically commits post-state and becomes one
canonical `NEO4PWIT`; the daemon re-executes that artifact inside SP1 before any proof reaches
`SettlementManager`.
...
unrepresentable fractional L1 NEO payout. GAS stays 8→8, USDT/USDC stay 6→6,
and BTC stays 8→8. Because these platform L2 asset ids are chain-invariant,
cross-L2 transfers can burn/lock on the source L2, route through NeoHub or the
Gateway message path, and mint the same catalog asset on the target L2 without
application-level symbol remapping.
...
The watcher daemon is production-shaped (graceful SIGTERM, `/healthz`,
...
```bash
# Generate the 24-step ordered production bundle:
