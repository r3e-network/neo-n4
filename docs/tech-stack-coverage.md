...
| RISC-V ZK (Stage 2) prover/verifier — full path | ✅ | `src/Neo.L2.Proving.RiscVZk/` + `src/Neo.L2.Proving.Sp1/` (C# P/Invoke path through `bridge/neo-zkvm-bridge/` cdylib over the upstream `external/neo-zkvm` submodule — Rust 1.75-pinned, sp1-zkvm 5.2). Plus a modern, pure-Rust path: `bridge/neo-zkvm-guest/` (compiles to RISC-V ELF via `cargo prove build` on sp1-zkvm 6.0; **executes real Neo N3 VM** for every batch tx by depending on `external/neo-zkvm/crates/neo-vm-guest` — proof attests to opcodes / gas accounting / halt-or-fault / top-of-stack result, not just hashed bytes) + `bridge/neo-zkvm-host/` (sp1-sdk 6.0 orchestrator with `execute()` / `prove()` / `verify()` API + `prove-batch --prove <hex>` CLI that writes proof.bin + proof.vk for on-chain submission). End-to-end verified: zkVM execution agrees byte-for-byte with host-mode Neo VM execution (~42s default test); real CPU proof generation + verification + tampered-hash rejection covered by 2 `#[ignore]`-gated tests (~3.5 min combined, run pre-release). |
