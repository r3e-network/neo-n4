...
1. `test` — `dotnet test Neo.L2.sln` (1430 tests, 34 projects)
2. `contracts` — installs `Neo.Compiler.CSharp`, type-checks all 23 deployable NeoHub
   contracts plus the 2 sample contracts, asserts 25 `.nef` + 25 `.manifest.json`
