...
上重新构建:dBFT 2.0 终结性、NEP-17、NeoVM2/RISC-V 执行、NeoFS。
...
- **DAWriter** —— 默认把批次数据写到 NeoFS DA；L1 DA、外部 DA、DAC 是显式覆盖项
...
- **NeoFS DA** —— 默认/推荐路径，Neo 生态原生、内容寻址、可检索。
- **L1 DA** —— 显式高成本覆盖项，面向必须把每个字节都放到 L1 的链。
...
| 问题                  | 选择                              | 原因                                     |
| --------------------- | --------------------------------- | ---------------------------------------- |
| L2 执行内核            | Neo 4 core                        | 运行 NeoVM2/RISC-V,并复用原生合约、工具链 |
