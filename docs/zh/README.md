...
- **测试(1467 .NET + 202 跨语言)** —— 34 个 .NET 工程的 1467 条测试;
  16 TS + 10 Rust SDK + 5 shared execution-core + 7 SP1 guest +
  103 Rust 桥 watcher(eth 87 / tron 7 / sol 9) + 39 Foundry + 22 Solana router —— 全绿。
...
| [`crate-visual-guide.md`](./crate-visual-guide.md)                       | ???????        | ?? Rust crate ????????,????????????????       |
