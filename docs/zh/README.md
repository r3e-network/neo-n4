...
> 路线图归 Neo 项目所有。代码正在做生产加固：真实密码学与持久化组件已经交付，
> 但通用 NeoVM fraud proof、Gateway 独立审计和真实递归证明部署证据仍是明确门禁。请像看待
> 任何第三方公链协议实现那样对待它:阅读[安全模型](security-model.md)、
> 主网使用前进行审计、并按你的部署需求接入有文档的生产接缝
> (运维密钥/HSM 策略、真实 NeoFS 适配器、经过审查的 dBFT 节点发布包)。
...
- **Neo Gateway(Phase 5,可选)** —— 把多条 L2 的证明聚合为 L1 上的单次结算
  提交。`BinaryTreeAggregator` 在 log-N 轮内归约；`MultisigRoundProver` 与
  `MerklePathRoundProver` 提供非递归 round，`PassThroughRoundProver` 仅为参照；
  `neo-zkvm-gateway-{guest,host}` 提供 SP1 6.2.1 递归终端证明。
...
- **核心链下库(16)** —— `Neo.L2.{Abstractions, Audit, Batch, Bridge,
  Censorship, Challenge, Executor, Executor.RiscV, ForcedInclusion,
  ExternalBridge, Messaging, Persistence, Proving, Sequencer, State,
  Telemetry}`。
- **RPC adapter 库(2)** —— `Neo.L2.Gateway.Rpc`、`Neo.L2.Settlement.Rpc`。
...
- **智能合约（26 个 NeoHub + 10 个 L2 原生）** —— 26 个 NeoHub L1 合约项目（24 个生产合约 + 1 个仅审计用 `GovernanceFraudVerifier` + 1 个测试 stub，包含 `ContractZkVerifier`、immutable `Sp1Groth16Verifier` 与 `L2PayoutAdapter`）+ 10 个 L2 原生合约（均经
...
- **应用 SDK 源码实现(4)** —— `src/Neo.L2.Sdk/`(.NET)· `sdk/typescript/`
  (`@neo-n4/sdk`)· `sdk/rust/`(`neo-n4-sdk`)· `sdk/python/`(`neo-n4-sdk`)——
  共享相同的十方法线上协议、四类错误模型，以及
  [`sdk-conformance.md`](./sdk-conformance.md) 所述的离线/真实节点一致性测试；不声称已有对应 package 发布证据。
- **静态 Web 体验(4)** —— `sdk/web-explorer/index.html`（运维浏览器）·
  `experience-hub/index.html`（架构导览）· `interactive-runtime/index.html`
  （运行时剧场）· `interactive-math/index.html`（证明数学实验室）。
- **Rust 证明者/核心(5)** —— `bridge/neo-execution-core/`(共享规范执行语义)·
  `bridge/neo-zkvm-guest/`(SP1 RISC-V 程序 + 同 runtime host-native executor)·
  `bridge/neo-zkvm-host/`(batch SP1 证明生产者)·
  `bridge/neo-zkvm-gateway-{host,guest}/`(Gateway 递归 SP1 证明生产者/程序)。
- **Submodule(5)** —— `external/neo`(`r3e-network/neo` fork，L2 分支 `r3e/neo-n4-core`；同一 fork 内的 L1 core 分支为 `r3e/neo-n3-core`)、`external/neo-devpack-dotnet`、
  `external/neo-riscv-vm`、`external/neo-zkvm`、`external/neo-vm-rs`。
- **测试(38 个 .NET 测试工程 + 跨语言门禁)** —— solution 动态发现当前用例，加 TypeScript、Rust SDK/core/watchers/zkVM、Python SDK、Node 体验层、vendored `neo-zkvm` / `neo-riscv-vm`、Solidity、Solana 和 SP1 发布证明门禁。
...
| 阶段 | 目标 | 设计 | 代码形态 | 集成路径 | 密码学强制 | 当前版本部署证据 | 生产完备 |
| ---- | ---- | :--: | :------: | :------: | :--------: | :--------------: | :------: |
| 0 | 侧链 PoC | ✅ | ✅ | ✅ 本地 | N/A | ❌ | ❌ |
| 1 | NeoHub v0 + 共享桥 | ✅ | ✅ | ✅ 本地 | 🟡 取决于安全档 | ❌ | ❌ |
| 2 | 批次结算 | ✅ | ✅ | ✅ 本地 | 🟡 取决于证明档 | ❌ | ❌ |
| 3 | 乐观挑战窗口 | ✅ | 🟡 受限 v4 | 🟡 受限状态转移 | 🟡 仅精确 v4 | ❌ | ❌ |
| 4 | Stateful NeoVM V1 / SP1 RISC-V 有效性 | ✅ | ✅ | ✅ 本地 + CI | ✅ 精确语义 native executor + 固定 SP1 Groth16 路径 | ❌ | ❌ |
| 5 | Neo Gateway 聚合 | ✅ | ✅ | ✅ 本地 + CI | 🟡 取决于有效性档 | ❌ | ❌ |
| 6 | Neo Stack CLI / 模板 | ✅ | ✅ | 🟡 仍需钱包适配器 | N/A | ❌ | ❌ |

图例：✅ 当前版本已有证据 · 🟡 部分完成或依赖配置 · ❌ 缺失。任何一行都不等于
生产完备声明；完整证据矩阵见 [`IMPLEMENTATION_STATUS.md`](../../IMPLEMENTATION_STATUS.md)。

按工程的覆盖详情:[`IMPLEMENTATION_STATUS.md`](../../IMPLEMENTATION_STATUS.md)。

> **L1 信任模型 —— 今天结算实际验证的是什么。** 链上结算信任三种明确档位之一：
> `ProofType.Multisig`（Stage 0）的 secp256r1 委员会；`ProofType.Optimistic`
> （Stage 1）的受限可执行欺诈证明窗口；或 `ProofType.Zk`（Stage 2）的
> `ContractZkVerifier` + immutable `Sp1Groth16Verifier`，后者执行固定 SP1
> Groth16/BN254 pairing equation。生产部署计划永久禁用 SP1 `envelope-only`；
> 该不安全捷径仅允许私有 devnet 显式启用。生产执行还必须使用 SHA-256 锁定、与 guest
> 同 runtime 的 `neo-zkvm-executor`；独立 PolkaVM profile 没有匹配 prover 时不得声明
> validity。完整的按 `ProofType` 拆解见
