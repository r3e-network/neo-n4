...
- **Rust 证明者/核心(5)** —— `bridge/neo-execution-core/`(共享规范执行语义)·
  `bridge/neo-zkvm-{host,guest}/`(batch SP1 证明生产者/程序)·
...
| 5    | Neo Gateway 证明聚合                 | 🟡   | 递归 SP1、持久发布、精确 finalized constituent 绑定及原子 `SettlementManager → MessageRouter` 发布已交付；独立审计和真实递归证明部署证据仍是发布门禁 |
