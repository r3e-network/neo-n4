...
- **`RestrictedExecutionFraudVerifier`** —— 结构性 v3 fraud verifier
  —— 在链上从每个 storage 证明的叶 hash + sibling + leafIndex 重建
  pre/post Merkle 状态根,检查它们与挑战者所提交 payload header 中的
  `PreStateRoot` / `ReplayedPostStateRoot` 内部自洽。所有状态根都来自
  同一份挑战者 payload:它不读取排序器在 `SettlementManager` 上已承诺的
  批次状态根,也不重放争议 tx,因此并非无信任。被接受的 v3 payload 仍需
  council 仲裁,且该 verifier 必须注册为仅限批准(approved-only,绝不
  走无许可自动罚没路径)。绑定已承诺状态根的链上完整无信任重放仍属
  规划中(roadmap)。
...
L1 委员会成员集合、成员数量、M-of-N 阈值与 timelock 在 `GovernanceController._deploy`
中一次性写入,没有链上修改入口:委员会在该部署的整个生命周期内不可变。轮换成员或
更改阈值需要重新部署一份 `GovernanceController` 并把每个消费者重新接线到新 hash。
没有链上 NEO 持有者公投。链上委员会轮换与 NEO 持有者公投仍属规划中(roadmap)。
...
| 治理                  | Neo 委员会(按部署不可变;轮换 + NEO 持有者公投仍属规划中) | DAO + 安全委员会         | Optimism 基金会 + 委员会       | Arbitrum DAO + 安全委员会        |
