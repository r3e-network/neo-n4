...
24 个生产合约加 1 个仅测试 stub。

<p align="center">
  <img src="figures/architecture/neohub-anatomy.svg" alt="NeoHub L1 解剖:25 个项目(24 个生产合约 + 1 个测试 stub)按关注点分组 —— Settlement(SettlementManager + VerifierRegistry + ContractZkVerifier + immutable Sp1Groth16Verifier)、Bridge、Messaging、Security、Governance 与 Phase B/C External Bridge。ExternalBridgeStubVerifier 仅测试用" width="900">
</p>
...
- **`RestrictedExecutionFraudVerifier`** —— v3 从挑战者 payload 的 storage
  证明重新派生 pre/post roots，但不绑定已提交批次且不执行交易，因此必须经
  approved-only 注册与 council 仲裁。v4 绑定 `SettlementManager` 已提交的
...
25 个 NeoHub 项目(24 个生产合约 + 1 个测试 stub)都对
...
- **Stage 1 —— `OptimisticVerifier`。**生产者:`OptimisticProofPayload`
  + 排序器签名。状态:Stage-1 verifier;已交付的 fraud verifier 是
  结构性 / 治理仲裁式的(它们校验 payload 结构与自洽性,并非绑定已承诺
  状态根或重放的自包含密码学证明)。`BisectionGame` 是链下的 log-N
  争议 tx 收敛。
...
| L1 结算合约           | NeoHub(24 个生产合约 + 1 个测试 stub) | BridgeHub + SharedBridge + V.R. | OptimismPortal 等             | RollupCore + Inbox              |
