...
SharedBridge、VerifierRegistry、MessageRouter 合并成一个套件。共 25 个项目:
23 个生产合约、1 个仅审计用结构验证器和 1 个仅测试 stub。

<p align="center">
  <img src="figures/architecture/neohub-anatomy.svg" alt="NeoHub L1 解剖:25 个项目(23 个生产合约 + 1 个仅审计用结构验证器 + 1 个测试 stub)按关注点分组 —— Settlement(SettlementManager + VerifierRegistry + ContractZkVerifier + immutable Sp1Groth16Verifier)、Bridge、Messaging、Security、Governance 与 Phase B/C External Bridge。ExternalBridgeStubVerifier 仅测试用" width="900">
</p>
...
25 个 NeoHub 项目(23 个生产合约 + 1 个仅审计用结构验证器 + 1 个测试 stub)都对
...
| L1 结算合约           | NeoHub(23 个生产合约 + 1 个仅审计用结构验证器 + 1 个测试 stub) | BridgeHub + SharedBridge + V.R. | OptimismPortal 等             | RollupCore + Inbox              |
