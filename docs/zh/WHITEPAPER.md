...
<p align="center">
  <img src="figures/architecture.svg" alt="Neo Elastic Network —— 三层架构:L1 NeoHub 锚(26 个项目中的 24 个生产合约按 Settlement、Bridge、Messaging、Security、Governance 分组;ContractZkVerifier 把 ZK 证明路由到可部署验证器合约)、可选的 Phase 5 Neo Gateway 聚合器、N 条弹性 L2 链" width="900">
</p>
...
SharedBridge、VerifierRegistry、MessageRouter 合并成一个套件。共 26 个项目：
24 个生产合约、1 个仅审计用结构验证器和 1 个仅测试 stub。

<p align="center">
  <img src="figures/architecture/neohub-anatomy.svg" alt="NeoHub L1 解剖：26 个项目（24 个生产合约 + 1 个仅审计用结构验证器 + 1 个测试 stub）按关注点分组 —— Settlement（SettlementManager + VerifierRegistry + ContractZkVerifier + immutable Sp1Groth16Verifier）、Bridge、Messaging、Security、Governance 与 Phase B/C External Bridge。ExternalBridgeStubVerifier 仅测试用" width="900">
</p>
...
- **`ChainRegistry`** —— 注册 / 配置 / 暂停 L2 链。每条记录:
  ...
  active}`。
- **`SharedBridge`** —— 托管规范 GAS / NEO / USDT / USDC / BTC / NEP-17。lock-mint 与
...
- **`OptimisticChallenge`** —— Phase-3 挑战窗口;二分挑战游戏
  fraud-proof 流程的入口。
- **`GovernanceFraudVerifier`** —— 仅审计用 v1/v2 结构
  verifier。解码规范的 `FraudProofPayload`(v1 = 101 字节定长、
  v2 = 105+N 字节带争议 tx witness),校验结构完整性,发出
  accept/reject 事件与 reason code，但不进入生产 challenge 路径。
- **`RestrictedExecutionFraudVerifier`** —— v3 从挑战者 payload 的 storage
  证明重新派生 pre/post roots，但不绑定已提交批次且不执行交易，因此仅供审计。
  `OptimisticChallenge` 即使收到 governance witness 也拒绝 v3。v4 绑定 `SettlementManager` 已提交的
  规范头/roots、交易证明、claim id、受限 transcript 与 replay domain，并在
  链上执行精确注册的单笔、已有 key 的 Counter Increment 语义。通用 NeoVM、
  多交易与其它 custom executor 语义 fail closed。

26 个 NeoHub 项目（24 个生产合约 + 1 个仅审计用结构验证器 + 1 个测试 stub）都对
`Neo.SmartContract.Framework` 类型检查通过。`Neo.Hub.Deploy` 工具发出
拓扑排序 + 依赖解析过的 24 步生产部署 bundle。
...
10 个 L2 链上原生合约,在每条 L2 上以同一形式部署:
...
- `L2NativeExternalBridgeContract` —— 外链桥 L2 侧 burn/mint 对应合约。
- `BridgedNep17Contract` —— 规范 bridged NEP-17 表示。
- `L2AccountAbstraction` —— validator/paymaster/nonce entry point。
- `L2InteropVerifier` —— global message-root 镜像与包含证明验证。
...
内置生产 profile 中，C# 不再独立重写这份函数。`Sp1SettlementExecutionStack` 用
...
```
Stage 0 —— 多签 attestation        (具备生产形态；仍需运维门禁)
Stage 1 —— 乐观 + 二分挑战游戏     (精确 executable-v4 挑战流程)
Stage 2 —— ZK 有效性证明           (SP1 RISC-V guest 内的 stateful NeoVM V1)
```
...
- **Stage 0 —— `AttestationVerifier`。**生产者:`AttestationProver` +
  `ISignerSet`。状态:具备生产形态，但当前 revision 尚无独立审计或部署证据；
  在规范 public-input 字节上执行 M-of-N secp256r1。
- **Stage 1 —— `OptimisticVerifier`。**生产者:`OptimisticProofPayload`
  + 排序器签名。状态:结构性 v1/v2/v3 只供审计且 fail closed；已交付的
  executable v4 只覆盖精确注册的单笔 Counter 语义并绑定 committed roots。
  `BisectionGame` 是链下的 log-N 争议 tx 收敛；通用 NeoVM fraud proof 尚未完成。
- **Stage 2 —— `ContractZkVerifier` → `Sp1Groth16Verifier`。**生产者是 SHA-256
  锁定的 `neo-zkvm-executor` + `Sp1BatchProofProver`/`prove-batch daemon`(真实进程外)；
  `MockRiscVProver` 仅为进程内测试 seam。native execution 与 SP1 guest 共享精确
  stateful runtime/semantic id，全部请求、结果与 public-input 校验后才原子提交完整状态。
  immutable terminal 合约执行 SP1 6.2.x wrapper 的 BN254 Groth16 校验。N4 genesis V1
  的有界 native/syscall 与不可变 deployed-contract descriptor 集合是明确语义上限；
  unsupported 行为 fail closed，不能从另一 executor 继承 validity 标签。
...
<p align="center">
  <img src="figures/forced-inclusion.svg" alt="强制纳入问责序列 —— 用户在 L1 入队强制交易，L2 批处理器轮询；逾期后任何人可报告并暂停链，治理仅在审阅已最终化 dBFT 归责证据后才能罚没" width="900">
</p>
...
1. **强制纳入队列**(`NeoHub.ForcedInclusion`、`Neo.L2.ForcedInclusion`、
   ...
   `CensorshipReport`（`Neo.L2.Censorship.CensorshipDetector`），它可以在不虚构
   proposer 身份的情况下暂停链。

2. **排序器抵押**(`NeoHub.SequencerBond`、`NeoHub.SequencerRegistry`)。
   每个活跃排序器委员会成员都缴抵押；治理仅在已最终化 dBFT 证据完成归责后才能
   对已报告事件罚没指定成员，permissionless report 本身绝不直接罚没。

3. **逃生通道**(`NeoHub.EmergencyManager`)。排序器侧 liveness 失败被确认后,
   治理可以暂停 L2。暂停期间,链上自动出款仅适用于排序器已经最终化的提款,
   经 `SharedBridge.EmergencyFinalizeWithdrawalWithProof`(对批次 `withdrawalRoot`
   校验)完成。`EmergencyManager.EscapeHatchExit` 针对状态叶并不转移任何资产:
   它记录一笔防重放的退出 CLAIM(按 `(chainId, leafHash)`)并发出事件;一笔
   从未最终化的状态叶退出只会生成一个 claim,由治理 / 链下结算据此释放托管资金。
   直接从 L1 完成的完整自动状态叶出款仍属规划中(roadmap)。
...
| 层级       | 权威                                                  | 控制什么                                                                           |
| ---------- | ----------------------------------------------------- | ---------------------------------------------------------------------------------- |
| L1         | Neo 委员会(`GovernanceController`)                  | NeoHub 升级、verifier registry、桥升级、紧急暂停、L2 准入策略                      |
| L2 本地    | L2 自己的治理合约                                      | 排序器委员会、本地费策略、app-chain 参数、DA 模式(在批准范围内)                  |
| App        | 各 dApp / RWA 发行人 / 稳定币策略                     | 按 app 规则、KYC 名单、企业准入                                                    |

L1 委员会与门限在 `GovernanceController._deploy` 初始化，之后可经 proposal-bound、
threshold-approved、timelocked 的 `RotateCouncil` 原子替换。每次轮换递增
`councilEpoch`、使旧 epoch 的全部 proposal 失效，并按 proposal id 防重放。
timelock 参数仍在部署时固定，合约也没有 `ContractManagement.Update` 路径。
链上 NEO 持有者公投仍属规划中(roadmap)。
...
| #  | 威胁                            | 主要缓解                                                              |
| -- | ------------------------------- | --------------------------------------------------------------------- |
| 1  | 排序器审查                      | 强制纳入 + 无许可暂停 + 基于最终 dBFT 归责证据的治理罚没 + 逃生通道(§10) |
...
| L1 结算合约           | NeoHub（24 个生产合约 + 1 个仅审计用结构验证器 + 1 个测试 stub） | BridgeHub + SharedBridge + V.R. | OptimismPortal 等             | RollupCore + Inbox              |
...
| 治理                  | Neo 委员会(epoch-bound timelock 轮换;NEO 持有者公投仍属规划中) | DAO + 安全委员会         | Optimism 基金会 + 委员会       | Arbitrum DAO + 安全委员会        |
...
| **NeoHub**                 | 每条 L2 共享的 26 项目 L1 套件：24 个生产合约、1 个仅审计用 verifier、1 个测试 stub。           |
...
| **二分挑战游戏**            | 链下的 log-N 收敛优化(`Neo.L2.Challenge.BisectionGame`),收敛到单笔争议 tx 的索引。收敛得到的索引是元数据:没有链上二分合约,`OptimisticChallenge.Challenge` 是单次的(一次 `verifyFraud` 调用,无链上重放)。链上交互式二分仍属规划中(roadmap)。 |
