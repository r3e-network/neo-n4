...
```text
   1. 别的 L2 需要读这段状态吗?
        ├─ 是 → 上 L1(例如 ChainRegistry、MessageRouter)
        └─ 否  → 也许 L2;继续。

   2. 这段状态可罚没 / 承担经济安全吗?
        ├─ 是 → L1(例如 SequencerBond、ExternalBridgeBond)
        └─ 否  → 也许 L2;继续。

   3. 这是*定义*在 L1 上什么合法的东西(信任边界)吗?
        ├─ 是 → L1(例如 SettlementManager、VerifierRegistry)
        └─ 否  → 也许 L2;继续。

   4. 吞吐量超过 L1 能处理的(tx/秒)吗?
        ├─ 是 → 必须 L2(例如用户转账、DEX 撮合)
        └─ 否  → 都行;默认 L2 让 L1 苗条。

   5. 这是单条链的应用特定逻辑吗?
        ├─ 是 → L2(例如 RWA KYC 检查、自定义执行器)
        └─ 否  → 重新评估上面。
```
