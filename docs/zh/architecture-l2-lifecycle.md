...
- `IRoundProver` —— 2 份非递归实现、1 份参照与内置递归终端路径:
  - `MultisigRoundProver` —— Secp256r1 阈值证明轮
  - `MerklePathRoundProver` —— 逐成员 包含证明
  - `PassThroughRoundProver` —— 最低成本参照
  - `neo-zkvm-gateway-{guest,host}` —— 内置 SP1 6.2.1 递归终端证明
  - Halo2 / Risc0 可作为替代实现接入同一 seam

L1 入口固定为 `SettlementManager.PublishGatewayGlobalRoot`，而不是直接调用 Router。
它绑定 1..4096 个精确 finalized constituent identities，以有界 streaming frontier 重建
两棵 proof-bound roots，推进每链不可回退 publication watermark，并原子转发到
`MessageRouter.PublishGlobalRoot`。
