...
归责尚未证明时可传 `UInt160.Zero`。无许可报告只会至多一次记录 overdue 并暂停链，
不会直接罚没。治理审阅已最终化 dBFT 证据后，再单独调用 owner-gated 的
`SlashReportedCensorship`；只有该路径才调用 `SequencerBond.Slash` 扣减已证实成员的
保证金。
