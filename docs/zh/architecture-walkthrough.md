...
| §4 Neo Gateway          | 证明聚合               | `Neo.Plugins.L2Gateway`(含 `BinaryTreeAggregator` + `IRoundProver`)         |
...
| §9 代币 / GAS 模型       | 桥接资产记账           | `Neo.L2.Bridge.AssetRegistry` + `L2Native.L2BridgeContract`                   |
| §10 Neo Connect         | 跨链消息传递           | `Neo.L2.Messaging.*` + `L2Native.L2MessageContract`                           |
...
| §13 L2 原生合约         | L2 上系统合约           | `contracts/L2Native.*/`(6 个合约)                                          |
...
| §15.2 充值               | L1→L2                  | `Neo.L2.Bridge.DepositProcessor` + `L2Native.L2BridgeContract.ApplyDeposit`   |
