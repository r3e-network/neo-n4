...
嵌入到从 `NeoHub.SharedBridge` 跑向 Neo Core 原生 `L2BridgeContract` 的 `CrossChainMessage`
的 `payload` 字段里。
...
amount 使用源域的最小单位。对 NEO 充值,这表示 L1 的整枚 NEO 单位
(`l1Decimals = 0`);L2 原生桥解析 `TokenRegistry` / `L2BridgeContract` 元数据后,
把它缩放到映射后的 L2 NEO 表示(`l2Decimals = 8`)。提款时,`WithdrawalRecord`
里的 amount 已经是规范 L1 支付金额,因此在记录发出前会拒绝有损降精度。
对平台目录资产,USDT/USDC 两侧保持 6 decimals,BTC 两侧保持 8 decimals；L2 asset id
在所有 L2 上保持一致,所以跨 L2 路由可以携带同一 token identity,不需要应用层维护额外
重映射表。
