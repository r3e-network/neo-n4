...
- N4 Layer-2 运行时模块、r3e Neo core fork 中的 L2 原生合约，以及
  NeoVM2/RISC-V 执行边界。
- NeoFS 数据可用性接入点和验证证据。
- 通过可部署 NeoHub 合约加 L1 native accelerator hook 的 ZK 验证路径。
...
### 已复核：历史文件名 `StubCommands.cs` 容易误导，但实现是功能性的

宽泛 placeholder 扫描命中了
`tools/Neo.Stack.Cli/Commands/StubCommands.cs`。人工复核后确认，该文件不是空实现
...
- 本检查点不需要做正确性修复。
- 后续可以单独做一次纯整理型重命名，降低审计噪音，但不应和保证证据混在同一改动里。
...
| L1 native ZK accelerator | 架构边界正确。 | 重型 ZK 数学放在 native accelerator hook；`NativeZkVerifier` 仍是 NeoHub 治理和结算流暴露的可部署适配器。 |
