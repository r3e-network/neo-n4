...
| `r3e-network/neo` | r3e 维护的 Neo core fork，负责无法放在合约或插件中的最小 core 改动，例如 L2 native contracts、ChainMode、执行内核 hook、共识/RPC 改动。 |
...
| 层级 | 上游基线 | r3e 维护分支 | 用途 |
| --- | --- | --- | --- |
| L1 core | `neo-project/neo` `master-n3` | `r3e/neo-n3-core` | Neo N3 L1 节点行为，以及确实不能由部署合约或插件实现的最小 core hook。 |
...
```mermaid
flowchart TD
    change["请求的 N4 改动"] --> l1{"需要 Neo N3 L1 节点行为\n且不能用合约或插件实现？"}
...
NeoHub L1 contracts 属于另一条边界：它们是部署在 L1 上的合约，不是 Neo core
native contracts。它们对标 ZKsync L1 Bridgehub/shared-bridge 生态，但目标是让
Neo N3 L1 节点尽量贴近 upstream。NeoHub 的规范实现保留在
`r3e-network/neo-n4` 的 `contracts/NeoHub.*`；`neo-hub-deploy` 负责生成生产部署
bundle 和 post-deploy wiring 提示。

`r3e/neo-n3-core` 不应在 `NativeContract` 中注册 NeoHub 业务合约，也不应保留
`src/Neo/SmartContract/Native/NeoHub` 实现。只有在某个能力不能通过下列方式实现时，
才考虑进入 L1 core fork：

- 可部署 NeoHub 合约；
- Neo 节点插件；
- SDK、CLI、watcher、relayer 或运维服务；
- 受控配置或 RPC 扩展。

如果未来某个需求看起来需要 L1 core 支持，先设计合约/插件版本。只有在文档说明为什么
部署合约或插件无法满足之后，才把最窄的 hook 放进 `r3e/neo-n3-core`。

必须进入 fork 的典型改动包括：

- 必须存在于 Neo N3 L1 节点内部、且不能由部署合约或插件实现的最小 L1 节点行为。
