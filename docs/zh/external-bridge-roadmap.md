...
4B  externalChainId       (Eth 主网=0xE0_00_00_01,Sepolia=0xE0_00_00_02,
                           ...
                           空间下保留)
4B  neoChainId            (目标 Neo 域；0 = L1，非零 = L2 chainId)
...
规范线值严格为 `102 + N` bytes。`MessageHash` 是 `ExternalMessageBuilder` 从这些
bytes 计算的链下派生元数据，算法为 Neo `Hash256`（double-SHA256）；它不作为调用方
提供的尾字段在线序列化。escrow 在派发前校验已签名的 source/destination domain、
direction、deadline、payload length 与资产 payload。
...
`AssetAndCall` 在此前缀后追加 adapter calldata。C#、Rust 与链上合约一致拒绝零资产、
零金额、非最短金额编码和超过 uint256 的金额。

### 1.4 生产 payout 边界

- 零 adapter 仅允许 escrow 显式绑定 Neo L1（`neoChainId == 0`）时使用；它从按
  `(externalChainId, neoAsset)` 隔离的注资池直接调用 NEP-17 `transfer`。余额不足或
  token 返回 `false` 会原子回滚余额与 replay marker。`FundLiquidity` 只接受已建立且
  active 的 direct route，避免把 token 注入不可领取的孤立池。
- 非零 adapter 接收所有已签名的路由/价值字段与原始规范 bytes。它必须报告
  `payoutVersion() == 1`，且是 `UpdateCounter == 0` 的全新部署；route 固定该 counter，
  任何原地更新都 fail closed。
- 非零 L2 目标必须配置非零 adapter。adapter 只有在目标 credit/mint 指令已经同一
  NeoVM 事务持久化或入队后才能返回 `true`；最终目标链执行是异步的。
- foreign→Neo asset 映射不可改写，并在每个外链域内保持 reverse-unique。生产环境
  连接 `GovernanceController` 后调用不可逆 `LockGovernance()`；之后只能通过精确绑定
  escrow script hash、目标 Neo 域及全部参数的一次性 timelocked proposal 升级 route
  或 registry。

## 2. 密码学

Neo 的 `CryptoLib` 已经把 **secp256k1 + SHA256** 的 `VerifyWithECDsa` 和
`VerifyWithEd25519` 暴露成原生 syscall。所以:

- **Eth/Tron**:链上 secp256k1+SHA256 委员会签名验证便宜且现成。Keccak256 仅用于
  EVM event topic / 地址生态，不用于 Neo 委员会签名或消息身份。
...
- `watchers/neo-bridge-watcher-eth/` —— ethers-rs RPC 客户端。订阅 Eth 侧
  ...
  每个事件,构造规范 `ExternalCrossChainMessage`,用 secp256k1 签名,发到 Neo 上
