...
3 条路径,都在默认 `neo-hub-deploy plan` 的 22 步 bundle 中:
...
`register-chain` 起作用之前,22 个生产 NeoHub 合约必须部署到目标 L1。仅测试用的
...
```bash
# 1. 脚手架一个起步 plan(22 个生产 NeoHub 部署步骤按依赖序,
...
所有 22 步部署 + post-deploy 接线完成后,把**真实链上**合约哈希(由你的钱包从
