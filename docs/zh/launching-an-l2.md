...
#    带这四个 UInt160 哈希以及审阅后的非零创世状态根：
#    输出需要一并提交的规范 91 字节 configBytes 与不可变根。
...
`L2SettlementPlugin.WireProduction(...)` 是 L1 结算的生产 composition root。它会按
明确配置创建 network-pinned RPC sender、settlement client，以及成对的 finalized
forced-inclusion scanner/source/finalizer；私钥由宿主注入的 HSM/KMS signer 持有，插件
配置不得包含 WIF 或私钥。

ZK validity 档不能把 reference executor 传给生产接线。必须先用同一个 caller-owned
持久状态库构造完整 SP1 stack：

```csharp
using var state = new RocksDbKeyValueStore("/var/lib/neo-l2/state");

// 仅首次部署：把经过审阅的 Neo protocol settings 写入这一份状态库。
...
// batch 1 前把非零 root 写入签名 deployment manifest；每次重启都必须读回比较。
...
// 把这个精确值与 ChainRegistry config 原子注册；ZK profile、签名部署 manifest 与
// L1 trust anchor 禁止使用不同的根。
...
`ExecutorSha256Hex` 必须来自已审阅/签名的 release manifest，不能在启动时从同一可变
路径现算并自我背书。每次执行都会边复制边校验 digest，只运行私有 scratch 目录中的
锁定副本；随后校验完整 `NEO4EXR1` 请求、语义、结果、post-state 与 public input，全部
通过后才用一次原子 `ReplaceAll` 提交完整状态。N4 genesis V1 不允许一次 transition
增删替换合约 descriptor，未覆盖的 native/consensus 语义 fail closed；扩大能力必须同步
升级 guest、native executor、VK、链上 verifier route 与跨语言 golden tests。

状态库、signer、DA writer、proof-witness store、forced-inclusion event store 与 queue 都由
宿主拥有并按正常 shutdown 顺序释放。该本地闭环不等于已有同版本公网部署；公开链标注
`securityLevel=3` 前仍需独立审计、精确 NEF/VK 部署及同一组正负向 smoke evidence。

prover queue 是安全边界，不是临时 scratch。Unix 下 queue/archive 目录强制 `0700`，每个
工件为 `0600`，symlink、非当前 owner 或更宽 mode 都 fail closed。watch+archive 默认合计
上限为 16 GiB 和 64 个 content-addressed task，可用 `--max-queue-bytes` 与
`--max-queue-tasks` 调低。禁止 TTL。只有 proof manifest 已持久记录 `SettlementFinalized`，
settlement pipeline 才写入 `<content-hash>.proof.ack`；daemon 校验其 32-byte 内容后才清理
匹配 request/proof 集合。
...
live deploy 因而必须显式传入 `--fraud-replay-domain`:
  ...
  1. **无许可受限 v4 模式**:默认生产 bundle 用
     ...
     的 Counter Increment；它绑定规范批次头、交易证明、退化 `[0,1]` transcript、
...
用 provider 计算目标活跃集合，再把公钥交给
`SequencerCommitteeTransactionBuilder.BuildSetValidatorsScript`，由 L2 治理提交。
dBFT 不直接轮询链下 provider，而只读取已经最终确认的原生状态，因此 RPC 暂时
不可用或不同运维者观察事件的时序不同，都不会让共识成员集合产生分歧。NeoHub 的
`SequencerRegistry` 仍是准入来源，确认后的 L2 原生交易是共识激活边界。
...
`register-chain` 起作用之前，24 个生产 NeoHub 合约必须部署到目标 L1。仅审计用的
...
# 2. 编辑 plan,填 OWNER_REPLACE_ME / BOND_ASSET_REPLACE_ME /
#    GOVERNANCE_COUNCIL_REPLACE_ME / GOVERNANCE_THRESHOLD_REPLACE_ME 占位
...
# 或运行带生产保护的 live testnet deployer。治理必须显式配置 M-of-N：
# 2..64 个互不重复的压缩 secp256r1 key，threshold 至少为 2。
...
`OptimisticChallenge` 只保留对 `Challengeable` batch 的即时欺诈回滚路径。

> **关于哈希的提示**:bundle 的按步 `Hash` 字段是*确定性 stub*,从步骤名派生
> (这样 `plan` 可在无钱包的情况下复现)。真实的 L1 合约哈希只在你的钱包调
> `ContractManagement.Deploy` 之后才存在。把钱包返回的四个真实哈希与签名 genesis root
> 一并传入下面 `register-chain` 的 5 个必填 flag —— **不要**用 bundle 中的 stub 哈希。

所有 24 步部署 + post-deploy 接线完成后，把**真实链上**合约哈希（由你的钱包从
每次 `ContractManagement.Deploy` 调用返回）和签名 genesis root 传入 `register-chain`
的 5 个 flag:
...
neo-stack register-chain --chain-id 1099 --output ./my-l2 \
    ...
    --genesis-state-root <签名 deployment manifest 中的非零根>
```

这会输出规范 91 字节 `configBytes` 与不可变创世根，让钱包调用
`registerChain(chainId, configBytes, genesisStateRoot)`。该根必须等于审阅后的 genesis
bootstrap 所观察并写入签名 deployment manifest 的值；batch 1 的 `preStateRoot` 不匹配该
链上 trust anchor 时必须拒绝，后续 config 更新也不能替换它。
...
# → 返回不可变、非零的 batch-1 pre-state trust anchor
...
```bash
# 从仓库根目录构建生产 native executor 与两个证明守护进程:
...
# 把 neo-zkvm-executor 的 SHA-256 写入签名 operator release manifest。
...
# 每个文件是规范 ProofWitnessArtifactV1,写入 <batch-number>.batch.bin。
...
# 仅在启用 Gateway 聚合时运行；.NET Sp1GatewayProofProver 会把规范请求与
# readiness manifest 写入 --queue。
...
batch 守护进程 poll `--watch` 找 `*.batch.bin`,为每个产生真实 ZK 证明,写出
`<name>.proof.bin`(链上提交工件)+ `<name>.proof.vk`(verifying key,按 guest
ELF 稳定)；设置 `--gateway-sidecars` 时还会原子发布规范 compressed child sidecar。
`--watch`、`--archive` 与 sidecar 目录必须位于支持 hard link 的文件系统，且 archive
必须与 watch 在同一文件系统。失败时输入留在原地并大声报警,
让监控捕获 poison-pill 批次。
证明成功后同样保留全部 content-addressed 证据；只有 .NET settlement pipeline 已持久观察
L1 结算并发布匹配的 `*.proof.ack`，daemon 才开始清理。watch/archive 删除与 ack 删除均可在
崩溃后幂等恢复。

Gateway 守护进程只接受 tuple 派生的 sidecar 文件名，使用 commitment 与 sidecar 中
补充的两个 hash 重建完整 332-byte public inputs，以编译期 batch VK 递归验证全部
compressed proof，最后输出 host 已验证的 356-byte Gateway Groth16，并最后发布 result manifest。
重启时，只有在精确核对 manifest/工件并再次执行终端 Groth16 验证后，完整 result 才能
幂等跳过；没有 marker 时，只清理 regular non-symlink orphan 后重新证明。batch daemon
对 proof/VK/public-values 三件套执行同样的 fail-closed 恢复策略。

proof-bound RPC publisher 查询 `MessageRouter.GetGlobalRoot*` 做 reconciliation，但签名提交
固定调用 `SettlementManager.PublishGatewayGlobalRoot(epoch,references,globalRoot,
constituentRoot,count,backend,proofSystem,vkId,replayDomain,proof)`。`references` 是 1..4096 个
严格排序的 `chainId:uint32 LE || batchNumber:uint64 LE` packed entries。SettlementManager
...
`neo-execution-core` 和 vendored `neo-vm-rs` stateful N4 V1 runtime。C# 在原子提交
状态前校验规范 `NEO4EXR1` 的精确请求 hash、semantic id、roots、gas、effects、完整
post-state 与 public-input hash；随后 SP1 在 zkVM 内重新执行生成的 `NEO4PWIT`。
因此证明绑定 HALT/FAULT、gas、receipt、storage/event effects、post-state、提款/消息根
与结算 public inputs。篡改任一绑定字节都会被 native 校验或 proof verification 拒绝。
