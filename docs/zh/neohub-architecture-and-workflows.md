...
- `neo-n4` 中有 24 个 `contracts/NeoHub.*` 项目。
- 其中 23 个是生产 NeoHub 合约；`ExternalBridgeStubVerifier` 只用于测试。
- 生产 NeoHub deploy plan 部署其中 23 个合约。
- `NativeZkVerifier` 是可部署 NeoHub 合约,但重型 ZK 证明数学通过 L1 native accelerator 完成,
  不在普通合约字节码里硬算。
...
| 结算 | `SettlementManager`, `VerifierRegistry`, `NativeZkVerifier` | 批次 commitment 校验、proof dispatch、ZK verifier adapter 到 L1 native accelerator、root finalization、batch status。 |
...
`NativeZkVerifier`，后者校验 N4 batch commitment 布局、RISC-V proof payload
envelope、已登记的 verification-key id、以及 public-input hash 边界；随后调用 L1
native accelerator 的 `verifyZkProof(...)` ABI 执行证明系统数学。这样 NeoHub 仍然是
可部署、可升级的合约套件，同时避免在普通合约里低效执行 ZK 验证。
...
| `NativeZkVerifier` | 校验 `ProofType.Zk` 的 commitment/proof envelope，并把重型证明数学委托给 L1 native accelerator。 | Batch commitment bytes、proof-system tag、verification-key id、public-input hash、accelerator hash。 | ZK proof accepted/rejected；verification key registered/removed。 | `VerifierRegistry`、governance/operator。 |
