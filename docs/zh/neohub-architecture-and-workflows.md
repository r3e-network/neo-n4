...
- `ContractZkVerifier` 是可部署 NeoHub 合约,但proof-system 验证工作通过 L1 可部署验证器合约 完成,
  不在普通合约字节码里硬算。
...
flowchart TB
    ...
    settle --> verifier
    verifier --> nativezk
    nativezk --> nativeacc["L1 可部署验证器合约"]
    settle --> da
    settle --> seq
    bridge --> token
    bridge --> chain
...
| 结算 | `SettlementManager`, `VerifierRegistry`, `ContractZkVerifier` | 批次 commitment 校验、proof dispatch、ZK verifier router 到 L1 可部署验证器合约、root finalization、batch status。 |
...
sequenceDiagram
    ...
    participant Native as L1 可部署验证器合约
    participant Chain as ChainRegistry
...
可部署验证器合约 的 `verifyZkProof(...)` ABI 执行证明系统数学。这样 NeoHub 仍然是
...
| `ContractZkVerifier` | 校验 `ProofType.Zk` 的 commitment/proof envelope，并把proof-system 验证工作委托给 L1 可部署验证器合约。 | Batch commitment bytes、proof-system tag、verification-key id、public-input hash、verifier contract hash。 | ZK proof accepted/rejected；verification key registered/removed。 | `VerifierRegistry`、governance/operator。 |
