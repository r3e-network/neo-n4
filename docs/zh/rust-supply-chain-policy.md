...
根 lockfile 与独立锁定的 `external/neo-zkvm` 图原先均解析到
`serial_test 3.4.0 -> scc 2.4.0`。兼容 semver 的 `serial_test 3.5.0` 更新已移除
`scc`，因此两个依赖图均不再包含 RUSTSEC-2026-0205。
...
| RUSTSEC-2026-0173 | `proc-macro-error2 2.0.1` | SP1 JIT 的 `dynasm 3.2.1` | 传递依赖，暂时接受 |
| RUSTSEC-2025-0134 | `rustls-pemfile 2.2.0` | `tonic 0.12.3` via SP1 prover types | 传递依赖，暂时接受 |
| RUSTSEC-2023-0089 | `atomic-polyfill 1.0.3` | 独立 RISC-V workspace 中的 `neo-riscv-host -> postcard -> heapless` | submodule 兼容性依赖，暂时接受 |
| yanked，无 advisory | `spin 0.9.8` | SP1 的 `lazy_static` graph 与 `neo-riscv-host -> postcard -> heapless` | 精确 lock 可复现；上游提供非 yanked 兼容版本后替换 |
