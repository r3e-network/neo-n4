...
| 异构链与 Watcher 架构 | 见 `docs/external-bridge-evm-chains.md` |
...
```text
Neo N4 = NeoHub L1 可部署合约锚定层 (Lean 4 Pillars)
       + 多条 Neo 4 L2 执行链
       + NeoFS DA
       + 可插拔证明系统 (SP1 6.2.x BN254 Groth16)
       + 共享桥和消息路由
       + 可选 Gateway 聚合层
...
| 第二卷：实现 | 第 4、8-13、15 章 | 按源码目录理解每个模块如何工作 |
