# 中文版本：Deterministic Batch Executor Spec

> 对应英文文档：[src/Neo.L2.Executor/SPEC.md](../../../../src/Neo.L2.Executor/SPEC.md)
> 维护规则：英文文档发生结构、命令、路径、接口、合约数量、测试证据或安全结论变更时，本中文版本必须同步更新。

## 本页用途

这是运行时代码模块规格。中文版本用于解释模块职责、接口和安全边界。

## 中文摘要

- 对应文件：src/Neo.L2.Executor/SPEC.md
- 中文路径：docs/zh/src/Neo.L2.Executor/SPEC.md
...
本文件已作为 src/Neo.L2.Executor/SPEC.md 的中文对应版本纳入仓库级本地化覆盖检查。后续修改由单元测试强制要求中文 counterpart 继续存在。