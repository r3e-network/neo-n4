...
中的 L2 原生合约、NeoFS 数据可用性路径、原生加速的 ZK verifier router、
...
- 让架构可视化：L1 NeoHub 合约、L1 可部署 ZK 验证器合约、NeoFS DA、Gateway、桥、L2 执行、L2 原生合约，以及可选 VM profile。
...
- 架构地图：L1、L2、NeoFS DA、Bridge、Gateway、可部署 ZK 验证器合约、SDK、watcher、可选 VM profile。
...
- 私有网络部署演练报告。
- 安全清单和威胁模型证据。
- ZK verifier 路径证据：`ContractZkVerifier` envelope 检查，以及 L1 可部署验证器合约 dispatch 状态。
