...
**25 个 NeoHub 合约项目。** 23 个生产合约 + 1 个仅审计用结构验证器 + 1 个测试 stub 全部经 `Neo.SmartContract.Framework` 通过类型检查;CI 用
...
- **Phase-5 证明聚合** 🟡 — `src/Neo.Plugins.L2Gateway/` —— `BinaryTreeAggregator`、Secp256r1/Merkle round prover、规范 binding、持久 outbox 和链上 route 已交付；专用 recursive Gateway SP1 guest/prover 与 proof-bound 生产 RPC publisher 仍缺失，opaque round proof bytes 不能替代最终有效性证明。

**16 个链下库 + 8 个插件。** 都有 `tests/Neo.*.UnitTests/` 镜像;solution 当前包含 37
.NET 测试工程，准确用例数由 runner 动态报告。
Rust workspace 出货 21 条默认 CI 测试(host-mode 密码学 + SDK +
zkVM execute 往返)加 2 条 `#[ignore]` 把关测试,演练真实 CPU 证明产生 + 验证(墙
钟 ~4 分钟)。TypeScript SDK 出货 16 条 vitest 测试。
...
- **应用开发者 SDK(TypeScript)** ✅ — `sdk/typescript/` —— `@neo-n4/sdk` 类型化封装所有 10 条 RPC 方法。16 条 vitest 测试在进程内 stub fetch 上通过。线协议形态 + 4 类错误分类与 .NET SDK 一致。
...
| 节点基础设施 | 🟡 | Gateway 缺少专用 recursive SP1 guest/prover 与 proof-bound 生产 RPC publisher。 |
...
`Neo.Hub.Deploy` 把整个桥栈与 NeoHub 一起脚手架化:24 步部署 + 32 条
