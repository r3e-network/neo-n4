...
- `.NET`：`scripts/test-coverage.ps1` 使用锁定的 Cargo 依赖构建当前平台的 `neo_riscv_host` 原生库，把它放到 Release RISC-V 测试程序集旁，强制全部真实原生测试执行，再使用 `coverlet.collector` 运行完整 `Neo.L2.sln` 测试，按源码文件/行合并 Cobertura 报告，并应用双门槛：协议/运行时源码根目录 90% line coverage，以及所有已上报源码 80% overall reported line coverage。原生库缺失或无法加载会直接失败，不会被转换成 coverage 跳过项。
...
摘要会在 `nativeRiscVHost` 中记录实际加载的平台原生库路径和 SHA-256。因此本地运行除 .NET 10 外还需要 Rust 工具链。
