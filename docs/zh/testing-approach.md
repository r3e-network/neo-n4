...
## 测试面(1430 .NET + 159 跨语言 + 2 SP1 发布关口)
...
1. `test` —— `dotnet test Neo.L2.sln`(1430 测试 / 34 工程)
2. `contracts` —— 安装 `Neo.Compiler.CSharp`,类型检查所有 33 个 NeoHub +
   L2Native 合约加 2 个 sample 合约,断言 35 个 `.nef` + 35 个 `.manifest.json` artifact
