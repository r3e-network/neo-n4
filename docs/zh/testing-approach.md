...
1. `test` —— `dotnet test Neo.L2.sln`(1521 测试 / 34 工程)
2. `contracts` —— 安装 `Neo.Compiler.CSharp`,类型检查 24 个 `NeoHub.*`
   deployable 合约加 2 个 `Sample.*` 合约,断言 26 个 `.nef` + 26 个
