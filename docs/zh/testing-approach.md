...
## 测试面(1411 .NET + 155 跨语言 + 2 SP1 发布关口)

| 层 | 框架 | 位置 | 内容 |
|---|---|---|---|
| 单元 | MSTest(xUnit 风) | `tests/Neo.L2.*.UnitTests/`(33 个工程) | 每类不变量、边界、空参 + 空字段守卫、指标 emission 钉 |
...
| Foundry | Solidity 不变量 + 多链 | `external/foreign-contracts/eth/test/`(20 测试) | EVM-family Solidity router —— 13 个单链 + 7 个多链,固定 17 个主网槽的每实例状态隔离 |
...
- `external/foreign-contracts/eth/` 中的 20 个 Foundry 测试 —— 同一 Solidity
...
1. `test` —— `dotnet test Neo.L2.sln`(1411 测试 / 33 工程)
2. `contracts` —— 安装 `Neo.Compiler.CSharp`,类型检查所有 28 个 NeoHub +
   L2Native 合约,断言 28 个 `.nef` + 28 个 `.manifest.json` artifact
