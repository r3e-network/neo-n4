...
| **`L1/L2NativeTokenVault`** —— 资产 ID 派生、桥接代币部署 | `NeoHub.TokenRegistry` + Neo Core 原生 `L2BridgeContract` + 原生 `BridgedNep17Contract` | N4 设计下对等 —— L2 代币记账在 core native 层,不是运维方后期部署模板 |
...
**状态**:默认 bridged NEP-17 由 Neo Core 原生 `BridgedNep17Contract` 提供;`TokenRegistry`
在注册新资产时绑定该原生路径。
...
并与 Neo Core 原生 `L2PaymasterContract` 天然搭配。目前只有充值赞助模式。

**状态**:已增加 Neo Core 原生 `L2AccountAbstraction`,包含 validate-hook 规约 +
