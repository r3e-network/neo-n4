...
| Gateway 是可选证明聚合中间件 | `Neo.Plugins.L2Gateway` + `SettlementManager.PublishGatewayGlobalRoot` → `MessageRouter.PublishGlobalRoot` | 链可走直连 NeoHub 结算或原子、绑定 finalized constituents 的 Gateway 路径，但资产托管不会迁移到 Gateway |
| Rollup / validium DA 选择必须显式 | `DARegistry` + `DAValidator` + NeoFS / L1 / DAC writer | 批次 finalize 检查当前 DA 策略；NeoFS 是默认 Neo-native 外部 DA 层 |
| Forced inclusion 防 sequencer 审查 | `NeoHub.ForcedInclusion` + `SequencerBond` + `ChainRegistry` pauser 接线 | 过期强制交易允许至多一次无归因 report 并暂停链；只有 governance 独立复核已终局 dBFT 归因证据后才能罚没 |
| L2 系统合约暴露桥、消息、费用、AA、interop 原语 | `external/neo` 下的 Neo core 原生 L2 合约 | 在不引入 EraVM bytecode/deployer/nonce-holder 机制的情况下提供等价原语 |
| ZK 有效性证明是无信任结算目标 | 同 runtime native `Sp1StatefulNeoVmV1` 执行 + SP1 RISC-V guest + `ContractZkVerifier` 路由 | 生产 ZK 链必须锁定 native executor/VK、注册真实 verifier 并永久关闭 `envelope-only`；无关 executor profile 不能继承该标签 |
...
- **Boojum / Airbender 电路栈** → Neo 执行证明 adapter（当前 host-native 与 SP1
  RISC-V guest 都执行精确 `Sp1StatefulNeoVmV1` runtime；独立 PolkaVM profile 仍需匹配
  prover）；
...
对齐——共享桥、链注册表、聚合消息根、DA 闸、强制包含、逃生舱——并已在仓库内交付
SP1 Groth16/BN254 终端验证器。今天一个 L1 批次结算实际信任什么,取决于该链配置的
`ProofType`:
...
- **`ProofType.Zk`(Stage 2)**——`ContractZkVerifier` 校验规范 batch/proof 信封，并把
  ...
  规范 `NEO4EXR1` 后原子提交完整状态；daemon 再于 SP1 内重执行同一 `NEO4PWIT`。
  生产计划注册准确 program VK，并在暴露 ZK settlement 路由前永久关闭 SP1
  `envelope-only`。显式 envelope-only 仅保留给私有 devnet 和尚未接入终端验证器的其它
  proof system。

简言之：neo4 现在同时交付弹性链式 proof routing 拓扑和 SP1 Groth16 链上有效性验证器。
这不是 ZKsync Boojum/Plonk 字节码复刻，而是 Neo 内置 stateful NeoVM/SP1-RISC-V
profile 对应的无信任结算边界，不是对每个 RISC-V executor 的笼统声明。下表中
...
| **ZK Gateway**(结算层证明聚合器) | `Neo.Plugins.L2Gateway`(链下)+ 链上 `SettlementManager.PublishGatewayGlobalRoot` → `MessageRouter.PublishGlobalRoot` | 协议/代码层面对等；独立审计与真实生产部署证据仍待完成 |
...
| **`zksync-cli` + 多语言 SDK** | `Neo.Stack.Cli` + 6 个其他 CLI；`sdk/typescript`、`sdk/rust`、`sdk/python`、`src/Neo.L2.Sdk` | 部分 —— 仓库中已有四套由一致性向量钉定的 SDK 源码实现；仍缺 Go，且不声称已有包发布证据 |
...
.NET、TypeScript、Rust 与 Python SDK 已镜像同一组 10 个 RPC 方法，并消费
共享一致性向量。ZKsync 还提供 `zksync2-go`，这仍是索引器和交易所集成需要补齐的差距。

**建议**：从共享一致性表面生成 Go SDK，不再手工维护第五套独立线协议实现。
