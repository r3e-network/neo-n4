...
| **`L1/L2NativeTokenVault`** —— 资产 ID 派生、桥接代币部署 | `NeoHub.TokenRegistry` + Neo Core 原生 `L2BridgeContract` + 原生 `BridgedNep17Contract` | N4 设计下对等 —— L2 代币记账在 core native 层,不是运维方后期部署模板；NEO 明确从 L1 不可分割单位(`0` decimals)映射到 L2 decimal 单位(`8` decimals) |
