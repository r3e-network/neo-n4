...
eighteen Z3 models and two CTL models — the `BatchSerializer.Decode` length arithmetic,
...
arithmetic invariants model (monotonicity, non-overlap, revert constraints), an inductive model
...
pipeline-liveness-model.md, batch-arithmetic-model.md and mutation-results.md.
...
arithmetic layer (batch/block monotonicity, non-overlap, revert constraints); full
