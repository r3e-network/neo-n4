...
**全系统形式化验证尚未完成。** 本目录包含十八个 Z3 模型与两个 CTL 模型——BatchSerializer.Decode
...
（强制入列队列 × 批次生命周期）、批次算术不变量（单调性、不重叠、回滚约束），以及
...
config-spec-model.md、batch-spec-model.md、settlement-abi-model.md、bridge-abi-model.md、pipeline-liveness-model.md、batch-arithmetic-model.md（中文要点见下文）与 mutation-results.zh.md。
...
（rollback 收窄）、doc.md §3.2 链配置与批次承诺线格式对应性、§3.2 结算方法面、§10/§11 桥接方法面、跨组件管道组合、批次算术不变量与结算状态机现由有限范围模型覆盖
...
回滚约束），完整多组件网络组合与完整执行语义仍未关闭。未关闭的证明义务包括：方法面之外的
