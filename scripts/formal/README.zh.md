...
**全系统形式化验证尚未完成。** 本目录包含十六个 Z3 模型与一个 CTL 模型——BatchSerializer.Decode
...
崩溃原子性、CTL 可达性活性）、乐观挑战二分游戏、doc.md §3.2 链配置与批次承诺线格式对应性、§3.2 结算方法面，以及
...
config-spec-model.md、batch-spec-model.md、settlement-abi-model.md（中文要点见下文）与 mutation-results.zh.md。
...
（rollback 收窄）、doc.md §3.2 链配置与批次承诺线格式对应性、§3.2 结算方法面与结算状态机现由有限范围模型覆盖
...
原子性。config-spec、batch-spec 与 settlement-abi 模型覆盖链配置、批次承诺线格式与结算
方法面对应性切片；更广的 spec-to-implementation 对应性（桥接 ABI）仍未关闭。未关闭
的证明义务包括：桥接 ABI 的 spec 对应、执行语义、完整 nonce 重放绑定与 SHA-256 抗碰撞、
