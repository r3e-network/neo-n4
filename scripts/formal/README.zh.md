...
**全系统形式化验证尚未完成。** 本目录包含十一个 Z3 模型与一个 CTL 模型——BatchSerializer.Decode
长度算术、原子 RegisterChain 注册状态机、强制包含 FIFO 队列、SharedBridge L1 escrow
守恒、L2 桥代币供给账本、GovernanceController 授权门、DA 写入侧失效切换策略、消息/提款
canonical preimage 结构注入性、Gateway 发布 outbox（归纳安全、崩溃恢复再水合、CTL 可达性
活性），以及已注册链结算状态机的归纳安全模型——不是 C#/NeoVM 程序验证器。性质测试、
变异测试与 SP1 执行证明属于不同证据，均不能替代全系统正确性证明。另见
settlement-model.md、registration-model.md、forced-inclusion-model.md、
...
gateway-outbox-recovery-model.md、gateway-outbox-liveness-model.md（中文要点见下文）与
...
gateway-outbox-recovery-result.json / gateway-outbox-liveness-result.json /
l2-bridge-result.json / settlement-result.json），记录源码/脚本/规范 SHA-256、求解器
版本、查询结果和退出码。每次修订应重新生成，已保存的结果不是当前运行证据。缺少依赖、
源码锚点变化、异常、非预期 SAT 或 UNKNOWN 均返回退出码 1。自测会对临时源码副本执行
故意失败的检查；其打印的 `FAILED` 是预期结果，但 unittest 进程整体必须退出 0。
...
（归纳安全、崩溃恢复再水合、CTL 可达性活性）与结算状态机现由有限范围模型覆盖（见
...
gateway-outbox-liveness-model.md / settlement-model.md）。各模型的信任假设在各自文档中
如实记录，均不声称超出其所述范围。outbox 的 rec 模型覆盖协议层恢复与活性；它们不建模
RocksDB 内部崩溃一致性（WAL、单次写入原子性）、持久化写入顺序论证或 L1 RPC 确认语义。
未关闭的证明义务包括：规范与实现对应、执行语义、回滚状态机、完整 nonce 重放绑定与
SHA-256 抗碰撞、RocksDB 内部崩溃一致性/持久化与 L1 核心 ChainMode 门控 GAS 钩子，以及
明确网络假设下的组合正确性和活性。每项均需要模型、审核后的假设、反例、实现关联和
可复核 CI 证据。SP1 发布检查保持独立：它证明固定程序的执行，不代表程序与外围系统
满足所有预期性质。
