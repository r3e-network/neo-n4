...
**全系统形式化验证尚未完成。** 本目录包含十七个 Z3 模型与两个 CTL 模型——BatchSerializer.Decode
...
（强制入列队列 × 批次生命周期），以及已注册链结算状态机的归纳安全模型——不是 C#/NeoVM 程序验证器。性质测试、变异测试与
...
config-spec-model.md、batch-spec-model.md、settlement-abi-model.md、bridge-abi-model.md、pipeline-liveness-model.md（中文要点见下文）与 mutation-results.zh.md。
...
（rollback 收窄）、doc.md §3.2 链配置与批次承诺线格式对应性、§3.2 结算方法面、§10/§11 桥接方法面、跨组件管道组合与结算状态机现由有限范围模型覆盖
...
批次生命周期），完整多组件网络组合仍未关闭。未关闭的证明义务包括：方法面之外的
