...
**全系统形式化验证尚未完成。** 本目录包含十五个 Z3 模型与一个 CTL 模型——BatchSerializer.Decode
...
崩溃原子性、CTL 可达性活性）、乐观挑战二分游戏、doc.md §3.2 链配置与批次承诺线格式对应性，以及
...
config-spec-model.md、batch-spec-model.md（中文要点见下文）与 mutation-results.zh.md。
...
（rollback 收窄）、doc.md §3.2 链配置与批次承诺线格式对应性与结算状态机现由有限范围模型覆盖
...
原子性。config-spec 与 batch-spec 模型覆盖链配置与批次承诺线格式对应性切片；更广的
spec-to-implementation 对应性（结算/桥接 ABI）仍未关闭。未关闭的证明义务包括：
结算/桥接 ABI 的 spec 对应、执行语义、完整 nonce 重放绑定与 SHA-256 抗碰撞、
