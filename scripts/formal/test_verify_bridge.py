"""Check the bridge-conservation proof runner's fail-closed behavior and negative controls."""
import tempfile
import unittest
...
import z3
import verify_bridge as verification


class BridgeConservationModelTests(unittest.TestCase):
    def test_all_obligations_and_negative_controls(self):
        ...
        self.assertEqual(14, len(report["obligations"]))
    ...
    def test_changed_source_requires_review(self):
        ...
        anchor = 'ExecutionEngine.Assert(currentBal >= amount, "withdrawal exceeds chain\'s escrowed balance");'
        self.assertIn(anchor, source)
        with tempfile.TemporaryDirectory() as directory:
            changed = Path(directory) / "SharedBridgeContract.cs"
