"""Check the DA-failover proof runner's fail-closed behavior and negative controls."""
import tempfile
import unittest
...
import z3
import verify_da_failover as verification


class DAFailoverModelTests(unittest.TestCase):
    ...
    def test_changed_source_requires_review(self):
        ...
        anchor = 'catch (IOException ex) when (IsTransient(ex))'
        self.assertIn(anchor, source)
        with tempfile.TemporaryDirectory() as directory:
            changed = Path(directory) / "FailoverDAWriter.cs"
