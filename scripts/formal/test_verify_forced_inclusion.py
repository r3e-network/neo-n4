"""Check the forced-inclusion proof runner's fail-closed behavior and negative controls."""
import tempfile
import unittest
...
import z3
import verify_forced_inclusion as verification


class ForcedInclusionModelTests(unittest.TestCase):
    def test_all_obligations_and_negative_controls(self):
        ...
        self.assertEqual(18, len(report["obligations"]))
    ...
    def test_changed_source_requires_review(self):
        ...
        anchor = 'ExecutionEngine.Assert(head + count <= tail, "forced transaction queue underflow");'
        self.assertIn(anchor, source)
        with tempfile.TemporaryDirectory() as directory:
            changed = Path(directory) / "RollupHubContract.cs"
