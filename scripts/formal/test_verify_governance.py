"""Check the governance proof runner's fail-closed behavior and negative controls."""
import tempfile
import unittest
...
import z3
import verify_governance as verification
...
class GovernanceModelTests(unittest.TestCase):
    def test_all_obligations_and_negative_controls(self):
        ...
        self.assertEqual(26, len(report["obligations"]))
    ...
    def test_changed_source_requires_review(self):
        ...
        anchor = 'if (GetProposalEpoch(proposalId) != GetCouncilEpoch()) return false;'
        self.assertIn(anchor, source)
        with tempfile.TemporaryDirectory() as directory:
            changed = Path(directory) / "GovernanceControllerContract.cs"
