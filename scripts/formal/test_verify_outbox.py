"""Check the gateway-outbox proof runner's fail-closed behavior and negative controls."""
import tempfile
import unittest
...
import z3
import verify_outbox as verification


class GatewayOutboxModelTests(unittest.TestCase):
    ...
    def test_changed_source_requires_review(self):
        ...
        anchor = 'public GatewayOutboxRecovery Recover()'
    ...
    def test_line_endings_do_not_change_review_digest(self):
        contract = verification.CONTRACT.read_text(encoding="utf-8-sig")
        plugin = verification.PLUGIN.read_text(encoding="utf-8-sig")
        with tempfile.TemporaryDirectory() as directory:
            c = Path(directory) / "GatewayOutbox.cs"
            c.write_bytes(contract.replace("\n", "\r\n").encode())
            p = Path(directory) / "L2GatewayPlugin.cs"
            p.write_bytes(plugin.replace("\n", "\r\n").encode())
            self.assertEqual(verification.REVIEWED_SOURCE_SHA256,
                             verification.source_digest(c) + ":" + verification.source_digest(p))
