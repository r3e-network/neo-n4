"""Check the preimage-injectivity proof runner's fail-closed behavior and negative controls."""
import tempfile
import unittest
...
import z3
import verify_preimage as verification


class PreimageModelTests(unittest.TestCase):
    ...
    def test_changed_source_requires_review(self):
        ...
        anchor = 'BinaryPrimitives.WriteUInt64LittleEndian(span.Slice(pos, 8), message.Nonce); pos += 8;'
        self.assertIn(anchor, source)
        with tempfile.TemporaryDirectory() as directory:
            changed = Path(directory) / "MessageHasher.cs"
