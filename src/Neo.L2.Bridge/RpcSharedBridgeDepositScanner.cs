...
/// <summary>
/// Durable Neo L1 event scanner for <c>NeoHub.SharedBridge.DepositEnqueued</c>.
/// </summary>
/// <remarks>
/// See doc.md §15.2. Each observed nonce is persisted before the block cursor advances so a
/// crash can only replay a finalized block; it cannot skip an undiscovered deposit.
/// </remarks>
public sealed class RpcSharedBridgeDepositScanner : IDisposable
{
    ...
    private const string EventName = "DepositEnqueued";
    private static readonly byte[] Namespace = Encoding.ASCII.GetBytes("neo-n4:shared-bridge-deposits:v1:");
    ...
    public RpcSharedBridgeDepositScanner(
        JsonRpcClient rpc,
        ...
        int maximumBlocksPerScan = 256)
    {
        ...
        if (contractHash.Equals(UInt160.Zero))
            throw new ArgumentException("SharedBridge contract hash must not be zero", nameof(contractHash));
        if (chainId == 0)
    ...
    /// <summary>Persist a nonce before exposing it to the in-memory deposit source.</summary>
    ...
    /// <summary>Forget a nonce only after the local batcher confirmed inclusion.</summary>
    ...
    /// <summary>
    /// Scan finalized blocks from the durable cursor and register matching deposit nonces.
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var executionToken in executions)
        {
            ...
            foreach (var notificationToken in notifications)
            {
                ...
                // DepositEnqueued(targetChainId, nonce, caller, l2Recipient, amount)
                ...
                var eventChainId = ParseStackUInt32(values[0], "DepositEnqueued.targetChainId");
                var nonce = ParseStackUInt64(values[1], "DepositEnqueued.nonce");
    ...
    private ScannerCursor ReadCursor()
    {
        ...
        if (bytes.Length != CursorLength || bytes[0] != EncodingVersion)
            throw new InvalidDataException("shared-bridge deposit scan cursor has an unsupported encoding");
        var nextHeight = BinaryPrimitives.ReadUInt32LittleEndian(bytes.AsSpan(1, sizeof(uint)));
        if (nextHeight < _startHeight)
            throw new InvalidDataException(
                $"shared-bridge deposit scan cursor {nextHeight} precedes configured start height {_startHeight}");
    ...
    private ulong DecodeNonceKey(byte[] key)
    {
        if (key.Length != _noncePrefix.Length + sizeof(ulong)
            ...
            throw new InvalidDataException("shared-bridge deposit nonce key has an invalid encoding");
