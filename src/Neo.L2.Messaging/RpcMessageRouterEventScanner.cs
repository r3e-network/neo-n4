...
public sealed class RpcMessageRouterEventScanner : IDisposable
{
    ...
    private const string EventName = "L1ToL2Enqueued";
    private static readonly byte[] Namespace = Encoding.ASCII.GetBytes("neo-n4:message-router-l1-to-l2:v1:");
    ...
    /// <summary>Persist a nonce before exposing it to the MessageRouter known-nonce set.</summary>
    ...
    /// <summary>Forget a nonce after the L2 batcher has locally consumed the inbound message.</summary>
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var executionToken in executions)
        {
            ...
            foreach (var notificationToken in notifications)
            {
                ...
                var eventChainId = ParseStackUInt32(values[0], "L1ToL2Enqueued.targetChainId");
                var nonce = ParseStackUInt64(values[1], "L1ToL2Enqueued.nonce");
    ...
    private ScannerCursor ReadCursor()
    {
        ...
        if (bytes.Length != CursorLength || bytes[0] != EncodingVersion)
            throw new InvalidDataException("message-router L1→L2 scan cursor has an unsupported encoding");
        var nextHeight = BinaryPrimitives.ReadUInt32LittleEndian(bytes.AsSpan(1, sizeof(uint)));
        if (nextHeight < _startHeight)
            throw new InvalidDataException(
                $"message-router L1→L2 scan cursor {nextHeight} precedes configured start height {_startHeight}");
    ...
    private ulong DecodeNonceKey(byte[] key)
    {
        if (key.Length != _noncePrefix.Length + sizeof(ulong)
            ...
            throw new InvalidDataException("message-router L1→L2 nonce key has an invalid encoding");
