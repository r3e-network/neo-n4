...
public sealed class UT_E2E_HostComposition_FromDeployReport
{
    ...
    public void MultisigLocalHost_SoftSeal_EmptyBlock_PersistsLocalCheckpoint()
    {
        ...
        try
        {
            ...
            var twelfthOutbound = AssertSoftSealAfterEleventhRecoverTwelfthOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-eleventh-recover-twelfth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-eleventh-recover-twelfth-outbound-rpc.json")));
            Assert.AreNotEqual(UInt256.Zero, twelfthOutbound.WithdrawalLeaf);
            Assert.AreNotEqual(UInt256.Zero, twelfthOutbound.OutboundMessageHash);
            AssertSoftSealTwelfthPoisonRecoverRetention(
                () => host.ReconcileAsync(),
                () => host.SubmitNextAsync(),
                (batch, hash) => host.RecoverPoisonedBatchAsync(batch, hash),
                () => host.IsSettlementPoisonedAsync().AsTask(),
                ...
                host.GetRpcWithdrawalProof,
                host.GetRpcMessageProof,
                twelfthOutbound.WithdrawalLeaf,
                twelfthOutbound.OutboundMessageHash,
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-twelfth-poison-recover.json")));
            AssertSoftSealAfterTwelfthRecoverDaAndThirteenthDeposit(
                req => host.PublishDaAsync(req).AsTask(),
                receipt => host.IsDaAvailableAsync(receipt).AsTask(),
                () => host.SupportsLocalDaReader,
                () => host.CreateLocalDaReader(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-twelfth-recover-da-deposit.json")));
            var thirteenthOutbound = AssertSoftSealAfterTwelfthRecoverThirteenthOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-twelfth-recover-thirteenth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-twelfth-recover-thirteenth-outbound-rpc.json")));
            Assert.AreNotEqual(UInt256.Zero, thirteenthOutbound.WithdrawalLeaf);
            Assert.AreNotEqual(UInt256.Zero, thirteenthOutbound.OutboundMessageHash);
            AssertSoftSealThirteenthPoisonRecoverRetention(
                () => host.ReconcileAsync(),
                () => host.SubmitNextAsync(),
                (batch, hash) => host.RecoverPoisonedBatchAsync(batch, hash),
                () => host.IsSettlementPoisonedAsync().AsTask(),
                ...
                host.GetRpcWithdrawalProof,
                host.GetRpcMessageProof,
                thirteenthOutbound.WithdrawalLeaf,
                thirteenthOutbound.OutboundMessageHash,
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-thirteenth-poison-recover.json")));
            AssertSoftSealAfterThirteenthRecoverDaAndFourteenthDeposit(
                req => host.PublishDaAsync(req).AsTask(),
                receipt => host.IsDaAvailableAsync(receipt).AsTask(),
                () => host.SupportsLocalDaReader,
                () => host.CreateLocalDaReader(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-da-deposit.json")));
            var fourteenthOutbound = AssertSoftSealAfterThirteenthRecoverFourteenthOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-fourteenth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-fourteenth-outbound-rpc.json")));
            Assert.AreNotEqual(UInt256.Zero, fourteenthOutbound.WithdrawalLeaf);
            Assert.AreNotEqual(UInt256.Zero, fourteenthOutbound.OutboundMessageHash);
            AssertSoftSealFourteenthPoisonRecoverRetention(
                () => host.ReconcileAsync(),
                () => host.SubmitNextAsync(),
                (batch, hash) => host.RecoverPoisonedBatchAsync(batch, hash),
                () => host.IsSettlementPoisonedAsync().AsTask(),
                ...
                host.GetRpcWithdrawalProof,
                host.GetRpcMessageProof,
                fourteenthOutbound.WithdrawalLeaf,
                fourteenthOutbound.OutboundMessageHash,
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-fourteenth-poison-recover.json")));
            AssertSoftSealAfterFourteenthRecoverDaAndFifteenthDeposit(
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-da-deposit.json")));
            var fifteenthOutbound = AssertSoftSealAfterFourteenthRecoverFifteenthOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-fifteenth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-fifteenth-outbound-rpc.json")));
            AssertSoftSealFifteenthPoisonRecoverRetention(
                () => host.ReconcileAsync(),
                () => host.SubmitNextAsync(),
                (batch, hash) => host.RecoverPoisonedBatchAsync(batch, hash),
                () => host.IsSettlementPoisonedAsync().AsTask(),
                ...
                host.GetRpcWithdrawalProof,
                host.GetRpcMessageProof,
                fifteenthOutbound.WithdrawalLeaf,
                fifteenthOutbound.OutboundMessageHash,
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-fifteenth-poison-recover.json")));
            AssertSoftSealAfterFifteenthRecoverDaAndSixteenthDeposit(
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-da-deposit.json")));
            var sixteenthOutbound = AssertSoftSealAfterFifteenthRecoverSixteenthOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-sixteenth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-sixteenth-outbound-rpc.json")));
            AssertSoftSealSixteenthPoisonRecoverRetention(
                () => host.ReconcileAsync(),
                () => host.SubmitNextAsync(),
                (batch, hash) => host.RecoverPoisonedBatchAsync(batch, hash),
                () => host.IsSettlementPoisonedAsync().AsTask(),
                ...
                host.GetRpcWithdrawalProof,
                host.GetRpcMessageProof,
                sixteenthOutbound.WithdrawalLeaf,
                sixteenthOutbound.OutboundMessageHash,
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-sixteenth-poison-recover.json")));
            AssertSoftSealAfterSixteenthRecoverDaAndSeventeenthDeposit(
                req => host.PublishDaAsync(req).AsTask(),
                receipt => host.IsDaAvailableAsync(receipt).AsTask(),
                () => host.SupportsLocalDaReader,
                () => host.CreateLocalDaReader(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-da-deposit.json")));
            var seventeenthOutbound = AssertSoftSealAfterSixteenthRecoverSeventeenthOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-seventeenth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-seventeenth-outbound-rpc.json")));
            AssertSoftSealSeventeenthPoisonRecoverRetention(
                () => host.ReconcileAsync(),
                () => host.SubmitNextAsync(),
                (batch, hash) => host.RecoverPoisonedBatchAsync(batch, hash),
                () => host.IsSettlementPoisonedAsync().AsTask(),
                ...
                host.GetRpcWithdrawalProof,
                host.GetRpcMessageProof,
                seventeenthOutbound.WithdrawalLeaf,
                seventeenthOutbound.OutboundMessageHash,
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-seventeenth-poison-recover.json")));
            AssertSoftSealAfterSeventeenthRecoverDaAndEighteenthDeposit(
                req => host.PublishDaAsync(req).AsTask(),
                receipt => host.IsDaAvailableAsync(receipt).AsTask(),
                () => host.SupportsLocalDaReader,
                () => host.CreateLocalDaReader(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-da-deposit.json")));
            var eighteenthOutbound = AssertSoftSealAfterSeventeenthRecoverEighteenthOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-eighteenth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-eighteenth-outbound-rpc.json")));
            AssertSoftSealEighteenthPoisonRecoverRetention(
                () => host.ReconcileAsync(),
                () => host.SubmitNextAsync(),
                (batch, hash) => host.RecoverPoisonedBatchAsync(batch, hash),
                () => host.IsSettlementPoisonedAsync().AsTask(),
                ...
                host.GetRpcWithdrawalProof,
                host.GetRpcMessageProof,
                eighteenthOutbound.WithdrawalLeaf,
                eighteenthOutbound.OutboundMessageHash,
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-eighteenth-poison-recover.json")));
            AssertSoftSealAfterEighteenthRecoverDaAndNineteenthDeposit(
                req => host.PublishDaAsync(req).AsTask(),
                receipt => host.IsDaAvailableAsync(receipt).AsTask(),
                () => host.SupportsLocalDaReader,
                () => host.CreateLocalDaReader(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-da-deposit.json")));
            var nineteenthOutbound = AssertSoftSealAfterEighteenthRecoverNineteenthOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-nineteenth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-nineteenth-outbound-rpc.json")));
            AssertSoftSealNineteenthPoisonRecoverRetention(
                () => host.ReconcileAsync(),
                () => host.SubmitNextAsync(),
                (batch, hash) => host.RecoverPoisonedBatchAsync(batch, hash),
                () => host.IsSettlementPoisonedAsync().AsTask(),
                ...
                host.GetRpcWithdrawalProof,
                host.GetRpcMessageProof,
                nineteenthOutbound.WithdrawalLeaf,
                nineteenthOutbound.OutboundMessageHash,
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-nineteenth-poison-recover.json")));
            AssertSoftSealAfterNineteenthRecoverDaAndTwentiethDeposit(
                req => host.PublishDaAsync(req).AsTask(),
                receipt => host.IsDaAvailableAsync(receipt).AsTask(),
                () => host.SupportsLocalDaReader,
                () => host.CreateLocalDaReader(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-da-deposit.json")));
            var twentiethOutbound = AssertSoftSealAfterNineteenthRecoverTwentiethOutboundAndFi(
                host.StageWithdrawal,
                () => host.StagedWithdrawalCount,
                host.SealWithdrawalBatch,
                msgs => host.EnqueueOutboundMessagesAsync(msgs).AsTask(),
                ...
                path => host.WriteHealthProbeAsync(path).AsTask(),
                chainDir);
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-twentieth-outbound.json")));
            Assert.IsTrue(File.Exists(Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-twentieth-outbound-rpc.json")));
        }
        finally
        {
            if (Directory.Exists(chainDir))
    ...
    /// <summary>
    /// SoftSeal after eleventh recover + twelfth deposit: twelfth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns twelfth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterEleventhRecoverTwelfthOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(12, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(26));
        Assert.IsFalse(registerForcedInclusionNonce(26));
        Assert.AreEqual(12, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(12, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(26));
        Assert.IsFalse(registerInboundMessageNonce(26));
        Assert.AreEqual(12, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(12, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(12, status.ConsumedDepositCount);
        Assert.AreEqual(12, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(12, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(12, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(12, probe.ConsumedDepositCount);
        Assert.AreEqual(12, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(12, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(12, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);
        ...
        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 12");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 12");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 12");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 12");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 12");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 12");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 12");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-eleventh-recover-twelfth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 12");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 12");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 12");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 12");

        var probePath = Path.Combine(chainDir, "soft-seal-after-eleventh-recover-twelfth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 12");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 12");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-eleventh-recover-twelfth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 12,
              "outboundNonce": 24,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 12");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 12");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 12");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-eleventh-recover-twelfth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 12");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 12");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal full soft path after eleventh recover + duodecuple deposit/outbox/FI: twelfth
    /// Reconcile→Poison→Recover cycle must retain multi-batch RPC tip, duodecuple deposits,
    /// duodecuple outbox, duodecuple FI/inbound known counts, and twelfth-outbound proofs.
    /// Does not claim L1 settle.
    /// </summary>
    private static void AssertSoftSealTwelfthPoisonRecoverRetention(
        Func<Task> reconcileAsync,
        Func<Task> submitNextAsync,
        Func<ulong, UInt256, Task> recoverPoisonedBatchAsync,
        Func<Task<bool>> isSettlementPoisonedAsync,
        ...
        Func<UInt256, ReadOnlyMemory<byte>?> getRpcWithdrawalProof,
        Func<UInt256, ReadOnlyMemory<byte>?> getRpcMessageProof,
        UInt256 twelfthWithdrawalLeaf,
        UInt256 twelfthOutboundMessageHash,
        string chainDir)
    {
        var before = getOperatorStatus();
        Assert.IsTrue(before.IsSettlementRetrying);
        ...
        Assert.IsTrue(before.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, before.LatestCheckpointBatchNumber);
        Assert.AreEqual(12, before.ConsumedDepositCount);
        Assert.AreEqual(12, before.MessageOutboxL2ToL1Count);
        Assert.AreEqual(12, before.KnownForcedInclusionNonceCount);
        Assert.AreEqual(12, before.KnownInboundNonceCount);
        Assert.IsTrue(getPendingCount() >= 2);
        ...
        Assert.IsTrue(afterPoison.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, afterPoison.LatestCheckpointBatchNumber);
        Assert.AreEqual(12, afterPoison.ConsumedDepositCount);
        Assert.AreEqual(12, afterPoison.MessageOutboxL2ToL1Count);
        Assert.AreEqual(12, afterPoison.KnownForcedInclusionNonceCount);
        Assert.AreEqual(12, afterPoison.KnownInboundNonceCount);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestCheckpointPostStateRoot);
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestRpcStateRoot);
        Assert.AreEqual(12, afterRecover.ConsumedDepositCount);
        Assert.AreEqual(12, afterRecover.MessageOutboxL2ToL1Count);
        Assert.AreEqual(12, afterRecover.KnownForcedInclusionNonceCount);
        Assert.AreEqual(12, afterRecover.KnownInboundNonceCount);
        Assert.IsTrue(afterRecover.IsOfflinePassportComplete);
        Assert.IsTrue(afterRecover.IsOperatorReady);
        Assert.IsTrue(afterRecover.IsBatcherCheckpointAligned);
        Assert.IsFalse(afterRecover.IsPipelineHealthy);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, getRpcStateRootAtBatch(2));
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, getLatestRpcStateRoot());
        Assert.IsTrue(getRpcWithdrawalProof(twelfthWithdrawalLeaf) is { Length: > 0 });
        Assert.IsTrue(getRpcMessageProof(twelfthOutboundMessageHash) is { Length: > 0 });
        ...
        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(probeJson, "\"consumedDepositCount\": 12");
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 12");

        var statusPath = Path.Combine(chainDir, "soft-seal-twelfth-poison-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"isSettlementRetrying\": true");
        ...
        var probePath = Path.Combine(chainDir, "soft-seal-twelfth-poison-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 12");
        StringAssert.Contains(File.ReadAllText(probePath), "\"isSettlementRetrying\": true");

        var durablePath = Path.Combine(chainDir, "soft-seal-twelfth-poison-recover.json");
        File.WriteAllText(durablePath, $$"""
            {
              "twelfthPoisonBlockedBatch": {{blockedBatch}},
              "pendingSettlementCount": {{afterRecover.PendingSettlementCount}},
              "latestCheckpointBatchNumber": {{afterRecover.LatestCheckpointBatchNumber}},
              "consumedDepositCount": {{afterRecover.ConsumedDepositCount}},
              "messageOutboxL2ToL1Count": {{afterRecover.MessageOutboxL2ToL1Count}},
    ...
    /// <summary>
    /// SoftSeal after twelfth poison→recover: re-publish local DA for sealed batches 1+2
    /// and process a thirteenth offline deposit (nonce 13, IncludedInBatch=2) while settle
    /// remains Retrying with multi-batch pending and duodecuple outbox/FI known retained.
    /// Does not claim L1 deposit scan / production DA / settle.
    /// </summary>
    private static void AssertSoftSealAfterTwelfthRecoverDaAndThirteenthDeposit(
        Func<DAPublishRequest, Task<DAReceipt>> publishDaAsync,
        Func<DAReceipt, Task<bool>> isDaAvailableAsync,
        Func<bool> supportsLocalDaReader,
        Func<IDAReader> createLocalDaReader,
        ...
        string chainDir)
    {
        ...
        // Thirteenth offline mint while still Retrying after twelfth recover.
        var softL1Asset = UInt160.Parse("0x" + new string('1', 40));
        var softL2Asset = UInt160.Parse("0x" + new string('2', 40));
        var softDepositPayload = new DepositPayload
        {
        ...
        Assert.IsTrue(hasConsumedDeposit(0, 11));
        Assert.IsTrue(hasConsumedDeposit(0, 12));
        Assert.IsTrue(hasConsumedDeposit(0, 13));
        Assert.AreEqual(13, consumedDepositCount());
        recordRpcDeposit(new DepositStatus(0, 13, ConsumedOnL2: true, IncludedInBatch: 2));
        Assert.IsTrue(getRpcL1DepositStatus(0, 1) is { ConsumedOnL2: true, IncludedInBatch: 1UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 2) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 3) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 4) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        ...
        Assert.IsTrue(getRpcL1DepositStatus(0, 11) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 12) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 13) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.AreEqual(0, scanSharedBridgeDepositsAsync().GetAwaiter().GetResult());

        var status = getOperatorStatus();
        Assert.AreEqual(13, status.ConsumedDepositCount);
        Assert.AreEqual(12, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(12, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(12, status.KnownInboundNonceCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(13, probe.ConsumedDepositCount);
        Assert.AreEqual(12, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);
        Assert.IsTrue(probe.IsSettlementRetrying);
        ...
        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 13");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 12");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"isSettlementPoisoned\": false");
        ...
        var hostProm = exportPrometheusMetrics();
        Assert.IsFalse(string.IsNullOrWhiteSpace(hostProm));
        var hostPromPath = Path.Combine(chainDir, "soft-seal-after-twelfth-recover-host.prom");
        writePrometheusMetricsAsync(hostPromPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(hostPromPath));
        Assert.AreEqual(hostProm, File.ReadAllText(hostPromPath));

        var statusPath = Path.Combine(chainDir, "soft-seal-after-twelfth-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        StringAssert.Contains(File.ReadAllText(statusPath), "\"consumedDepositCount\": 13");
        StringAssert.Contains(File.ReadAllText(statusPath), "\"isSettlementRetrying\": true");

        var probePath = Path.Combine(chainDir, "soft-seal-after-twelfth-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 13");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-twelfth-recover-da-deposit.json");
        File.WriteAllText(durablePath, $$"""
            {
              "daBatch1Layer": "{{da1.Layer}}",
              "daBatch2Layer": "{{da2.Layer}}",
        ...
        StringAssert.Contains(durableFile, "\"daBatch1Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"daBatch2Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 13");
        StringAssert.Contains(durableFile, "\"deposit13IncludedInBatch\": 2");
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 12");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");
        StringAssert.Contains(durableFile, "\"isOfflinePassportComplete\": true");
    }
    ...
    /// <summary>
    /// SoftSeal after twelfth recover + thirteenth deposit: thirteenth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns thirteenth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterTwelfthRecoverThirteenthOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(13, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(28));
        Assert.IsFalse(registerForcedInclusionNonce(28));
        Assert.AreEqual(13, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(13, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(28));
        Assert.IsFalse(registerInboundMessageNonce(28));
        Assert.AreEqual(13, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(13, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(13, status.ConsumedDepositCount);
        Assert.AreEqual(13, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(13, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(13, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(13, probe.ConsumedDepositCount);
        Assert.AreEqual(13, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(13, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(13, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);
        ...
        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 13");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 13");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 13");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 13");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 13");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 13");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 13");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-twelfth-recover-thirteenth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 13");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 13");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 13");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 13");

        var probePath = Path.Combine(chainDir, "soft-seal-after-twelfth-recover-thirteenth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 13");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 13");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-twelfth-recover-thirteenth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 13,
              "outboundNonce": 26,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 13");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 13");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 13");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-twelfth-recover-thirteenth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 13");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 13");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal full soft path after twelfth recover + tredecuple deposit/outbox/FI: thirteenth
    /// Reconcile→Poison→Recover cycle must retain multi-batch RPC tip, tredecuple deposits,
    /// tredecuple outbox, tredecuple FI/inbound known counts, and thirteenth-outbound proofs.
    /// Does not claim L1 settle.
    /// </summary>
    private static void AssertSoftSealThirteenthPoisonRecoverRetention(
        Func<Task> reconcileAsync,
        Func<Task> submitNextAsync,
        Func<ulong, UInt256, Task> recoverPoisonedBatchAsync,
        Func<Task<bool>> isSettlementPoisonedAsync,
        ...
        Func<UInt256, ReadOnlyMemory<byte>?> getRpcWithdrawalProof,
        Func<UInt256, ReadOnlyMemory<byte>?> getRpcMessageProof,
        UInt256 thirteenthWithdrawalLeaf,
        UInt256 thirteenthOutboundMessageHash,
        string chainDir)
    {
        var before = getOperatorStatus();
        Assert.IsTrue(before.IsSettlementRetrying);
        ...
        Assert.IsTrue(before.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, before.LatestCheckpointBatchNumber);
        Assert.AreEqual(13, before.ConsumedDepositCount);
        Assert.AreEqual(13, before.MessageOutboxL2ToL1Count);
        Assert.AreEqual(13, before.KnownForcedInclusionNonceCount);
        Assert.AreEqual(13, before.KnownInboundNonceCount);
        Assert.IsTrue(getPendingCount() >= 2);
        ...
        Assert.IsTrue(afterPoison.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, afterPoison.LatestCheckpointBatchNumber);
        Assert.AreEqual(13, afterPoison.ConsumedDepositCount);
        Assert.AreEqual(13, afterPoison.MessageOutboxL2ToL1Count);
        Assert.AreEqual(13, afterPoison.KnownForcedInclusionNonceCount);
        Assert.AreEqual(13, afterPoison.KnownInboundNonceCount);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestCheckpointPostStateRoot);
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestRpcStateRoot);
        Assert.AreEqual(13, afterRecover.ConsumedDepositCount);
        Assert.AreEqual(13, afterRecover.MessageOutboxL2ToL1Count);
        Assert.AreEqual(13, afterRecover.KnownForcedInclusionNonceCount);
        Assert.AreEqual(13, afterRecover.KnownInboundNonceCount);
        Assert.IsTrue(afterRecover.IsOfflinePassportComplete);
        Assert.IsTrue(afterRecover.IsOperatorReady);
        Assert.IsTrue(afterRecover.IsBatcherCheckpointAligned);
        Assert.IsFalse(afterRecover.IsPipelineHealthy);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, getRpcStateRootAtBatch(2));
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, getLatestRpcStateRoot());
        Assert.IsTrue(getRpcWithdrawalProof(thirteenthWithdrawalLeaf) is { Length: > 0 });
        Assert.IsTrue(getRpcMessageProof(thirteenthOutboundMessageHash) is { Length: > 0 });
        ...
        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(probeJson, "\"consumedDepositCount\": 13");
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 13");

        var statusPath = Path.Combine(chainDir, "soft-seal-thirteenth-poison-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"isSettlementRetrying\": true");
        ...
        var probePath = Path.Combine(chainDir, "soft-seal-thirteenth-poison-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 13");
        StringAssert.Contains(File.ReadAllText(probePath), "\"isSettlementRetrying\": true");

        var durablePath = Path.Combine(chainDir, "soft-seal-thirteenth-poison-recover.json");
        File.WriteAllText(durablePath, $$"""
            {
              "thirteenthPoisonBlockedBatch": {{blockedBatch}},
              "pendingSettlementCount": {{afterRecover.PendingSettlementCount}},
              "latestCheckpointBatchNumber": {{afterRecover.LatestCheckpointBatchNumber}},
              "consumedDepositCount": {{afterRecover.ConsumedDepositCount}},
              "messageOutboxL2ToL1Count": {{afterRecover.MessageOutboxL2ToL1Count}},
    ...
    /// <summary>
    /// SoftSeal after thirteenth poison→recover: re-publish local DA for sealed batches 1+2
    /// and process a fourteenth offline deposit (nonce 14, IncludedInBatch=2) while settle
    /// remains Retrying with multi-batch pending and tredecuple outbox/FI known retained.
    /// Does not claim L1 deposit scan / production DA / settle.
    /// </summary>
    private static void AssertSoftSealAfterThirteenthRecoverDaAndFourteenthDeposit(
        Func<DAPublishRequest, Task<DAReceipt>> publishDaAsync,
        Func<DAReceipt, Task<bool>> isDaAvailableAsync,
        Func<bool> supportsLocalDaReader,
        Func<IDAReader> createLocalDaReader,
        ...
        string chainDir)
    {
        ...
        // Fourteenth offline mint while still Retrying after thirteenth recover.
        var softL1Asset = UInt160.Parse("0x" + new string('1', 40));
        var softL2Asset = UInt160.Parse("0x" + new string('2', 40));
        var softDepositPayload = new DepositPayload
        {
        ...
        Assert.IsTrue(hasConsumedDeposit(0, 12));
        Assert.IsTrue(hasConsumedDeposit(0, 13));
        Assert.IsTrue(hasConsumedDeposit(0, 14));
        Assert.AreEqual(14, consumedDepositCount());
        recordRpcDeposit(new DepositStatus(0, 14, ConsumedOnL2: true, IncludedInBatch: 2));
        Assert.IsTrue(getRpcL1DepositStatus(0, 1) is { ConsumedOnL2: true, IncludedInBatch: 1UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 2) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 3) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 4) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        ...
        Assert.IsTrue(getRpcL1DepositStatus(0, 12) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 13) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 14) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.AreEqual(0, scanSharedBridgeDepositsAsync().GetAwaiter().GetResult());

        var status = getOperatorStatus();
        Assert.AreEqual(14, status.ConsumedDepositCount);
        Assert.AreEqual(13, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(13, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(13, status.KnownInboundNonceCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(14, probe.ConsumedDepositCount);
        Assert.AreEqual(13, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);
        Assert.IsTrue(probe.IsSettlementRetrying);
        ...
        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 14");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 13");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"isSettlementPoisoned\": false");
        ...
        var hostProm = exportPrometheusMetrics();
        Assert.IsFalse(string.IsNullOrWhiteSpace(hostProm));
        var hostPromPath = Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-host.prom");
        writePrometheusMetricsAsync(hostPromPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(hostPromPath));
        Assert.AreEqual(hostProm, File.ReadAllText(hostPromPath));

        var statusPath = Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        StringAssert.Contains(File.ReadAllText(statusPath), "\"consumedDepositCount\": 14");
        StringAssert.Contains(File.ReadAllText(statusPath), "\"isSettlementRetrying\": true");

        var probePath = Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 14");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-da-deposit.json");
        File.WriteAllText(durablePath, $$"""
            {
              "daBatch1Layer": "{{da1.Layer}}",
              "daBatch2Layer": "{{da2.Layer}}",
        ...
        StringAssert.Contains(durableFile, "\"daBatch1Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"daBatch2Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 14");
        StringAssert.Contains(durableFile, "\"deposit14IncludedInBatch\": 2");
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 13");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");
        StringAssert.Contains(durableFile, "\"isOfflinePassportComplete\": true");
    }
    ...
    /// <summary>
    /// SoftSeal after thirteenth recover + fourteenth deposit: fourteenth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns fourteenth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterThirteenthRecoverFourteenthOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(14, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(30));
        Assert.IsFalse(registerForcedInclusionNonce(30));
        Assert.AreEqual(14, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(14, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(30));
        Assert.IsFalse(registerInboundMessageNonce(30));
        Assert.AreEqual(14, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(14, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(14, status.ConsumedDepositCount);
        Assert.AreEqual(14, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(14, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(14, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(14, probe.ConsumedDepositCount);
        Assert.AreEqual(14, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(14, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(14, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);
        ...
        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 14");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 14");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 14");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 14");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 14");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 14");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 14");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-fourteenth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 14");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 14");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 14");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 14");

        var probePath = Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-fourteenth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 14");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 14");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-fourteenth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 14,
              "outboundNonce": 28,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 14");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 14");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 14");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-thirteenth-recover-fourteenth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 14");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 14");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal after fourteenth recover + fifteenth deposit: fifteenth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns fifteenth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterFourteenthRecoverFifteenthOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(15, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(32));
        Assert.IsFalse(registerForcedInclusionNonce(32));
        Assert.AreEqual(15, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(15, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(32));
        Assert.IsFalse(registerInboundMessageNonce(32));
        Assert.AreEqual(15, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(15, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(15, status.ConsumedDepositCount);
        Assert.AreEqual(15, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(15, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(15, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(15, probe.ConsumedDepositCount);
        Assert.AreEqual(15, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(15, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(15, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);

        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 15");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 15");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 15");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 15");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 15");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 15");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 15");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-fifteenth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 15");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 15");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 15");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 15");

        var probePath = Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-fifteenth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 15");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 15");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-fifteenth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 15,
              "outboundNonce": 30,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 15");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 15");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 15");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-fifteenth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 15");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 15");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal after fifteenth recover + sixteenth deposit: sixteenth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns sixteenth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterFifteenthRecoverSixteenthOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(16, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(34));
        Assert.IsFalse(registerForcedInclusionNonce(34));
        Assert.AreEqual(16, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(16, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(34));
        Assert.IsFalse(registerInboundMessageNonce(34));
        Assert.AreEqual(16, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(16, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(16, status.ConsumedDepositCount);
        Assert.AreEqual(16, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(16, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(16, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(16, probe.ConsumedDepositCount);
        Assert.AreEqual(16, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(16, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(16, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);

        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 16");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 16");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 16");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 16");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 16");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 16");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 16");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-sixteenth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 16");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 16");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 16");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 16");

        var probePath = Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-sixteenth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 16");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 16");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-sixteenth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 16,
              "outboundNonce": 32,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 16");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 16");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 16");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-sixteenth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 16");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 16");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal after sixteenth recover + seventeenth deposit: seventeenth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns seventeenth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterSixteenthRecoverSeventeenthOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(17, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(36));
        Assert.IsFalse(registerForcedInclusionNonce(36));
        Assert.AreEqual(17, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(17, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(36));
        Assert.IsFalse(registerInboundMessageNonce(36));
        Assert.AreEqual(17, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(17, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(17, status.ConsumedDepositCount);
        Assert.AreEqual(17, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(17, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(17, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(17, probe.ConsumedDepositCount);
        Assert.AreEqual(17, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(17, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(17, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);

        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 17");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 17");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 17");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 17");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 17");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 17");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 17");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-seventeenth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 17");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 17");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 17");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 17");

        var probePath = Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-seventeenth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 17");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 17");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-seventeenth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 17,
              "outboundNonce": 34,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 17");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 17");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 17");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-seventeenth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 17");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 17");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal after seventeenth recover + eighteenth deposit: eighteenth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns eighteenth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterSeventeenthRecoverEighteenthOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(18, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(38));
        Assert.IsFalse(registerForcedInclusionNonce(38));
        Assert.AreEqual(18, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(18, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(38));
        Assert.IsFalse(registerInboundMessageNonce(38));
        Assert.AreEqual(18, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(18, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(18, status.ConsumedDepositCount);
        Assert.AreEqual(18, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(18, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(18, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(18, probe.ConsumedDepositCount);
        Assert.AreEqual(18, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(18, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(18, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);

        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 18");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 18");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 18");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 18");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 18");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 18");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 18");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-eighteenth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 18");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 18");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 18");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 18");

        var probePath = Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-eighteenth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 18");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 18");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-eighteenth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 18,
              "outboundNonce": 36,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 18");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 18");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 18");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-eighteenth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 18");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 18");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal after eighteenth recover + nineteenth deposit: nineteenth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns nineteenth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterEighteenthRecoverNineteenthOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(19, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(40));
        Assert.IsFalse(registerForcedInclusionNonce(40));
        Assert.AreEqual(19, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(19, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(40));
        Assert.IsFalse(registerInboundMessageNonce(40));
        Assert.AreEqual(19, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(19, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(19, status.ConsumedDepositCount);
        Assert.AreEqual(19, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(19, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(19, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(19, probe.ConsumedDepositCount);
        Assert.AreEqual(19, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(19, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(19, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);

        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 19");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 19");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 19");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 19");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 19");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 19");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 19");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-nineteenth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 19");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 19");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 19");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 19");

        var probePath = Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-nineteenth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 19");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 19");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-nineteenth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 19,
              "outboundNonce": 38,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 19");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 19");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 19");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-nineteenth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 19");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 19");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal after nineteenth recover + twentieth deposit: twentieth withdrawal seal + L2→L1 outbox
    /// enqueue + eighth FI/inbound nonces + RPC withdrawal/message/router proofs while settle
    /// remains Retrying with multi-batch pending. Does not claim L1 claim / FI drain / settle.
    /// Returns twentieth withdrawal leaf + outbound hash for follow-on pins.
    /// </summary>
    private static (UInt256 WithdrawalLeaf, UInt256 OutboundMessageHash) AssertSoftSealAfterNineteenthRecoverTwentiethOutboundAndFi(
        Func<WithdrawalRequest, UInt256> stageWithdrawal,
        Func<int> stagedWithdrawalCount,
        Func<(UInt256 Root, WithdrawalTree Tree)> sealWithdrawalBatch,
        Func<IReadOnlyList<CrossChainMessage>, Task> enqueueOutbound,
        ...
        string chainDir)
    {
        ...
        var outboundDraft = new CrossChainMessage
        {
            ...
            Receiver = softSender,
            MessageType = MessageType.Event,
            Payload = new byte[] { 0x14 },
            MessageHash = UInt256.Zero,
        };
        var outbound = outboundDraft with { MessageHash = MessageHasher.HashMessage(outboundDraft) };
        enqueueOutbound([outbound]).GetAwaiter().GetResult();
        Assert.AreEqual(20, messageOutboxL2ToL1Count());
        Assert.AreNotEqual(UInt256.Zero, messageOutboxL2ToL1Root());
        ...
        Assert.IsTrue(registerForcedInclusionNonce(42));
        Assert.IsFalse(registerForcedInclusionNonce(42));
        Assert.AreEqual(20, knownForcedInclusionNonceCount());
        Assert.AreEqual(0, openBatchForcedInclusionCount());
        Assert.IsFalse(hasOverdueForcedInclusionCached());
        invalidateForcedInclusionCache();
        Assert.AreEqual(20, knownForcedInclusionNonceCount());

        Assert.IsTrue(registerInboundMessageNonce(42));
        Assert.IsFalse(registerInboundMessageNonce(42));
        Assert.AreEqual(20, knownInboundNonceCount());
        Assert.AreEqual(0, openBatchL1MessageCount());
        Assert.AreEqual(0, l1InboxPendingCount());
        invalidateInboundMessageCache();
        Assert.AreEqual(20, knownInboundNonceCount());
        ...
        var status = getOperatorStatus();
        Assert.AreEqual(20, status.ConsumedDepositCount);
        Assert.AreEqual(20, status.MessageOutboxL2ToL1Count);
        Assert.AreEqual(0, status.StagedWithdrawalCount);
        Assert.AreEqual(20, status.KnownForcedInclusionNonceCount);
        Assert.AreEqual(20, status.KnownInboundNonceCount);
        Assert.IsFalse(status.HasOverdueForcedInclusion);
        Assert.AreEqual(0, status.OpenBatchForcedInclusionCount);
        Assert.AreEqual(0, status.OpenBatchL1MessageCount);
        Assert.AreEqual(2UL, status.LatestCheckpointBatchNumber);
        ...
        var probe = getHealthProbe();
        Assert.AreEqual(20, probe.ConsumedDepositCount);
        Assert.AreEqual(20, probe.MessageOutboxL2ToL1Count);
        Assert.AreEqual(20, probe.KnownForcedInclusionNonceCount);
        Assert.AreEqual(20, probe.KnownInboundNonceCount);
        Assert.IsTrue(probe.IsSettlementRetrying);
        Assert.AreEqual(2UL, probe.LatestCheckpointBatchNumber);
        Assert.IsTrue(probe.PendingSettlementCount >= 2);

        var statusJson = formatOperatorStatusJson();
        StringAssert.Contains(statusJson, "\"consumedDepositCount\": 20");
        StringAssert.Contains(statusJson, "\"messageOutboxL2ToL1Count\": 20");
        StringAssert.Contains(statusJson, "\"knownForcedInclusionNonceCount\": 20");
        StringAssert.Contains(statusJson, "\"knownInboundNonceCount\": 20");
        StringAssert.Contains(statusJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(statusJson, "\"latestCheckpointBatchNumber\": 2");

        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 20");
        StringAssert.Contains(probeJson, "\"knownForcedInclusionNonceCount\": 20");
        StringAssert.Contains(probeJson, "\"knownInboundNonceCount\": 20");
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");

        var statusPath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-twentieth-outbound-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"messageOutboxL2ToL1Count\": 20");
        StringAssert.Contains(statusFile, "\"knownForcedInclusionNonceCount\": 20");
        StringAssert.Contains(statusFile, "\"knownInboundNonceCount\": 20");
        StringAssert.Contains(statusFile, "\"consumedDepositCount\": 20");

        var probePath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-twentieth-outbound-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"messageOutboxL2ToL1Count\": 20");
        StringAssert.Contains(File.ReadAllText(probePath), "\"knownForcedInclusionNonceCount\": 20");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-twentieth-outbound.json");
        File.WriteAllText(durablePath, $$"""
            {
              "withdrawalNonce": 20,
              "outboundNonce": 40,
        ...
        Assert.IsTrue(File.Exists(durablePath));
        var durableFile = File.ReadAllText(durablePath);
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 20");
        StringAssert.Contains(durableFile, "\"knownForcedInclusionNonceCount\": 20");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 20");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");

        var rpcSurfacePath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-twentieth-outbound-rpc.json");
        File.WriteAllText(rpcSurfacePath, $$"""
            {
              "withdrawalLeaf": "{{wdLeaf}}",
              "outboundMessageHash": "{{outbound.MessageHash}}",
              "withdrawalProofBytes": {{proofBytes.Length}},
              "messageProofBytes": {{messageProofBytes.Length}},
              "messageOutboxL2ToL1Count": 20,
              "knownForcedInclusionNonceCount": 20,
              "knownInboundNonceCount": 20,
              "consumedDepositCount": 20,
              "isSettlementRetrying": true
            }
            """);
        Assert.IsTrue(File.Exists(rpcSurfacePath));
        ...
        StringAssert.Contains(rpcSurface, "\"withdrawalLeaf\": \"" + wdLeaf + "\"");
        StringAssert.Contains(rpcSurface, "\"outboundMessageHash\": \"" + outbound.MessageHash + "\"");
        StringAssert.Contains(rpcSurface, "\"messageOutboxL2ToL1Count\": 20");
        StringAssert.Contains(rpcSurface, "\"knownForcedInclusionNonceCount\": 20");
        return (wdLeaf, outbound.MessageHash);
    }

    /// <summary>
    /// SoftSeal full soft path after thirteenth recover + quattuordecuple deposit/outbox/FI: fourteenth
    /// Reconcile→Poison→Recover cycle must retain multi-batch RPC tip, quattuordecuple deposits,
    /// quattuordecuple outbox, quattuordecuple FI/inbound known counts, and fourteenth-outbound proofs.
    /// Does not claim L1 settle.
    /// </summary>
    private static void AssertSoftSealFourteenthPoisonRecoverRetention(
        Func<Task> reconcileAsync,
        Func<Task> submitNextAsync,
        Func<ulong, UInt256, Task> recoverPoisonedBatchAsync,
        Func<Task<bool>> isSettlementPoisonedAsync,
        ...
        Func<UInt256, ReadOnlyMemory<byte>?> getRpcWithdrawalProof,
        Func<UInt256, ReadOnlyMemory<byte>?> getRpcMessageProof,
        UInt256 fourteenthWithdrawalLeaf,
        UInt256 fourteenthOutboundMessageHash,
        string chainDir)
    {
        var before = getOperatorStatus();
        Assert.IsTrue(before.IsSettlementRetrying);
        ...
        Assert.IsTrue(before.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, before.LatestCheckpointBatchNumber);
        Assert.AreEqual(14, before.ConsumedDepositCount);
        Assert.AreEqual(14, before.MessageOutboxL2ToL1Count);
        Assert.AreEqual(14, before.KnownForcedInclusionNonceCount);
        Assert.AreEqual(14, before.KnownInboundNonceCount);
        Assert.IsTrue(getPendingCount() >= 2);
        ...
        Assert.IsTrue(afterPoison.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, afterPoison.LatestCheckpointBatchNumber);
        Assert.AreEqual(14, afterPoison.ConsumedDepositCount);
        Assert.AreEqual(14, afterPoison.MessageOutboxL2ToL1Count);
        Assert.AreEqual(14, afterPoison.KnownForcedInclusionNonceCount);
        Assert.AreEqual(14, afterPoison.KnownInboundNonceCount);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestCheckpointPostStateRoot);
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestRpcStateRoot);
        Assert.AreEqual(14, afterRecover.ConsumedDepositCount);
        Assert.AreEqual(14, afterRecover.MessageOutboxL2ToL1Count);
        Assert.AreEqual(14, afterRecover.KnownForcedInclusionNonceCount);
        Assert.AreEqual(14, afterRecover.KnownInboundNonceCount);
        Assert.IsTrue(afterRecover.IsOfflinePassportComplete);
        Assert.IsTrue(afterRecover.IsOperatorReady);
        Assert.IsTrue(afterRecover.IsBatcherCheckpointAligned);
        Assert.IsFalse(afterRecover.IsPipelineHealthy);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, getRpcStateRootAtBatch(2));
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, getLatestRpcStateRoot());
        Assert.IsTrue(getRpcWithdrawalProof(fourteenthWithdrawalLeaf) is { Length: > 0 });
        Assert.IsTrue(getRpcMessageProof(fourteenthOutboundMessageHash) is { Length: > 0 });
        ...
        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(probeJson, "\"consumedDepositCount\": 14");
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 14");

        var statusPath = Path.Combine(chainDir, "soft-seal-fourteenth-poison-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"isSettlementRetrying\": true");
        ...
        var probePath = Path.Combine(chainDir, "soft-seal-fourteenth-poison-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 14");
        StringAssert.Contains(File.ReadAllText(probePath), "\"isSettlementRetrying\": true");

        var durablePath = Path.Combine(chainDir, "soft-seal-fourteenth-poison-recover.json");
        File.WriteAllText(durablePath, $$"""
            {
              "fourteenthPoisonBlockedBatch": {{blockedBatch}},
              "pendingSettlementCount": {{afterRecover.PendingSettlementCount}},
              "latestCheckpointBatchNumber": {{afterRecover.LatestCheckpointBatchNumber}},
              "consumedDepositCount": {{afterRecover.ConsumedDepositCount}},
              "messageOutboxL2ToL1Count": {{afterRecover.MessageOutboxL2ToL1Count}},
    ...
    /// <summary>
    /// SoftSeal full soft path after thirteenth recover + quindecuple deposit/outbox/FI: fourteenth
    /// Reconcile→Poison→Recover cycle must retain multi-batch RPC tip, quindecuple deposits,
    /// quindecuple outbox, quindecuple FI/inbound known counts, and fourteenth-outbound proofs.
    /// Does not claim L1 settle.
    /// </summary>
    private static void AssertSoftSealFifteenthPoisonRecoverRetention(
        Func<Task> reconcileAsync,
        Func<Task> submitNextAsync,
        Func<ulong, UInt256, Task> recoverPoisonedBatchAsync,
        Func<Task<bool>> isSettlementPoisonedAsync,
        ...
        string chainDir)
    {
        ...
        Assert.IsTrue(before.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, before.LatestCheckpointBatchNumber);
        Assert.AreEqual(15, before.ConsumedDepositCount);
        Assert.AreEqual(15, before.MessageOutboxL2ToL1Count);
        Assert.AreEqual(15, before.KnownForcedInclusionNonceCount);
        Assert.AreEqual(15, before.KnownInboundNonceCount);
        Assert.IsTrue(getPendingCount() >= 2);
        ...
        Assert.IsTrue(afterPoison.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, afterPoison.LatestCheckpointBatchNumber);
        Assert.AreEqual(15, afterPoison.ConsumedDepositCount);
        Assert.AreEqual(15, afterPoison.MessageOutboxL2ToL1Count);
        Assert.AreEqual(15, afterPoison.KnownForcedInclusionNonceCount);
        Assert.AreEqual(15, afterPoison.KnownInboundNonceCount);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestCheckpointPostStateRoot);
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestRpcStateRoot);
        Assert.AreEqual(15, afterRecover.ConsumedDepositCount);
        Assert.AreEqual(15, afterRecover.MessageOutboxL2ToL1Count);
        Assert.AreEqual(15, afterRecover.KnownForcedInclusionNonceCount);
        Assert.AreEqual(15, afterRecover.KnownInboundNonceCount);
        Assert.IsTrue(afterRecover.IsOfflinePassportComplete);
        Assert.IsTrue(afterRecover.IsOperatorReady);
        Assert.IsTrue(afterRecover.IsBatcherCheckpointAligned);
        Assert.IsFalse(afterRecover.IsPipelineHealthy);
        ...
        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(probeJson, "\"consumedDepositCount\": 15");
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 15");

        var statusPath = Path.Combine(chainDir, "soft-seal-fifteenth-poison-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"isSettlementRetrying\": true");
        ...
        var probePath = Path.Combine(chainDir, "soft-seal-fifteenth-poison-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 15");
        StringAssert.Contains(File.ReadAllText(probePath), "\"isSettlementRetrying\": true");

        var durablePath = Path.Combine(chainDir, "soft-seal-fifteenth-poison-recover.json");
        File.WriteAllText(durablePath, $$"""
            {
              "fifteenthPoisonBlockedBatch": {{blockedBatch}},
              "pendingSettlementCount": {{afterRecover.PendingSettlementCount}},
              "latestCheckpointBatchNumber": {{afterRecover.LatestCheckpointBatchNumber}},
              "consumedDepositCount": {{afterRecover.ConsumedDepositCount}},
              "messageOutboxL2ToL1Count": {{afterRecover.MessageOutboxL2ToL1Count}},
    ...
    /// <summary>
    /// SoftSeal full soft path after thirteenth recover + sexdecuple deposit/outbox/FI: fourteenth
    /// Reconcile→Poison→Recover cycle must retain multi-batch RPC tip, sexdecuple deposits,
    /// sexdecuple outbox, sexdecuple FI/inbound known counts, and fourteenth-outbound proofs.
    /// Does not claim L1 settle.
    /// </summary>
    private static void AssertSoftSealSixteenthPoisonRecoverRetention(
        Func<Task> reconcileAsync,
        Func<Task> submitNextAsync,
        Func<ulong, UInt256, Task> recoverPoisonedBatchAsync,
        Func<Task<bool>> isSettlementPoisonedAsync,
        ...
        string chainDir)
    {
        ...
        Assert.IsTrue(before.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, before.LatestCheckpointBatchNumber);
        Assert.AreEqual(16, before.ConsumedDepositCount);
        Assert.AreEqual(16, before.MessageOutboxL2ToL1Count);
        Assert.AreEqual(16, before.KnownForcedInclusionNonceCount);
        Assert.AreEqual(16, before.KnownInboundNonceCount);
        Assert.IsTrue(getPendingCount() >= 2);
        ...
        Assert.IsTrue(afterPoison.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, afterPoison.LatestCheckpointBatchNumber);
        Assert.AreEqual(16, afterPoison.ConsumedDepositCount);
        Assert.AreEqual(16, afterPoison.MessageOutboxL2ToL1Count);
        Assert.AreEqual(16, afterPoison.KnownForcedInclusionNonceCount);
        Assert.AreEqual(16, afterPoison.KnownInboundNonceCount);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestCheckpointPostStateRoot);
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestRpcStateRoot);
        Assert.AreEqual(16, afterRecover.ConsumedDepositCount);
        Assert.AreEqual(16, afterRecover.MessageOutboxL2ToL1Count);
        Assert.AreEqual(16, afterRecover.KnownForcedInclusionNonceCount);
        Assert.AreEqual(16, afterRecover.KnownInboundNonceCount);
        Assert.IsTrue(afterRecover.IsOfflinePassportComplete);
        Assert.IsTrue(afterRecover.IsOperatorReady);
        Assert.IsTrue(afterRecover.IsBatcherCheckpointAligned);
        Assert.IsFalse(afterRecover.IsPipelineHealthy);
        ...
        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(probeJson, "\"consumedDepositCount\": 16");
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 16");

        var statusPath = Path.Combine(chainDir, "soft-seal-sixteenth-poison-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"isSettlementRetrying\": true");
        ...
        var probePath = Path.Combine(chainDir, "soft-seal-sixteenth-poison-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 16");
        StringAssert.Contains(File.ReadAllText(probePath), "\"isSettlementRetrying\": true");

        var durablePath = Path.Combine(chainDir, "soft-seal-sixteenth-poison-recover.json");
        File.WriteAllText(durablePath, $$"""
            {
              "sixteenthPoisonBlockedBatch": {{blockedBatch}},
              "pendingSettlementCount": {{afterRecover.PendingSettlementCount}},
              "latestCheckpointBatchNumber": {{afterRecover.LatestCheckpointBatchNumber}},
              "consumedDepositCount": {{afterRecover.ConsumedDepositCount}},
              "messageOutboxL2ToL1Count": {{afterRecover.MessageOutboxL2ToL1Count}},
    ...
    /// <summary>
    /// SoftSeal full soft path after thirteenth recover + septendecuple deposit/outbox/FI: fourteenth
    /// Reconcile→Poison→Recover cycle must retain multi-batch RPC tip, septendecuple deposits,
    /// septendecuple outbox, septendecuple FI/inbound known counts, and fourteenth-outbound proofs.
    /// Does not claim L1 settle.
    /// </summary>
    private static void AssertSoftSealSeventeenthPoisonRecoverRetention(
        Func<Task> reconcileAsync,
        Func<Task> submitNextAsync,
        Func<ulong, UInt256, Task> recoverPoisonedBatchAsync,
        Func<Task<bool>> isSettlementPoisonedAsync,
        ...
        string chainDir)
    {
        ...
        Assert.IsTrue(before.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, before.LatestCheckpointBatchNumber);
        Assert.AreEqual(17, before.ConsumedDepositCount);
        Assert.AreEqual(17, before.MessageOutboxL2ToL1Count);
        Assert.AreEqual(17, before.KnownForcedInclusionNonceCount);
        Assert.AreEqual(17, before.KnownInboundNonceCount);
        Assert.IsTrue(getPendingCount() >= 2);
        ...
        Assert.IsTrue(afterPoison.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, afterPoison.LatestCheckpointBatchNumber);
        Assert.AreEqual(17, afterPoison.ConsumedDepositCount);
        Assert.AreEqual(17, afterPoison.MessageOutboxL2ToL1Count);
        Assert.AreEqual(17, afterPoison.KnownForcedInclusionNonceCount);
        Assert.AreEqual(17, afterPoison.KnownInboundNonceCount);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestCheckpointPostStateRoot);
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestRpcStateRoot);
        Assert.AreEqual(17, afterRecover.ConsumedDepositCount);
        Assert.AreEqual(17, afterRecover.MessageOutboxL2ToL1Count);
        Assert.AreEqual(17, afterRecover.KnownForcedInclusionNonceCount);
        Assert.AreEqual(17, afterRecover.KnownInboundNonceCount);
        Assert.IsTrue(afterRecover.IsOfflinePassportComplete);
        Assert.IsTrue(afterRecover.IsOperatorReady);
        Assert.IsTrue(afterRecover.IsBatcherCheckpointAligned);
        Assert.IsFalse(afterRecover.IsPipelineHealthy);
        ...
        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(probeJson, "\"consumedDepositCount\": 17");
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 17");

        var statusPath = Path.Combine(chainDir, "soft-seal-seventeenth-poison-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"isSettlementRetrying\": true");
        ...
        var probePath = Path.Combine(chainDir, "soft-seal-seventeenth-poison-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 17");
        StringAssert.Contains(File.ReadAllText(probePath), "\"isSettlementRetrying\": true");

        var durablePath = Path.Combine(chainDir, "soft-seal-seventeenth-poison-recover.json");
        File.WriteAllText(durablePath, $$"""
            {
              "seventeenthPoisonBlockedBatch": {{blockedBatch}},
              "pendingSettlementCount": {{afterRecover.PendingSettlementCount}},
              "latestCheckpointBatchNumber": {{afterRecover.LatestCheckpointBatchNumber}},
              "consumedDepositCount": {{afterRecover.ConsumedDepositCount}},
              "messageOutboxL2ToL1Count": {{afterRecover.MessageOutboxL2ToL1Count}},
    ...
    /// <summary>
    /// SoftSeal full soft path after thirteenth recover + octodecuple deposit/outbox/FI: fourteenth
    /// Reconcile→Poison→Recover cycle must retain multi-batch RPC tip, octodecuple deposits,
    /// octodecuple outbox, octodecuple FI/inbound known counts, and fourteenth-outbound proofs.
    /// Does not claim L1 settle.
    /// </summary>
    private static void AssertSoftSealEighteenthPoisonRecoverRetention(
        Func<Task> reconcileAsync,
        Func<Task> submitNextAsync,
        Func<ulong, UInt256, Task> recoverPoisonedBatchAsync,
        Func<Task<bool>> isSettlementPoisonedAsync,
        ...
        string chainDir)
    {
        ...
        Assert.IsTrue(before.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, before.LatestCheckpointBatchNumber);
        Assert.AreEqual(18, before.ConsumedDepositCount);
        Assert.AreEqual(18, before.MessageOutboxL2ToL1Count);
        Assert.AreEqual(18, before.KnownForcedInclusionNonceCount);
        Assert.AreEqual(18, before.KnownInboundNonceCount);
        Assert.IsTrue(getPendingCount() >= 2);
        ...
        Assert.IsTrue(afterPoison.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, afterPoison.LatestCheckpointBatchNumber);
        Assert.AreEqual(18, afterPoison.ConsumedDepositCount);
        Assert.AreEqual(18, afterPoison.MessageOutboxL2ToL1Count);
        Assert.AreEqual(18, afterPoison.KnownForcedInclusionNonceCount);
        Assert.AreEqual(18, afterPoison.KnownInboundNonceCount);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestCheckpointPostStateRoot);
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestRpcStateRoot);
        Assert.AreEqual(18, afterRecover.ConsumedDepositCount);
        Assert.AreEqual(18, afterRecover.MessageOutboxL2ToL1Count);
        Assert.AreEqual(18, afterRecover.KnownForcedInclusionNonceCount);
        Assert.AreEqual(18, afterRecover.KnownInboundNonceCount);
        Assert.IsTrue(afterRecover.IsOfflinePassportComplete);
        Assert.IsTrue(afterRecover.IsOperatorReady);
        Assert.IsTrue(afterRecover.IsBatcherCheckpointAligned);
        Assert.IsFalse(afterRecover.IsPipelineHealthy);
        ...
        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(probeJson, "\"consumedDepositCount\": 18");
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 18");

        var statusPath = Path.Combine(chainDir, "soft-seal-eighteenth-poison-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"isSettlementRetrying\": true");
        ...
        var probePath = Path.Combine(chainDir, "soft-seal-eighteenth-poison-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 18");
        StringAssert.Contains(File.ReadAllText(probePath), "\"isSettlementRetrying\": true");

        var durablePath = Path.Combine(chainDir, "soft-seal-eighteenth-poison-recover.json");
        File.WriteAllText(durablePath, $$"""
            {
              "eighteenthPoisonBlockedBatch": {{blockedBatch}},
              "pendingSettlementCount": {{afterRecover.PendingSettlementCount}},
              "latestCheckpointBatchNumber": {{afterRecover.LatestCheckpointBatchNumber}},
              "consumedDepositCount": {{afterRecover.ConsumedDepositCount}},
              "messageOutboxL2ToL1Count": {{afterRecover.MessageOutboxL2ToL1Count}},
    ...
    /// <summary>
    /// SoftSeal full soft path after thirteenth recover + novemdecuple deposit/outbox/FI: fourteenth
    /// Reconcile→Poison→Recover cycle must retain multi-batch RPC tip, novemdecuple deposits,
    /// novemdecuple outbox, novemdecuple FI/inbound known counts, and fourteenth-outbound proofs.
    /// Does not claim L1 settle.
    /// </summary>
    private static void AssertSoftSealNineteenthPoisonRecoverRetention(
        Func<Task> reconcileAsync,
        Func<Task> submitNextAsync,
        Func<ulong, UInt256, Task> recoverPoisonedBatchAsync,
        Func<Task<bool>> isSettlementPoisonedAsync,
        ...
        string chainDir)
    {
        ...
        Assert.IsTrue(before.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, before.LatestCheckpointBatchNumber);
        Assert.AreEqual(19, before.ConsumedDepositCount);
        Assert.AreEqual(19, before.MessageOutboxL2ToL1Count);
        Assert.AreEqual(19, before.KnownForcedInclusionNonceCount);
        Assert.AreEqual(19, before.KnownInboundNonceCount);
        Assert.IsTrue(getPendingCount() >= 2);
        ...
        Assert.IsTrue(afterPoison.PendingSettlementCount >= 2);
        Assert.AreEqual(2UL, afterPoison.LatestCheckpointBatchNumber);
        Assert.AreEqual(19, afterPoison.ConsumedDepositCount);
        Assert.AreEqual(19, afterPoison.MessageOutboxL2ToL1Count);
        Assert.AreEqual(19, afterPoison.KnownForcedInclusionNonceCount);
        Assert.AreEqual(19, afterPoison.KnownInboundNonceCount);
        ...
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestCheckpointPostStateRoot);
        Assert.AreEqual(SoftPassThroughExecutor.PostStateRoot, afterRecover.LatestRpcStateRoot);
        Assert.AreEqual(19, afterRecover.ConsumedDepositCount);
        Assert.AreEqual(19, afterRecover.MessageOutboxL2ToL1Count);
        Assert.AreEqual(19, afterRecover.KnownForcedInclusionNonceCount);
        Assert.AreEqual(19, afterRecover.KnownInboundNonceCount);
        Assert.IsTrue(afterRecover.IsOfflinePassportComplete);
        Assert.IsTrue(afterRecover.IsOperatorReady);
        Assert.IsTrue(afterRecover.IsBatcherCheckpointAligned);
        Assert.IsFalse(afterRecover.IsPipelineHealthy);
        ...
        var probeJson = formatHealthProbeJson();
        StringAssert.Contains(probeJson, "\"isSettlementRetrying\": true");
        StringAssert.Contains(probeJson, "\"consumedDepositCount\": 19");
        StringAssert.Contains(probeJson, "\"messageOutboxL2ToL1Count\": 19");

        var statusPath = Path.Combine(chainDir, "soft-seal-nineteenth-poison-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        var statusFile = File.ReadAllText(statusPath);
        StringAssert.Contains(statusFile, "\"isSettlementRetrying\": true");
        ...
        var probePath = Path.Combine(chainDir, "soft-seal-nineteenth-poison-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 19");
        StringAssert.Contains(File.ReadAllText(probePath), "\"isSettlementRetrying\": true");

        var durablePath = Path.Combine(chainDir, "soft-seal-nineteenth-poison-recover.json");
        File.WriteAllText(durablePath, $$"""
            {
              "nineteenthPoisonBlockedBatch": {{blockedBatch}},
              "pendingSettlementCount": {{afterRecover.PendingSettlementCount}},
              "latestCheckpointBatchNumber": {{afterRecover.LatestCheckpointBatchNumber}},
              "consumedDepositCount": {{afterRecover.ConsumedDepositCount}},
              "messageOutboxL2ToL1Count": {{afterRecover.MessageOutboxL2ToL1Count}},
    ...
    /// <summary>
    /// SoftSeal after fourteenth poison→recover: re-publish local DA for sealed batches 1+2
    /// and process a fifteenth offline deposit (nonce 15, IncludedInBatch=2) while settle
    /// remains Retrying with multi-batch pending and quattuordecuple outbox/FI known retained.
    /// Does not claim L1 deposit scan / production DA / settle.
    /// </summary>
    private static void AssertSoftSealAfterFourteenthRecoverDaAndFifteenthDeposit(
        Func<DAPublishRequest, Task<DAReceipt>> publishDaAsync,
        Func<DAReceipt, Task<bool>> isDaAvailableAsync,
        Func<bool> supportsLocalDaReader,
        Func<IDAReader> createLocalDaReader,
        ...
        string chainDir)
    {
        ...
        // Fifteenth offline mint while still Retrying after fourteenth recover.
        var softL1Asset = UInt160.Parse("0x" + new string('1', 40));
        var softL2Asset = UInt160.Parse("0x" + new string('2', 40));
        var softDepositPayload = new DepositPayload
        {
        ...
        Assert.IsTrue(hasConsumedDeposit(0, 13));
        Assert.IsTrue(hasConsumedDeposit(0, 14));
        Assert.IsTrue(hasConsumedDeposit(0, 15));
        Assert.AreEqual(15, consumedDepositCount());
        recordRpcDeposit(new DepositStatus(0, 15, ConsumedOnL2: true, IncludedInBatch: 2));
        Assert.IsTrue(getRpcL1DepositStatus(0, 1) is { ConsumedOnL2: true, IncludedInBatch: 1UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 2) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 3) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 4) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        ...
        Assert.IsTrue(getRpcL1DepositStatus(0, 13) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 14) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 15) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.AreEqual(0, scanSharedBridgeDepositsAsync().GetAwaiter().GetResult());
        ...
        var hostProm = exportPrometheusMetrics();
        Assert.IsFalse(string.IsNullOrWhiteSpace(hostProm));
        var hostPromPath = Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-host.prom");
        writePrometheusMetricsAsync(hostPromPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(hostPromPath));
        Assert.AreEqual(hostProm, File.ReadAllText(hostPromPath));

        var statusPath = Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        StringAssert.Contains(File.ReadAllText(statusPath), "\"consumedDepositCount\": 15");
        StringAssert.Contains(File.ReadAllText(statusPath), "\"isSettlementRetrying\": true");

        var probePath = Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 15");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-fourteenth-recover-da-deposit.json");
        File.WriteAllText(durablePath, $$"""
            {
              "daBatch1Layer": "{{da1.Layer}}",
              "daBatch2Layer": "{{da2.Layer}}",
        ...
        StringAssert.Contains(durableFile, "\"daBatch2Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 15");
        StringAssert.Contains(durableFile, "\"deposit15IncludedInBatch\": 2");
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 14");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");
        StringAssert.Contains(durableFile, "\"isOfflinePassportComplete\": true");
    }
    ...
    /// <summary>
    /// SoftSeal after fourteenth poison→recover: re-publish local DA for sealed batches 1+2
    /// and process a sixteenth offline deposit (nonce 16, IncludedInBatch=2) while settle
    /// remains Retrying with multi-batch pending and quattuordecuple outbox/FI known retained.
    /// Does not claim L1 deposit scan / production DA / settle.
    /// </summary>
    private static void AssertSoftSealAfterFifteenthRecoverDaAndSixteenthDeposit(
        Func<DAPublishRequest, Task<DAReceipt>> publishDaAsync,
        Func<DAReceipt, Task<bool>> isDaAvailableAsync,
        Func<bool> supportsLocalDaReader,
        Func<IDAReader> createLocalDaReader,
        ...
        string chainDir)
    {
        ...
        // Sixteenth offline mint while still Retrying after fifteenth recover.
        var softL1Asset = UInt160.Parse("0x" + new string('1', 40));
        var softL2Asset = UInt160.Parse("0x" + new string('2', 40));
        var softDepositPayload = new DepositPayload
        {
        ...
        Assert.IsTrue(hasConsumedDeposit(0, 14));
        Assert.IsTrue(hasConsumedDeposit(0, 15));
        Assert.IsTrue(hasConsumedDeposit(0, 16));
        Assert.AreEqual(16, consumedDepositCount());
        recordRpcDeposit(new DepositStatus(0, 16, ConsumedOnL2: true, IncludedInBatch: 2));
        Assert.IsTrue(getRpcL1DepositStatus(0, 1) is { ConsumedOnL2: true, IncludedInBatch: 1UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 2) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 3) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 4) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        ...
        Assert.IsTrue(getRpcL1DepositStatus(0, 13) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 14) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 16) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.AreEqual(0, scanSharedBridgeDepositsAsync().GetAwaiter().GetResult());
        ...
        var hostProm = exportPrometheusMetrics();
        Assert.IsFalse(string.IsNullOrWhiteSpace(hostProm));
        var hostPromPath = Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-host.prom");
        writePrometheusMetricsAsync(hostPromPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(hostPromPath));
        Assert.AreEqual(hostProm, File.ReadAllText(hostPromPath));

        var statusPath = Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        StringAssert.Contains(File.ReadAllText(statusPath), "\"consumedDepositCount\": 16");
        StringAssert.Contains(File.ReadAllText(statusPath), "\"isSettlementRetrying\": true");

        var probePath = Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 16");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-fifteenth-recover-da-deposit.json");
        File.WriteAllText(durablePath, $$"""
            {
              "daBatch1Layer": "{{da1.Layer}}",
              "daBatch2Layer": "{{da2.Layer}}",
        ...
        StringAssert.Contains(durableFile, "\"daBatch2Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 16");
        StringAssert.Contains(durableFile, "\"deposit16IncludedInBatch\": 2");
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 15");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");
        StringAssert.Contains(durableFile, "\"isOfflinePassportComplete\": true");
    }
    ...
    /// <summary>
    /// SoftSeal after fourteenth poison→recover: re-publish local DA for sealed batches 1+2
    /// and process a seventeenth offline deposit (nonce 17, IncludedInBatch=2) while settle
    /// remains Retrying with multi-batch pending and quattuordecuple outbox/FI known retained.
    /// Does not claim L1 deposit scan / production DA / settle.
    /// </summary>
    private static void AssertSoftSealAfterSixteenthRecoverDaAndSeventeenthDeposit(
        Func<DAPublishRequest, Task<DAReceipt>> publishDaAsync,
        Func<DAReceipt, Task<bool>> isDaAvailableAsync,
        Func<bool> supportsLocalDaReader,
        Func<IDAReader> createLocalDaReader,
        ...
        string chainDir)
    {
        ...
        // Seventeenth offline mint while still Retrying after sixteenth recover.
        var softL1Asset = UInt160.Parse("0x" + new string('1', 40));
        var softL2Asset = UInt160.Parse("0x" + new string('2', 40));
        var softDepositPayload = new DepositPayload
        {
        ...
        Assert.IsTrue(hasConsumedDeposit(0, 15));
        Assert.IsTrue(hasConsumedDeposit(0, 16));
        Assert.IsTrue(hasConsumedDeposit(0, 17));
        Assert.AreEqual(17, consumedDepositCount());
        recordRpcDeposit(new DepositStatus(0, 17, ConsumedOnL2: true, IncludedInBatch: 2));
        Assert.IsTrue(getRpcL1DepositStatus(0, 1) is { ConsumedOnL2: true, IncludedInBatch: 1UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 2) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 3) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 4) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        ...
        Assert.IsTrue(getRpcL1DepositStatus(0, 13) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 14) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 17) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.AreEqual(0, scanSharedBridgeDepositsAsync().GetAwaiter().GetResult());
        ...
        var hostProm = exportPrometheusMetrics();
        Assert.IsFalse(string.IsNullOrWhiteSpace(hostProm));
        var hostPromPath = Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-host.prom");
        writePrometheusMetricsAsync(hostPromPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(hostPromPath));
        Assert.AreEqual(hostProm, File.ReadAllText(hostPromPath));

        var statusPath = Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        StringAssert.Contains(File.ReadAllText(statusPath), "\"consumedDepositCount\": 17");
        StringAssert.Contains(File.ReadAllText(statusPath), "\"isSettlementRetrying\": true");

        var probePath = Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 17");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-sixteenth-recover-da-deposit.json");
        File.WriteAllText(durablePath, $$"""
            {
              "daBatch1Layer": "{{da1.Layer}}",
              "daBatch2Layer": "{{da2.Layer}}",
        ...
        StringAssert.Contains(durableFile, "\"daBatch2Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 17");
        StringAssert.Contains(durableFile, "\"deposit17IncludedInBatch\": 2");
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 16");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");
        StringAssert.Contains(durableFile, "\"isOfflinePassportComplete\": true");
    }
    ...
    /// <summary>
    /// SoftSeal after fourteenth poison→recover: re-publish local DA for sealed batches 1+2
    /// and process a eighteenth offline deposit (nonce 18, IncludedInBatch=2) while settle
    /// remains Retrying with multi-batch pending and quattuordecuple outbox/FI known retained.
    /// Does not claim L1 deposit scan / production DA / settle.
    /// </summary>
    private static void AssertSoftSealAfterSeventeenthRecoverDaAndEighteenthDeposit(
        Func<DAPublishRequest, Task<DAReceipt>> publishDaAsync,
        Func<DAReceipt, Task<bool>> isDaAvailableAsync,
        Func<bool> supportsLocalDaReader,
        Func<IDAReader> createLocalDaReader,
        ...
        string chainDir)
    {
        ...
        // Eighteenth offline mint while still Retrying after seventeenth recover.
        var softL1Asset = UInt160.Parse("0x" + new string('1', 40));
        var softL2Asset = UInt160.Parse("0x" + new string('2', 40));
        var softDepositPayload = new DepositPayload
        {
        ...
        Assert.IsTrue(hasConsumedDeposit(0, 16));
        Assert.IsTrue(hasConsumedDeposit(0, 17));
        Assert.IsTrue(hasConsumedDeposit(0, 18));
        Assert.AreEqual(18, consumedDepositCount());
        recordRpcDeposit(new DepositStatus(0, 18, ConsumedOnL2: true, IncludedInBatch: 2));
        Assert.IsTrue(getRpcL1DepositStatus(0, 1) is { ConsumedOnL2: true, IncludedInBatch: 1UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 2) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 3) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 4) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        ...
        Assert.IsTrue(getRpcL1DepositStatus(0, 13) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 14) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 18) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.AreEqual(0, scanSharedBridgeDepositsAsync().GetAwaiter().GetResult());
        ...
        var hostProm = exportPrometheusMetrics();
        Assert.IsFalse(string.IsNullOrWhiteSpace(hostProm));
        var hostPromPath = Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-host.prom");
        writePrometheusMetricsAsync(hostPromPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(hostPromPath));
        Assert.AreEqual(hostProm, File.ReadAllText(hostPromPath));

        var statusPath = Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        StringAssert.Contains(File.ReadAllText(statusPath), "\"consumedDepositCount\": 18");
        StringAssert.Contains(File.ReadAllText(statusPath), "\"isSettlementRetrying\": true");

        var probePath = Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 18");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-seventeenth-recover-da-deposit.json");
        File.WriteAllText(durablePath, $$"""
            {
              "daBatch1Layer": "{{da1.Layer}}",
              "daBatch2Layer": "{{da2.Layer}}",
        ...
        StringAssert.Contains(durableFile, "\"daBatch2Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 18");
        StringAssert.Contains(durableFile, "\"deposit18IncludedInBatch\": 2");
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 17");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");
        StringAssert.Contains(durableFile, "\"isOfflinePassportComplete\": true");
    }
    ...
    /// <summary>
    /// SoftSeal after fourteenth poison→recover: re-publish local DA for sealed batches 1+2
    /// and process a nineteenth offline deposit (nonce 19, IncludedInBatch=2) while settle
    /// remains Retrying with multi-batch pending and quattuordecuple outbox/FI known retained.
    /// Does not claim L1 deposit scan / production DA / settle.
    /// </summary>
    private static void AssertSoftSealAfterEighteenthRecoverDaAndNineteenthDeposit(
        Func<DAPublishRequest, Task<DAReceipt>> publishDaAsync,
        Func<DAReceipt, Task<bool>> isDaAvailableAsync,
        Func<bool> supportsLocalDaReader,
        Func<IDAReader> createLocalDaReader,
        ...
        string chainDir)
    {
        ...
        // Nineteenth offline mint while still Retrying after eighteenth recover.
        var softL1Asset = UInt160.Parse("0x" + new string('1', 40));
        var softL2Asset = UInt160.Parse("0x" + new string('2', 40));
        var softDepositPayload = new DepositPayload
        {
        ...
        Assert.IsTrue(hasConsumedDeposit(0, 17));
        Assert.IsTrue(hasConsumedDeposit(0, 18));
        Assert.IsTrue(hasConsumedDeposit(0, 19));
        Assert.AreEqual(19, consumedDepositCount());
        recordRpcDeposit(new DepositStatus(0, 19, ConsumedOnL2: true, IncludedInBatch: 2));
        Assert.IsTrue(getRpcL1DepositStatus(0, 1) is { ConsumedOnL2: true, IncludedInBatch: 1UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 2) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 3) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 4) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        ...
        Assert.IsTrue(getRpcL1DepositStatus(0, 13) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 14) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 19) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.AreEqual(0, scanSharedBridgeDepositsAsync().GetAwaiter().GetResult());
        ...
        var hostProm = exportPrometheusMetrics();
        Assert.IsFalse(string.IsNullOrWhiteSpace(hostProm));
        var hostPromPath = Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-host.prom");
        writePrometheusMetricsAsync(hostPromPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(hostPromPath));
        Assert.AreEqual(hostProm, File.ReadAllText(hostPromPath));

        var statusPath = Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        StringAssert.Contains(File.ReadAllText(statusPath), "\"consumedDepositCount\": 19");
        StringAssert.Contains(File.ReadAllText(statusPath), "\"isSettlementRetrying\": true");

        var probePath = Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 19");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-eighteenth-recover-da-deposit.json");
        File.WriteAllText(durablePath, $$"""
            {
              "daBatch1Layer": "{{da1.Layer}}",
              "daBatch2Layer": "{{da2.Layer}}",
        ...
        StringAssert.Contains(durableFile, "\"daBatch2Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 19");
        StringAssert.Contains(durableFile, "\"deposit19IncludedInBatch\": 2");
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 18");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");
        StringAssert.Contains(durableFile, "\"isOfflinePassportComplete\": true");
    }
    ...
    /// <summary>
    /// SoftSeal after fourteenth poison→recover: re-publish local DA for sealed batches 1+2
    /// and process a twentieth offline deposit (nonce 20, IncludedInBatch=2) while settle
    /// remains Retrying with multi-batch pending and quattuordecuple outbox/FI known retained.
    /// Does not claim L1 deposit scan / production DA / settle.
    /// </summary>
    private static void AssertSoftSealAfterNineteenthRecoverDaAndTwentiethDeposit(
        Func<DAPublishRequest, Task<DAReceipt>> publishDaAsync,
        Func<DAReceipt, Task<bool>> isDaAvailableAsync,
        Func<bool> supportsLocalDaReader,
        Func<IDAReader> createLocalDaReader,
        ...
        string chainDir)
    {
        ...
        // Twentieth offline mint while still Retrying after nineteenth recover.
        var softL1Asset = UInt160.Parse("0x" + new string('1', 40));
        var softL2Asset = UInt160.Parse("0x" + new string('2', 40));
        var softDepositPayload = new DepositPayload
        {
        ...
        Assert.IsTrue(hasConsumedDeposit(0, 18));
        Assert.IsTrue(hasConsumedDeposit(0, 19));
        Assert.IsTrue(hasConsumedDeposit(0, 20));
        Assert.AreEqual(20, consumedDepositCount());
        recordRpcDeposit(new DepositStatus(0, 20, ConsumedOnL2: true, IncludedInBatch: 2));
        Assert.IsTrue(getRpcL1DepositStatus(0, 1) is { ConsumedOnL2: true, IncludedInBatch: 1UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 2) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 3) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 4) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        ...
        Assert.IsTrue(getRpcL1DepositStatus(0, 13) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 14) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.IsTrue(getRpcL1DepositStatus(0, 20) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
        Assert.AreEqual(0, scanSharedBridgeDepositsAsync().GetAwaiter().GetResult());
        ...
        var hostProm = exportPrometheusMetrics();
        Assert.IsFalse(string.IsNullOrWhiteSpace(hostProm));
        var hostPromPath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-host.prom");
        writePrometheusMetricsAsync(hostPromPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(hostPromPath));
        Assert.AreEqual(hostProm, File.ReadAllText(hostPromPath));

        var statusPath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-status.json");
        writeOperatorStatusAsync(statusPath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(statusPath));
        StringAssert.Contains(File.ReadAllText(statusPath), "\"consumedDepositCount\": 20");
        StringAssert.Contains(File.ReadAllText(statusPath), "\"isSettlementRetrying\": true");

        var probePath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-probe.json");
        writeHealthProbeAsync(probePath).GetAwaiter().GetResult();
        Assert.IsTrue(File.Exists(probePath));
        StringAssert.Contains(File.ReadAllText(probePath), "\"consumedDepositCount\": 20");

        var durablePath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-da-deposit.json");
        File.WriteAllText(durablePath, $$"""
            {
              "daBatch1Layer": "{{da1.Layer}}",
              "daBatch2Layer": "{{da2.Layer}}",
        ...
        StringAssert.Contains(durableFile, "\"daBatch2Layer\": \"Local\"");
        StringAssert.Contains(durableFile, "\"consumedDepositCount\": 20");
        StringAssert.Contains(durableFile, "\"deposit20IncludedInBatch\": 2");
        StringAssert.Contains(durableFile, "\"messageOutboxL2ToL1Count\": 19");
        StringAssert.Contains(durableFile, "\"isSettlementRetrying\": true");
        StringAssert.Contains(durableFile, "\"isOfflinePassportComplete\": true");
    }
