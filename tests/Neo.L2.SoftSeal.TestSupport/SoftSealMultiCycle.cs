...
public static class SoftSealMultiCycle
{
    ...
    /// <summary>
    /// Map Multisig LocalHost composition into the SoftSeal multi-cycle surface
    /// (delegates to <see cref="FromHost"/>).
    /// </summary>
    public static HostSurface FromMultisig(MultisigLocalHostComposition host, UInt256 expectedPostStateRoot)
        => FromHost(host, expectedPostStateRoot);

    /// <summary>
    /// Map Optimistic LocalHost composition into the SoftSeal multi-cycle surface
    /// (delegates to <see cref="FromHost"/>).
    /// </summary>
    ...
    /// <summary>
    /// Map any local-DA LocalHost composition (<see cref="LocalHostCompositionBase"/>) into
    /// the SoftSeal multi-cycle surface. Requires <see cref="LocalHostCompositionBase.SupportsLocalDaReader"/>.
    /// </summary>
    public static HostSurface FromHost(LocalHostCompositionBase host, UInt256 expectedPostStateRoot)
    {
        ArgumentNullException.ThrowIfNull(host);
        if (!host.SupportsLocalDaReader)
        {
            throw new NotSupportedException(
                $"{host.GetType().Name} does not support SoftSeal multi-cycle offline DA "
                + "(SupportsLocalDaReader=false; use Multisig/Optimistic or funded Zk DA).");
        }
