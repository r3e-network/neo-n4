...
public static class SoftSealMultiCycle
{
    ...
    /// <summary>Map Multisig LocalHost composition into the SoftSeal multi-cycle surface.</summary>
    public static HostSurface FromMultisig(MultisigLocalHostComposition host, UInt256 expectedPostStateRoot)
    {
    ...
    /// <summary>Map Optimistic LocalHost composition into the SoftSeal multi-cycle surface.</summary>
    public static HostSurface FromOptimistic(OptimisticLocalHostComposition host, UInt256 expectedPostStateRoot)
    {
        ArgumentNullException.ThrowIfNull(host);
        return FromLocalHost(
            req => host.PublishDaAsync(req).AsTask(),
    ...
    private static HostSurface FromLocalHost(
        ...
        UInt256 expectedPostStateRoot) => new()
    {
        PublishDaAsync = publishDaAsync,
        IsDaAvailableAsync = isDaAvailableAsync,
        SupportsLocalDaReader = supportsLocalDaReader,
        CreateLocalDaReader = createLocalDaReader,
        ProcessDeposit = processDeposit,
        HasConsumedDeposit = hasConsumedDeposit,
        ConsumedDepositCount = consumedDepositCount,
        RecordRpcDeposit = recordRpcDeposit,
        GetRpcL1DepositStatus = getRpcL1DepositStatus,
        ScanSharedBridgeDepositsAsync = scanSharedBridgeDepositsAsync,
        StageWithdrawal = stageWithdrawal,
        StagedWithdrawalCount = stagedWithdrawalCount,
        SealWithdrawalBatch = sealWithdrawalBatch,
        EnqueueOutbound = enqueueOutbound,
        MessageOutboxL2ToL1Count = messageOutboxL2ToL1Count,
        MessageOutboxL2ToL1Root = messageOutboxL2ToL1Root,
        RegisterForcedInclusionNonce = registerForcedInclusionNonce,
        KnownForcedInclusionNonceCount = knownForcedInclusionNonceCount,
        HasOverdueForcedInclusionCached = hasOverdueForcedInclusionCached,
        InvalidateForcedInclusionCache = invalidateForcedInclusionCache,
        OpenBatchForcedInclusionCount = openBatchForcedInclusionCount,
        RegisterInboundMessageNonce = registerInboundMessageNonce,
        KnownInboundNonceCount = knownInboundNonceCount,
        InvalidateInboundMessageCache = invalidateInboundMessageCache,
        OpenBatchL1MessageCount = openBatchL1MessageCount,
        L1InboxPendingCount = l1InboxPendingCount,
        RecordRpcWithdrawalProof = recordRpcWithdrawalProof,
        GetRpcWithdrawalProof = getRpcWithdrawalProof,
        RecordRpcMessageProof = recordRpcMessageProof,
        GetRpcMessageProof = getRpcMessageProof,
        RecordMessageRouterFinalizedProof = recordMessageRouterFinalizedProof,
        GetMessageRouterProofAsync = getMessageRouterProofAsync,
        ReconcileAsync = reconcileAsync,
        SubmitNextAsync = submitNextAsync,
        RecoverPoisonedBatchAsync = recoverPoisonedBatchAsync,
        IsSettlementPoisonedAsync = isSettlementPoisonedAsync,
        GetPendingCount = getPendingCount,
        GetOperatorStatus = getOperatorStatus,
        GetHealthProbe = getHealthProbe,
        FormatOperatorStatusJson = formatOperatorStatusJson,
        FormatHealthProbeJson = formatHealthProbeJson,
        ExportPrometheusMetrics = exportPrometheusMetrics,
        WritePrometheusMetricsAsync = writePrometheusMetricsAsync,
        WriteOperatorStatusAsync = writeOperatorStatusAsync,
        WriteHealthProbeAsync = writeHealthProbeAsync,
        GetRpcBatch = getRpcBatch,
        GetRpcBatchStatus = getRpcBatchStatus,
        GetRpcStateRootAtBatch = getRpcStateRootAtBatch,
        GetLatestRpcStateRoot = getLatestRpcStateRoot,
        ExpectedPostStateRoot = expectedPostStateRoot,
    };
