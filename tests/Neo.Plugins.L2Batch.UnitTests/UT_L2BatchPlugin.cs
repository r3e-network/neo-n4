...
public class UT_L2BatchPlugin
{
    ...
    public void TryRetryPendingSealedBatch_RetriesWithoutNewBlock()
    {
        ...
        Assert.IsTrue(plugin.MaxForcedTransactionsPerBatch > 0);
        Assert.IsTrue(plugin.MaxL1MessagesPerBatch > 0);
        ...
        Assert.IsNull(plugin.PendingSealedBatchNumber);
    ...
    public void ProcessCommittedBlock_TwoBlockSettings_ReportsOpenBatchRange()
    {
        ...
        Assert.AreEqual(0, plugin.OpenBatchL1MessageCount);
        ...
        Assert.AreEqual(1001u, plugin.ConfiguredChainId);
        Assert.IsFalse(plugin.HasPendingSealedBatch);
    }
