...
public sealed class UT_MultisigLocalHostComposition
{
    ...
    /// <summary>
    /// Soft offline settlement backfill: after seal batch 1 via ProcessCommittedBlock,
    ...
    public void SoftOffline_EnqueueAsync_BackfillsSettlementWithoutAdvancingBatcher()
    {
        ...
        var chainDir = Path.Combine(
            Path.GetTempPath(),
            "neo-n4-msig-enqueue-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(chainDir);
        try
        {
            MaterializeMultisigChain(chainDir, reportPath);
            ...
            // Settlement-only backfill (no ProcessCommittedBlock / batcher advance).
            ...
            // Durable operator JSON writers still work offline while unhealthy (scrape/status files).
            ...
            var statusPath = Path.Combine(chainDir, "soft-offline-enqueue-operator-status.json");
            ...
            var probePath = Path.Combine(chainDir, "soft-offline-enqueue-health-probe.json");
            ...
            var promPath = Path.Combine(chainDir, "soft-offline-enqueue-metrics.prom");
    ...
    public void SoftSeal_EmptyBlock_PersistsLocalCheckpoint_Multisig()
    {
        ...
        try
        {
            ...
            // Gateway multi-batch backlog still independent after multi-cycle recover / SubmitNext.
            Assert.IsTrue(gatewayHost.AggregatorPendingCount >= 4);
