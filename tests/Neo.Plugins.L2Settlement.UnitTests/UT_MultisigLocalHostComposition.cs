...
public sealed class UT_MultisigLocalHostComposition
{
    ...
    public void SoftSeal_EmptyBlock_PersistsLocalCheckpoint_Multisig()
    {
        ...
        try
        {
            ...
            // Twentieth offline withdrawal/outbox + FI/inbound after nineteenth recover (no L1).
            var twentiethSender = Account(0xcc);
            var twentiethL2Asset = UInt160.Parse("0x" + new string('2', 40));
            var twentiethWdLeaf = host.StageWithdrawal(new WithdrawalRequest
            {
                ChainId = 20260716u,
                EmittingContract = twentiethSender,
                L2Sender = twentiethSender,
                L1Recipient = twentiethSender,
                L2Asset = twentiethL2Asset,
                Amount = new BigInteger(100),
                Nonce = 20,
            });
            Assert.AreNotEqual(UInt256.Zero, twentiethWdLeaf);
            var twentiethSealedWd = host.SealWithdrawalBatch();
            Assert.AreNotEqual(UInt256.Zero, twentiethSealedWd.Root);
            Assert.AreEqual(0, host.StagedWithdrawalCount);
            Assert.IsTrue(twentiethSealedWd.Tree.Count >= 1);
            var twentiethMerkle = twentiethSealedWd.Tree.GetProof(twentiethSealedWd.Tree.Count - 1);
            Assert.AreEqual(twentiethWdLeaf, twentiethMerkle.Leaf);
            var twentiethWdProofBytes = MerkleProofSerializer.Encode(twentiethMerkle);
            Assert.IsTrue(twentiethWdProofBytes.Length >= MerkleProofSerializer.HeaderSize);
            host.RecordRpcWithdrawalProof(twentiethWdLeaf, twentiethWdProofBytes);
            CollectionAssert.AreEqual(
                twentiethWdProofBytes,
                host.GetRpcWithdrawalProof(twentiethWdLeaf)!.Value.ToArray());
            var twentiethOutboundDraft = new CrossChainMessage
            {
                SourceChainId = 20260716u,
                TargetChainId = 0,
                Nonce = 40,
                Sender = twentiethSender,
                Receiver = twentiethSender,
                MessageType = MessageType.Event,
                Payload = new byte[] { 0x14 },
                MessageHash = UInt256.Zero,
            };
            var twentiethOutbound = twentiethOutboundDraft with
            {
                MessageHash = MessageHasher.HashMessage(twentiethOutboundDraft),
            };
            host.EnqueueOutboundMessagesAsync([twentiethOutbound]).AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(20, host.MessageOutbox!.L2ToL1Count);
            var twentiethMsgProofBytes = twentiethOutbound.MessageHash.GetSpan().ToArray();
            host.RecordRpcMessageProof(twentiethOutbound.MessageHash, twentiethMsgProofBytes);
            CollectionAssert.AreEqual(
                twentiethMsgProofBytes,
                host.GetRpcMessageProof(twentiethOutbound.MessageHash)!.Value.ToArray());
            host.RecordMessageRouterFinalizedProof(twentiethOutbound.MessageHash, twentiethMsgProofBytes);
            CollectionAssert.AreEqual(
                twentiethMsgProofBytes,
                host.GetMessageRouterProofAsync(twentiethOutbound.MessageHash).AsTask().GetAwaiter().GetResult()!.Value.ToArray());
            Assert.IsTrue(host.RegisterForcedInclusionNonce(42));
            Assert.IsFalse(host.RegisterForcedInclusionNonce(42));
            Assert.AreEqual(20, host.KnownForcedInclusionNonceCount);
            Assert.IsTrue(host.RegisterInboundMessageNonce(42));
            Assert.IsFalse(host.RegisterInboundMessageNonce(42));
            Assert.AreEqual(20, host.KnownInboundNonceCount);
            Assert.AreEqual(0, host.OpenBatchForcedInclusionCount);
            Assert.AreEqual(0, host.OpenBatchL1MessageCount);
            var afterTwentiethOutbound = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(20, afterTwentiethOutbound.ConsumedDepositCount);
            Assert.AreEqual(20, afterTwentiethOutbound.MessageOutboxL2ToL1Count);
            Assert.AreEqual(20, afterTwentiethOutbound.KnownForcedInclusionNonceCount);
            Assert.AreEqual(20, afterTwentiethOutbound.KnownInboundNonceCount);
            Assert.IsTrue(afterTwentiethOutbound.IsSettlementRetrying);
            Assert.IsTrue(afterTwentiethOutbound.PendingSettlementCount >= 2);
            Assert.AreEqual(2UL, afterTwentiethOutbound.LatestCheckpointBatchNumber);
            var twentiethOutboundPath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-twentieth-outbound.json");
            File.WriteAllText(twentiethOutboundPath, $$"""
                {
                  "consumedDepositCount": {{afterTwentiethOutbound.ConsumedDepositCount}},
                  "messageOutboxL2ToL1Count": {{afterTwentiethOutbound.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentiethOutbound.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentiethOutbound.KnownInboundNonceCount}},
                  "withdrawalLeaf": "{{twentiethWdLeaf}}",
                  "outboundMessageHash": "{{twentiethOutbound.MessageHash}}",
                  "withdrawalProofBytes": {{twentiethWdProofBytes.Length}},
                  "messageProofBytes": {{twentiethMsgProofBytes.Length}},
                  "latestCheckpointBatchNumber": {{afterTwentiethOutbound.LatestCheckpointBatchNumber}},
                  "pendingSettlementCount": {{afterTwentiethOutbound.PendingSettlementCount}},
                  "isSettlementRetrying": true
                }
                """);
            Assert.IsTrue(File.Exists(twentiethOutboundPath));
            StringAssert.Contains(File.ReadAllText(twentiethOutboundPath), "\"messageOutboxL2ToL1Count\": 20");
            StringAssert.Contains(File.ReadAllText(twentiethOutboundPath), "\"knownForcedInclusionNonceCount\": 20");
            StringAssert.Contains(File.ReadAllText(twentiethOutboundPath), "\"consumedDepositCount\": 20");
            var twentiethOutboundRpcPath = Path.Combine(chainDir, "soft-seal-after-nineteenth-recover-twentieth-outbound-rpc.json");
            File.WriteAllText(twentiethOutboundRpcPath, $$"""
                {
                  "withdrawalLeaf": "{{twentiethWdLeaf}}",
                  "outboundMessageHash": "{{twentiethOutbound.MessageHash}}",
                  "withdrawalProofBytes": {{twentiethWdProofBytes.Length}},
                ...
                """);
            Assert.IsTrue(File.Exists(twentiethOutboundRpcPath));
            StringAssert.Contains(File.ReadAllText(twentiethOutboundRpcPath), "\"outboundMessageHash\": \"" + twentiethOutbound.MessageHash + "\"");

            // Twentieth poison→recover after viguple soft multi-batch path retains soft state.
            LocalHostOperatorStatus afterTwentiethPoison = afterTwentiethOutbound;
            for (var attempt = 0; attempt < 16; attempt++)
            {
                try
                {
                ...
                host.SubmitNextAsync().GetAwaiter().GetResult();
                afterTwentiethPoison = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
                if (afterTwentiethPoison.IsSettlementPoisoned)
                    break;
            }

            Assert.IsTrue(afterTwentiethPoison.IsSettlementPoisoned);
            Assert.IsFalse(afterTwentiethPoison.IsSettlementRetrying);
            Assert.IsTrue(afterTwentiethPoison.PendingSettlementCount >= 2);
            Assert.AreEqual(20, afterTwentiethPoison.ConsumedDepositCount);
            Assert.AreEqual(20, afterTwentiethPoison.MessageOutboxL2ToL1Count);
            Assert.AreEqual(20, afterTwentiethPoison.KnownForcedInclusionNonceCount);
            Assert.AreEqual(20, afterTwentiethPoison.KnownInboundNonceCount);
            Assert.IsNotNull(afterTwentiethPoison.Recovery.BlockedBatchNumber);
            Assert.IsNotNull(afterTwentiethPoison.Recovery.ArtifactContentHash);
            var twentiethBlocked = afterTwentiethPoison.Recovery.BlockedBatchNumber!.Value;
            var twentiethHash = afterTwentiethPoison.Recovery.ArtifactContentHash!;
            Assert.ThrowsExactly<InvalidOperationException>(
                () => host.RecoverPoisonedBatchAsync(twentiethBlocked, UInt256.Zero)
                    .GetAwaiter().GetResult());
            host.RecoverPoisonedBatchAsync(twentiethBlocked, twentiethHash).GetAwaiter().GetResult();
            var afterTwentiethRecover = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.IsFalse(afterTwentiethRecover.IsSettlementPoisoned);
            Assert.IsTrue(afterTwentiethRecover.IsSettlementRetrying);
            Assert.AreEqual(SettlementRecoveryState.Retrying, afterTwentiethRecover.Recovery.State);
            Assert.AreEqual(0, afterTwentiethRecover.Recovery.RetryCount);
            Assert.IsTrue(host.GetPendingCountAsync().AsTask().GetAwaiter().GetResult() >= 2);
            Assert.AreEqual(2UL, afterTwentiethRecover.LatestCheckpointBatchNumber);
            Assert.AreEqual(20, afterTwentiethRecover.ConsumedDepositCount);
            Assert.AreEqual(20, afterTwentiethRecover.MessageOutboxL2ToL1Count);
            Assert.AreEqual(20, afterTwentiethRecover.KnownForcedInclusionNonceCount);
            Assert.AreEqual(20, afterTwentiethRecover.KnownInboundNonceCount);
            Assert.AreEqual(BatchStatus.Finalized, host.GetRpcBatchStatus(1));
            Assert.AreEqual(BatchStatus.Finalized, host.GetRpcBatchStatus(2));
            Assert.IsTrue(host.GetRpcWithdrawalProof(twentiethWdLeaf) is { Length: > 0 });
            Assert.IsTrue(host.GetRpcMessageProof(twentiethOutbound.MessageHash) is { Length: > 0 });
            Assert.IsTrue(afterTwentiethRecover.IsOfflinePassportComplete);
            Assert.IsTrue(afterTwentiethRecover.IsOperatorReady);
            var twentiethPoisonPath = Path.Combine(chainDir, "soft-seal-twentieth-poison-recover.json");
            File.WriteAllText(twentiethPoisonPath, $$"""
                {
                  "twentiethPoisonBlockedBatch": {{twentiethBlocked}},
                  "pendingSettlementCount": {{afterTwentiethRecover.PendingSettlementCount}},
                  "latestCheckpointBatchNumber": {{afterTwentiethRecover.LatestCheckpointBatchNumber}},
                  "consumedDepositCount": {{afterTwentiethRecover.ConsumedDepositCount}},
                  "messageOutboxL2ToL1Count": {{afterTwentiethRecover.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentiethRecover.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentiethRecover.KnownInboundNonceCount}},
                  "rpcBatch1Status": "{{host.GetRpcBatchStatus(1)}}",
                  "rpcBatch2Status": "{{host.GetRpcBatchStatus(2)}}",
                  "twentiethWithdrawalProofPresent": true,
                  "twentiethMessageProofPresent": true,
                  "isSettlementRetrying": true,
                  "isSettlementPoisoned": false
                }
                """);
            Assert.IsTrue(File.Exists(twentiethPoisonPath));
            StringAssert.Contains(File.ReadAllText(twentiethPoisonPath), "\"rpcBatch2Status\": \"Finalized\"");
            StringAssert.Contains(File.ReadAllText(twentiethPoisonPath), "\"consumedDepositCount\": 20");
            StringAssert.Contains(File.ReadAllText(twentiethPoisonPath), "\"messageOutboxL2ToL1Count\": 20");
            StringAssert.Contains(File.ReadAllText(twentiethPoisonPath), "\"isSettlementRetrying\": true");

            // After twentieth recover: re-publish local DA + twenty-first offline deposit while Retrying.
            Assert.IsTrue(host.SupportsLocalDaReader);
            var twentiethRecoverDa1Payload = new byte[] { 0xDA, 0xA1, 0x01 };
            var twentiethRecoverDa1 = host.PublishDaAsync(new DAPublishRequest
            {
                ChainId = 20260716u,
                BatchNumber = 1,
                Payload = twentiethRecoverDa1Payload,
            }).AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(DAMode.Local, twentiethRecoverDa1.Layer);
            Assert.IsTrue(host.IsDaAvailableAsync(twentiethRecoverDa1).AsTask().GetAwaiter().GetResult());
            var twentiethRecoverDa1Read = host.CreateLocalDaReader().ReadAsync(twentiethRecoverDa1).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(twentiethRecoverDa1Read is { Length: 3 });
            CollectionAssert.AreEqual(twentiethRecoverDa1Payload, twentiethRecoverDa1Read!.Value.ToArray());
            var twentiethRecoverDa2Payload = new byte[] { 0xDA, 0xA2, 0x02 };
            var twentiethRecoverDa2 = host.PublishDaAsync(new DAPublishRequest
            {
                ChainId = 20260716u,
                BatchNumber = 2,
                Payload = twentiethRecoverDa2Payload,
            }).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(host.IsDaAvailableAsync(twentiethRecoverDa2).AsTask().GetAwaiter().GetResult());
            var twentiethRecoverDa2Read = host.CreateLocalDaReader().ReadAsync(twentiethRecoverDa2).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(twentiethRecoverDa2Read is { Length: 3 });
            CollectionAssert.AreEqual(twentiethRecoverDa2Payload, twentiethRecoverDa2Read!.Value.ToArray());
            var twentiethRecoverL1Asset = UInt160.Parse("0x" + new string('1', 40));
            var twentiethRecoverL2Asset = UInt160.Parse("0x" + new string('2', 40));
            var deposit21 = new CrossChainMessage
            {
                SourceChainId = 0,
                TargetChainId = 20260716u,
                Nonce = 21,
                ...
                Payload = new DepositPayload
                {
                    L1Asset = twentiethRecoverL1Asset,
                    L2Recipient = Account(0x55),
                    Amount = new BigInteger(21_000),
                }.Encode(),
                MessageHash = UInt256.Zero,
            };
            var mint21 = host.ProcessDeposit(deposit21);
            Assert.AreEqual(twentiethRecoverL2Asset, mint21.L2Asset);
            Assert.IsTrue(host.HasConsumedDeposit(0, 1));
            Assert.IsTrue(host.HasConsumedDeposit(0, 2));
            Assert.IsTrue(host.HasConsumedDeposit(0, 3));
            Assert.IsTrue(host.HasConsumedDeposit(0, 4));
            ...
            Assert.IsTrue(host.HasConsumedDeposit(0, 19));
            Assert.IsTrue(host.HasConsumedDeposit(0, 20));
            Assert.IsTrue(host.HasConsumedDeposit(0, 21));
            Assert.AreEqual(21, host.ConsumedDepositCount);
            host.RecordRpcDeposit(new DepositStatus(0, 21, ConsumedOnL2: true, IncludedInBatch: 2));
            Assert.IsTrue(host.GetRpcL1DepositStatus(0, 21) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
            Assert.AreEqual(0, host.ScanSharedBridgeDepositsAsync().AsTask().GetAwaiter().GetResult());
            var afterTwentyFirstDeposit = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(21, afterTwentyFirstDeposit.ConsumedDepositCount);
            Assert.AreEqual(20, afterTwentyFirstDeposit.MessageOutboxL2ToL1Count);
            Assert.AreEqual(20, afterTwentyFirstDeposit.KnownForcedInclusionNonceCount);
            Assert.AreEqual(20, afterTwentyFirstDeposit.KnownInboundNonceCount);
            Assert.IsTrue(afterTwentyFirstDeposit.IsSettlementRetrying);
            Assert.IsTrue(afterTwentyFirstDeposit.PendingSettlementCount >= 2);
            Assert.AreEqual(2UL, afterTwentyFirstDeposit.LatestCheckpointBatchNumber);
            Assert.IsTrue(afterTwentyFirstDeposit.IsOfflinePassportComplete);
            var afterTwentiethRecoverDaPath = Path.Combine(chainDir, "soft-seal-after-twentieth-recover-da-deposit.json");
            File.WriteAllText(afterTwentiethRecoverDaPath, $$"""
                {
                  "daBatch1Layer": "{{twentiethRecoverDa1.Layer}}",
                  "daBatch2Layer": "{{twentiethRecoverDa2.Layer}}",
                  "consumedDepositCount": {{afterTwentyFirstDeposit.ConsumedDepositCount}},
                  "deposit21IncludedInBatch": 2,
                  "messageOutboxL2ToL1Count": {{afterTwentyFirstDeposit.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentyFirstDeposit.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentyFirstDeposit.KnownInboundNonceCount}},
                  "latestCheckpointBatchNumber": {{afterTwentyFirstDeposit.LatestCheckpointBatchNumber}},
                  "pendingSettlementCount": {{afterTwentyFirstDeposit.PendingSettlementCount}},
                  "isSettlementRetrying": true,
                  "isOfflinePassportComplete": true
                }
                """);
            Assert.IsTrue(File.Exists(afterTwentiethRecoverDaPath));
            StringAssert.Contains(File.ReadAllText(afterTwentiethRecoverDaPath), "\"consumedDepositCount\": 21");
            StringAssert.Contains(File.ReadAllText(afterTwentiethRecoverDaPath), "\"deposit21IncludedInBatch\": 2");
            StringAssert.Contains(File.ReadAllText(afterTwentiethRecoverDaPath), "\"daBatch2Layer\": \"Local\"");
            var afterTwentiethRecoverProm = host.ExportPrometheusMetrics();
            Assert.IsFalse(string.IsNullOrWhiteSpace(afterTwentiethRecoverProm));
            var afterTwentiethRecoverPromPath = Path.Combine(chainDir, "soft-seal-after-twentieth-recover-host.prom");
            host.WritePrometheusMetricsAsync(afterTwentiethRecoverPromPath).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(File.Exists(afterTwentiethRecoverPromPath));
            Assert.AreEqual(afterTwentiethRecoverProm, File.ReadAllText(afterTwentiethRecoverPromPath));

            // Twenty-first offline withdrawal/outbox + FI/inbound after twentieth recover (no L1).
            var twentyFirstSender = Account(0xcd);
            var twentyFirstL2Asset = UInt160.Parse("0x" + new string('2', 40));
            var twentyFirstWdLeaf = host.StageWithdrawal(new WithdrawalRequest
            {
                ChainId = 20260716u,
                EmittingContract = twentyFirstSender,
                L2Sender = twentyFirstSender,
                L1Recipient = twentyFirstSender,
                L2Asset = twentyFirstL2Asset,
                Amount = new BigInteger(100),
                Nonce = 21,
            });
            Assert.AreNotEqual(UInt256.Zero, twentyFirstWdLeaf);
            var twentyFirstSealedWd = host.SealWithdrawalBatch();
            Assert.AreNotEqual(UInt256.Zero, twentyFirstSealedWd.Root);
            Assert.AreEqual(0, host.StagedWithdrawalCount);
            Assert.IsTrue(twentyFirstSealedWd.Tree.Count >= 1);
            var twentyFirstMerkle = twentyFirstSealedWd.Tree.GetProof(twentyFirstSealedWd.Tree.Count - 1);
            Assert.AreEqual(twentyFirstWdLeaf, twentyFirstMerkle.Leaf);
            var twentyFirstWdProofBytes = MerkleProofSerializer.Encode(twentyFirstMerkle);
            Assert.IsTrue(twentyFirstWdProofBytes.Length >= MerkleProofSerializer.HeaderSize);
            host.RecordRpcWithdrawalProof(twentyFirstWdLeaf, twentyFirstWdProofBytes);
            CollectionAssert.AreEqual(
                twentyFirstWdProofBytes,
                host.GetRpcWithdrawalProof(twentyFirstWdLeaf)!.Value.ToArray());
            var twentyFirstOutboundDraft = new CrossChainMessage
            {
                SourceChainId = 20260716u,
                TargetChainId = 0,
                Nonce = 42,
                Sender = twentyFirstSender,
                Receiver = twentyFirstSender,
                MessageType = MessageType.Event,
                Payload = new byte[] { 0x15 },
                MessageHash = UInt256.Zero,
            };
            var twentyFirstOutbound = twentyFirstOutboundDraft with
            {
                MessageHash = MessageHasher.HashMessage(twentyFirstOutboundDraft),
            };
            host.EnqueueOutboundMessagesAsync([twentyFirstOutbound]).AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(21, host.MessageOutbox!.L2ToL1Count);
            var twentyFirstMsgProofBytes = twentyFirstOutbound.MessageHash.GetSpan().ToArray();
            host.RecordRpcMessageProof(twentyFirstOutbound.MessageHash, twentyFirstMsgProofBytes);
            CollectionAssert.AreEqual(
                twentyFirstMsgProofBytes,
                host.GetRpcMessageProof(twentyFirstOutbound.MessageHash)!.Value.ToArray());
            host.RecordMessageRouterFinalizedProof(twentyFirstOutbound.MessageHash, twentyFirstMsgProofBytes);
            CollectionAssert.AreEqual(
                twentyFirstMsgProofBytes,
                host.GetMessageRouterProofAsync(twentyFirstOutbound.MessageHash).AsTask().GetAwaiter().GetResult()!.Value.ToArray());
            Assert.IsTrue(host.RegisterForcedInclusionNonce(44));
            Assert.IsFalse(host.RegisterForcedInclusionNonce(44));
            Assert.AreEqual(21, host.KnownForcedInclusionNonceCount);
            Assert.IsTrue(host.RegisterInboundMessageNonce(44));
            Assert.IsFalse(host.RegisterInboundMessageNonce(44));
            Assert.AreEqual(21, host.KnownInboundNonceCount);
            Assert.AreEqual(0, host.OpenBatchForcedInclusionCount);
            Assert.AreEqual(0, host.OpenBatchL1MessageCount);
            var afterTwentyFirstOutbound = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(21, afterTwentyFirstOutbound.ConsumedDepositCount);
            Assert.AreEqual(21, afterTwentyFirstOutbound.MessageOutboxL2ToL1Count);
            Assert.AreEqual(21, afterTwentyFirstOutbound.KnownForcedInclusionNonceCount);
            Assert.AreEqual(21, afterTwentyFirstOutbound.KnownInboundNonceCount);
            Assert.IsTrue(afterTwentyFirstOutbound.IsSettlementRetrying);
            Assert.IsTrue(afterTwentyFirstOutbound.PendingSettlementCount >= 2);
            Assert.AreEqual(2UL, afterTwentyFirstOutbound.LatestCheckpointBatchNumber);
            var twentyFirstOutboundPath = Path.Combine(chainDir, "soft-seal-after-twentieth-recover-twenty-first-outbound.json");
            File.WriteAllText(twentyFirstOutboundPath, $$"""
                {
                  "consumedDepositCount": {{afterTwentyFirstOutbound.ConsumedDepositCount}},
                  "messageOutboxL2ToL1Count": {{afterTwentyFirstOutbound.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentyFirstOutbound.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentyFirstOutbound.KnownInboundNonceCount}},
                  "withdrawalLeaf": "{{twentyFirstWdLeaf}}",
                  "outboundMessageHash": "{{twentyFirstOutbound.MessageHash}}",
                  "withdrawalProofBytes": {{twentyFirstWdProofBytes.Length}},
                  "messageProofBytes": {{twentyFirstMsgProofBytes.Length}},
                  "latestCheckpointBatchNumber": {{afterTwentyFirstOutbound.LatestCheckpointBatchNumber}},
                  "pendingSettlementCount": {{afterTwentyFirstOutbound.PendingSettlementCount}},
                  "isSettlementRetrying": true
                }
                """);
            Assert.IsTrue(File.Exists(twentyFirstOutboundPath));
            StringAssert.Contains(File.ReadAllText(twentyFirstOutboundPath), "\"messageOutboxL2ToL1Count\": 21");
            StringAssert.Contains(File.ReadAllText(twentyFirstOutboundPath), "\"knownForcedInclusionNonceCount\": 21");
            StringAssert.Contains(File.ReadAllText(twentyFirstOutboundPath), "\"consumedDepositCount\": 21");
            var twentyFirstOutboundRpcPath = Path.Combine(chainDir, "soft-seal-after-twentieth-recover-twenty-first-outbound-rpc.json");
            File.WriteAllText(twentyFirstOutboundRpcPath, $$"""
                {
                  "withdrawalLeaf": "{{twentyFirstWdLeaf}}",
                  "outboundMessageHash": "{{twentyFirstOutbound.MessageHash}}",
                  "withdrawalProofBytes": {{twentyFirstWdProofBytes.Length}},
                  "messageProofBytes": {{twentyFirstMsgProofBytes.Length}},
                  "messageOutboxL2ToL1Count": 21,
                  "knownForcedInclusionNonceCount": 21,
                  "knownInboundNonceCount": 21,
                  "consumedDepositCount": 21,
                  "isSettlementRetrying": true
                }
                """);
            Assert.IsTrue(File.Exists(twentyFirstOutboundRpcPath));
            StringAssert.Contains(File.ReadAllText(twentyFirstOutboundRpcPath), "\"outboundMessageHash\": \"" + twentyFirstOutbound.MessageHash + "\"");

            // Twenty-first poison→recover after unvigintuple soft multi-batch path retains soft state.
            LocalHostOperatorStatus afterTwentyFirstPoison = afterTwentyFirstOutbound;
            for (var attempt = 0; attempt < 16; attempt++)
            {
                try
                {
                ...
                host.SubmitNextAsync().GetAwaiter().GetResult();
                afterTwentyFirstPoison = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
                if (afterTwentyFirstPoison.IsSettlementPoisoned)
                    break;
            }

            Assert.IsTrue(afterTwentyFirstPoison.IsSettlementPoisoned);
            Assert.IsFalse(afterTwentyFirstPoison.IsSettlementRetrying);
            Assert.IsTrue(afterTwentyFirstPoison.PendingSettlementCount >= 2);
            Assert.AreEqual(21, afterTwentyFirstPoison.ConsumedDepositCount);
            Assert.AreEqual(21, afterTwentyFirstPoison.MessageOutboxL2ToL1Count);
            Assert.AreEqual(21, afterTwentyFirstPoison.KnownForcedInclusionNonceCount);
            Assert.AreEqual(21, afterTwentyFirstPoison.KnownInboundNonceCount);
            Assert.IsNotNull(afterTwentyFirstPoison.Recovery.BlockedBatchNumber);
            Assert.IsNotNull(afterTwentyFirstPoison.Recovery.ArtifactContentHash);
            var twentyFirstBlocked = afterTwentyFirstPoison.Recovery.BlockedBatchNumber!.Value;
            var twentyFirstHash = afterTwentyFirstPoison.Recovery.ArtifactContentHash!;
            Assert.ThrowsExactly<InvalidOperationException>(
                () => host.RecoverPoisonedBatchAsync(twentyFirstBlocked, UInt256.Zero)
                    .GetAwaiter().GetResult());
            host.RecoverPoisonedBatchAsync(twentyFirstBlocked, twentyFirstHash).GetAwaiter().GetResult();
            var afterTwentyFirstRecover = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.IsFalse(afterTwentyFirstRecover.IsSettlementPoisoned);
            Assert.IsTrue(afterTwentyFirstRecover.IsSettlementRetrying);
            Assert.AreEqual(SettlementRecoveryState.Retrying, afterTwentyFirstRecover.Recovery.State);
            Assert.AreEqual(0, afterTwentyFirstRecover.Recovery.RetryCount);
            Assert.IsTrue(host.GetPendingCountAsync().AsTask().GetAwaiter().GetResult() >= 2);
            Assert.AreEqual(2UL, afterTwentyFirstRecover.LatestCheckpointBatchNumber);
            Assert.AreEqual(21, afterTwentyFirstRecover.ConsumedDepositCount);
            Assert.AreEqual(21, afterTwentyFirstRecover.MessageOutboxL2ToL1Count);
            Assert.AreEqual(21, afterTwentyFirstRecover.KnownForcedInclusionNonceCount);
            Assert.AreEqual(21, afterTwentyFirstRecover.KnownInboundNonceCount);
            Assert.AreEqual(BatchStatus.Finalized, host.GetRpcBatchStatus(1));
            Assert.AreEqual(BatchStatus.Finalized, host.GetRpcBatchStatus(2));
            Assert.IsTrue(host.GetRpcWithdrawalProof(twentyFirstWdLeaf) is { Length: > 0 });
            Assert.IsTrue(host.GetRpcMessageProof(twentyFirstOutbound.MessageHash) is { Length: > 0 });
            Assert.IsTrue(afterTwentyFirstRecover.IsOfflinePassportComplete);
            Assert.IsTrue(afterTwentyFirstRecover.IsOperatorReady);
            var twentyFirstPoisonPath = Path.Combine(chainDir, "soft-seal-twenty-first-poison-recover.json");
            File.WriteAllText(twentyFirstPoisonPath, $$"""
                {
                  "twentyFirstPoisonBlockedBatch": {{twentyFirstBlocked}},
                  "pendingSettlementCount": {{afterTwentyFirstRecover.PendingSettlementCount}},
                  "latestCheckpointBatchNumber": {{afterTwentyFirstRecover.LatestCheckpointBatchNumber}},
                  "consumedDepositCount": {{afterTwentyFirstRecover.ConsumedDepositCount}},
                  "messageOutboxL2ToL1Count": {{afterTwentyFirstRecover.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentyFirstRecover.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentyFirstRecover.KnownInboundNonceCount}},
                  "rpcBatch1Status": "{{host.GetRpcBatchStatus(1)}}",
                  "rpcBatch2Status": "{{host.GetRpcBatchStatus(2)}}",
                  "twentyFirstWithdrawalProofPresent": true,
                  "twentyFirstMessageProofPresent": true,
                  "isSettlementRetrying": true,
                  "isSettlementPoisoned": false
                }
                """);
            Assert.IsTrue(File.Exists(twentyFirstPoisonPath));
            StringAssert.Contains(File.ReadAllText(twentyFirstPoisonPath), "\"rpcBatch2Status\": \"Finalized\"");
            StringAssert.Contains(File.ReadAllText(twentyFirstPoisonPath), "\"consumedDepositCount\": 21");
            StringAssert.Contains(File.ReadAllText(twentyFirstPoisonPath), "\"messageOutboxL2ToL1Count\": 21");
            StringAssert.Contains(File.ReadAllText(twentyFirstPoisonPath), "\"isSettlementRetrying\": true");

            // After twenty-first recover: re-publish local DA + twenty-second offline deposit while Retrying.
            Assert.IsTrue(host.SupportsLocalDaReader);
            var twentyFirstRecoverDa1Payload = new byte[] { 0xDA, 0xA1, 0x01 };
            var twentyFirstRecoverDa1 = host.PublishDaAsync(new DAPublishRequest
            {
                ChainId = 20260716u,
                BatchNumber = 1,
                Payload = twentyFirstRecoverDa1Payload,
            }).AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(DAMode.Local, twentyFirstRecoverDa1.Layer);
            Assert.IsTrue(host.IsDaAvailableAsync(twentyFirstRecoverDa1).AsTask().GetAwaiter().GetResult());
            var twentyFirstRecoverDa1Read = host.CreateLocalDaReader().ReadAsync(twentyFirstRecoverDa1).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(twentyFirstRecoverDa1Read is { Length: 3 });
            CollectionAssert.AreEqual(twentyFirstRecoverDa1Payload, twentyFirstRecoverDa1Read!.Value.ToArray());
            var twentyFirstRecoverDa2Payload = new byte[] { 0xDA, 0xA2, 0x02 };
            var twentyFirstRecoverDa2 = host.PublishDaAsync(new DAPublishRequest
            {
                ChainId = 20260716u,
                BatchNumber = 2,
                Payload = twentyFirstRecoverDa2Payload,
            }).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(host.IsDaAvailableAsync(twentyFirstRecoverDa2).AsTask().GetAwaiter().GetResult());
            var twentyFirstRecoverDa2Read = host.CreateLocalDaReader().ReadAsync(twentyFirstRecoverDa2).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(twentyFirstRecoverDa2Read is { Length: 3 });
            CollectionAssert.AreEqual(twentyFirstRecoverDa2Payload, twentyFirstRecoverDa2Read!.Value.ToArray());
            var twentyFirstRecoverL1Asset = UInt160.Parse("0x" + new string('1', 40));
            var twentyFirstRecoverL2Asset = UInt160.Parse("0x" + new string('2', 40));
            var deposit22 = new CrossChainMessage
            {
                SourceChainId = 0,
                TargetChainId = 20260716u,
                Nonce = 22,
                ...
                Payload = new DepositPayload
                {
                    L1Asset = twentyFirstRecoverL1Asset,
                    L2Recipient = Account(0x55),
                    Amount = new BigInteger(22_000),
                }.Encode(),
                MessageHash = UInt256.Zero,
            };
            var mint22 = host.ProcessDeposit(deposit22);
            Assert.AreEqual(twentyFirstRecoverL2Asset, mint22.L2Asset);
            Assert.IsTrue(host.HasConsumedDeposit(0, 1));
            Assert.IsTrue(host.HasConsumedDeposit(0, 2));
            Assert.IsTrue(host.HasConsumedDeposit(0, 3));
            Assert.IsTrue(host.HasConsumedDeposit(0, 4));
            ...
            Assert.IsTrue(host.HasConsumedDeposit(0, 20));
            Assert.IsTrue(host.HasConsumedDeposit(0, 21));
            Assert.IsTrue(host.HasConsumedDeposit(0, 22));
            Assert.AreEqual(22, host.ConsumedDepositCount);
            host.RecordRpcDeposit(new DepositStatus(0, 22, ConsumedOnL2: true, IncludedInBatch: 2));
            Assert.IsTrue(host.GetRpcL1DepositStatus(0, 22) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
            Assert.AreEqual(0, host.ScanSharedBridgeDepositsAsync().AsTask().GetAwaiter().GetResult());
            var afterTwentySecondDeposit = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(22, afterTwentySecondDeposit.ConsumedDepositCount);
            Assert.AreEqual(21, afterTwentySecondDeposit.MessageOutboxL2ToL1Count);
            Assert.AreEqual(21, afterTwentySecondDeposit.KnownForcedInclusionNonceCount);
            Assert.AreEqual(21, afterTwentySecondDeposit.KnownInboundNonceCount);
            Assert.IsTrue(afterTwentySecondDeposit.IsSettlementRetrying);
            Assert.IsTrue(afterTwentySecondDeposit.PendingSettlementCount >= 2);
            Assert.AreEqual(2UL, afterTwentySecondDeposit.LatestCheckpointBatchNumber);
            Assert.IsTrue(afterTwentySecondDeposit.IsOfflinePassportComplete);
            var afterTwentyFirstRecoverDaPath = Path.Combine(chainDir, "soft-seal-after-twenty-first-recover-da-deposit.json");
            File.WriteAllText(afterTwentyFirstRecoverDaPath, $$"""
                {
                  "daBatch1Layer": "{{twentyFirstRecoverDa1.Layer}}",
                  "daBatch2Layer": "{{twentyFirstRecoverDa2.Layer}}",
                  "consumedDepositCount": {{afterTwentySecondDeposit.ConsumedDepositCount}},
                  "deposit22IncludedInBatch": 2,
                  "messageOutboxL2ToL1Count": {{afterTwentySecondDeposit.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentySecondDeposit.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentySecondDeposit.KnownInboundNonceCount}},
                  "latestCheckpointBatchNumber": {{afterTwentySecondDeposit.LatestCheckpointBatchNumber}},
                  "pendingSettlementCount": {{afterTwentySecondDeposit.PendingSettlementCount}},
                  "isSettlementRetrying": true,
                  "isOfflinePassportComplete": true
                }
                """);
            Assert.IsTrue(File.Exists(afterTwentyFirstRecoverDaPath));
            StringAssert.Contains(File.ReadAllText(afterTwentyFirstRecoverDaPath), "\"consumedDepositCount\": 22");
            StringAssert.Contains(File.ReadAllText(afterTwentyFirstRecoverDaPath), "\"deposit22IncludedInBatch\": 2");
            StringAssert.Contains(File.ReadAllText(afterTwentyFirstRecoverDaPath), "\"daBatch2Layer\": \"Local\"");
            var afterTwentyFirstRecoverProm = host.ExportPrometheusMetrics();
            Assert.IsFalse(string.IsNullOrWhiteSpace(afterTwentyFirstRecoverProm));
            var afterTwentyFirstRecoverPromPath = Path.Combine(chainDir, "soft-seal-after-twenty-first-recover-host.prom");
            host.WritePrometheusMetricsAsync(afterTwentyFirstRecoverPromPath).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(File.Exists(afterTwentyFirstRecoverPromPath));
            Assert.AreEqual(afterTwentyFirstRecoverProm, File.ReadAllText(afterTwentyFirstRecoverPromPath));

            // Twenty-second offline withdrawal/outbox + FI/inbound after twenty-first recover (no L1).
            var twentySecondSender = Account(0xce);
            var twentySecondL2Asset = UInt160.Parse("0x" + new string('2', 40));
            var twentySecondWdLeaf = host.StageWithdrawal(new WithdrawalRequest
            {
                ChainId = 20260716u,
                EmittingContract = twentySecondSender,
                L2Sender = twentySecondSender,
                L1Recipient = twentySecondSender,
                L2Asset = twentySecondL2Asset,
                Amount = new BigInteger(100),
                Nonce = 22,
            });
            Assert.AreNotEqual(UInt256.Zero, twentySecondWdLeaf);
            var twentySecondSealedWd = host.SealWithdrawalBatch();
            Assert.AreNotEqual(UInt256.Zero, twentySecondSealedWd.Root);
            Assert.AreEqual(0, host.StagedWithdrawalCount);
            Assert.IsTrue(twentySecondSealedWd.Tree.Count >= 1);
            var twentySecondMerkle = twentySecondSealedWd.Tree.GetProof(twentySecondSealedWd.Tree.Count - 1);
            Assert.AreEqual(twentySecondWdLeaf, twentySecondMerkle.Leaf);
            var twentySecondWdProofBytes = MerkleProofSerializer.Encode(twentySecondMerkle);
            Assert.IsTrue(twentySecondWdProofBytes.Length >= MerkleProofSerializer.HeaderSize);
            host.RecordRpcWithdrawalProof(twentySecondWdLeaf, twentySecondWdProofBytes);
            CollectionAssert.AreEqual(
                twentySecondWdProofBytes,
                host.GetRpcWithdrawalProof(twentySecondWdLeaf)!.Value.ToArray());
            var twentySecondOutboundDraft = new CrossChainMessage
            {
                SourceChainId = 20260716u,
                TargetChainId = 0,
                Nonce = 44,
                Sender = twentySecondSender,
                Receiver = twentySecondSender,
                MessageType = MessageType.Event,
                Payload = new byte[] { 0x16 },
                MessageHash = UInt256.Zero,
            };
            var twentySecondOutbound = twentySecondOutboundDraft with
            {
                MessageHash = MessageHasher.HashMessage(twentySecondOutboundDraft),
            };
            host.EnqueueOutboundMessagesAsync([twentySecondOutbound]).AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(22, host.MessageOutbox!.L2ToL1Count);
            var twentySecondMsgProofBytes = twentySecondOutbound.MessageHash.GetSpan().ToArray();
            host.RecordRpcMessageProof(twentySecondOutbound.MessageHash, twentySecondMsgProofBytes);
            CollectionAssert.AreEqual(
                twentySecondMsgProofBytes,
                host.GetRpcMessageProof(twentySecondOutbound.MessageHash)!.Value.ToArray());
            host.RecordMessageRouterFinalizedProof(twentySecondOutbound.MessageHash, twentySecondMsgProofBytes);
            CollectionAssert.AreEqual(
                twentySecondMsgProofBytes,
                host.GetMessageRouterProofAsync(twentySecondOutbound.MessageHash).AsTask().GetAwaiter().GetResult()!.Value.ToArray());
            Assert.IsTrue(host.RegisterForcedInclusionNonce(46));
            Assert.IsFalse(host.RegisterForcedInclusionNonce(46));
            Assert.AreEqual(22, host.KnownForcedInclusionNonceCount);
            Assert.IsTrue(host.RegisterInboundMessageNonce(46));
            Assert.IsFalse(host.RegisterInboundMessageNonce(46));
            Assert.AreEqual(22, host.KnownInboundNonceCount);
            Assert.AreEqual(0, host.OpenBatchForcedInclusionCount);
            Assert.AreEqual(0, host.OpenBatchL1MessageCount);
            var afterTwentySecondOutbound = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(22, afterTwentySecondOutbound.ConsumedDepositCount);
            Assert.AreEqual(22, afterTwentySecondOutbound.MessageOutboxL2ToL1Count);
            Assert.AreEqual(22, afterTwentySecondOutbound.KnownForcedInclusionNonceCount);
            Assert.AreEqual(22, afterTwentySecondOutbound.KnownInboundNonceCount);
            Assert.IsTrue(afterTwentySecondOutbound.IsSettlementRetrying);
            Assert.IsTrue(afterTwentySecondOutbound.PendingSettlementCount >= 2);
            Assert.AreEqual(2UL, afterTwentySecondOutbound.LatestCheckpointBatchNumber);
            var twentySecondOutboundPath = Path.Combine(chainDir, "soft-seal-after-twenty-first-recover-twenty-second-outbound.json");
            File.WriteAllText(twentySecondOutboundPath, $$"""
                {
                  "consumedDepositCount": {{afterTwentySecondOutbound.ConsumedDepositCount}},
                  "messageOutboxL2ToL1Count": {{afterTwentySecondOutbound.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentySecondOutbound.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentySecondOutbound.KnownInboundNonceCount}},
                  "withdrawalLeaf": "{{twentySecondWdLeaf}}",
                  "outboundMessageHash": "{{twentySecondOutbound.MessageHash}}",
                  "withdrawalProofBytes": {{twentySecondWdProofBytes.Length}},
                  "messageProofBytes": {{twentySecondMsgProofBytes.Length}},
                  "latestCheckpointBatchNumber": {{afterTwentySecondOutbound.LatestCheckpointBatchNumber}},
                  "pendingSettlementCount": {{afterTwentySecondOutbound.PendingSettlementCount}},
                  "isSettlementRetrying": true
                }
                """);
            Assert.IsTrue(File.Exists(twentySecondOutboundPath));
            StringAssert.Contains(File.ReadAllText(twentySecondOutboundPath), "\"messageOutboxL2ToL1Count\": 22");
            StringAssert.Contains(File.ReadAllText(twentySecondOutboundPath), "\"knownForcedInclusionNonceCount\": 22");
            StringAssert.Contains(File.ReadAllText(twentySecondOutboundPath), "\"consumedDepositCount\": 22");
            var twentySecondOutboundRpcPath = Path.Combine(chainDir, "soft-seal-after-twenty-first-recover-twenty-second-outbound-rpc.json");
            File.WriteAllText(twentySecondOutboundRpcPath, $$"""
                {
                  "withdrawalLeaf": "{{twentySecondWdLeaf}}",
                  "outboundMessageHash": "{{twentySecondOutbound.MessageHash}}",
                  "withdrawalProofBytes": {{twentySecondWdProofBytes.Length}},
                  "messageProofBytes": {{twentySecondMsgProofBytes.Length}},
                  "messageOutboxL2ToL1Count": 22,
                  "knownForcedInclusionNonceCount": 22,
                  "knownInboundNonceCount": 22,
                  "consumedDepositCount": 22,
                  "isSettlementRetrying": true
                }
                """);
            Assert.IsTrue(File.Exists(twentySecondOutboundRpcPath));
            StringAssert.Contains(File.ReadAllText(twentySecondOutboundRpcPath), "\"outboundMessageHash\": \"" + twentySecondOutbound.MessageHash + "\"");

            // Twenty-second poison→recover after duovigintuple soft multi-batch path retains soft state.
            LocalHostOperatorStatus afterTwentySecondPoison = afterTwentySecondOutbound;
            for (var attempt = 0; attempt < 16; attempt++)
            {
                try
                {
                ...
                host.SubmitNextAsync().GetAwaiter().GetResult();
                afterTwentySecondPoison = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
                if (afterTwentySecondPoison.IsSettlementPoisoned)
                    break;
            }

            Assert.IsTrue(afterTwentySecondPoison.IsSettlementPoisoned);
            Assert.IsFalse(afterTwentySecondPoison.IsSettlementRetrying);
            Assert.IsTrue(afterTwentySecondPoison.PendingSettlementCount >= 2);
            Assert.AreEqual(22, afterTwentySecondPoison.ConsumedDepositCount);
            Assert.AreEqual(22, afterTwentySecondPoison.MessageOutboxL2ToL1Count);
            Assert.AreEqual(22, afterTwentySecondPoison.KnownForcedInclusionNonceCount);
            Assert.AreEqual(22, afterTwentySecondPoison.KnownInboundNonceCount);
            Assert.IsNotNull(afterTwentySecondPoison.Recovery.BlockedBatchNumber);
            Assert.IsNotNull(afterTwentySecondPoison.Recovery.ArtifactContentHash);
            var twentySecondBlocked = afterTwentySecondPoison.Recovery.BlockedBatchNumber!.Value;
            var twentySecondHash = afterTwentySecondPoison.Recovery.ArtifactContentHash!;
            Assert.ThrowsExactly<InvalidOperationException>(
                () => host.RecoverPoisonedBatchAsync(twentySecondBlocked, UInt256.Zero)
                    .GetAwaiter().GetResult());
            host.RecoverPoisonedBatchAsync(twentySecondBlocked, twentySecondHash).GetAwaiter().GetResult();
            var afterTwentySecondRecover = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.IsFalse(afterTwentySecondRecover.IsSettlementPoisoned);
            Assert.IsTrue(afterTwentySecondRecover.IsSettlementRetrying);
            Assert.AreEqual(SettlementRecoveryState.Retrying, afterTwentySecondRecover.Recovery.State);
            Assert.AreEqual(0, afterTwentySecondRecover.Recovery.RetryCount);
            Assert.IsTrue(host.GetPendingCountAsync().AsTask().GetAwaiter().GetResult() >= 2);
            Assert.AreEqual(2UL, afterTwentySecondRecover.LatestCheckpointBatchNumber);
            Assert.AreEqual(22, afterTwentySecondRecover.ConsumedDepositCount);
            Assert.AreEqual(22, afterTwentySecondRecover.MessageOutboxL2ToL1Count);
            Assert.AreEqual(22, afterTwentySecondRecover.KnownForcedInclusionNonceCount);
            Assert.AreEqual(22, afterTwentySecondRecover.KnownInboundNonceCount);
            Assert.AreEqual(BatchStatus.Finalized, host.GetRpcBatchStatus(1));
            Assert.AreEqual(BatchStatus.Finalized, host.GetRpcBatchStatus(2));
            Assert.IsTrue(host.GetRpcWithdrawalProof(twentySecondWdLeaf) is { Length: > 0 });
            Assert.IsTrue(host.GetRpcMessageProof(twentySecondOutbound.MessageHash) is { Length: > 0 });
            Assert.IsTrue(afterTwentySecondRecover.IsOfflinePassportComplete);
            Assert.IsTrue(afterTwentySecondRecover.IsOperatorReady);
            var twentySecondPoisonPath = Path.Combine(chainDir, "soft-seal-twenty-second-poison-recover.json");
            File.WriteAllText(twentySecondPoisonPath, $$"""
                {
                  "twentySecondPoisonBlockedBatch": {{twentySecondBlocked}},
                  "pendingSettlementCount": {{afterTwentySecondRecover.PendingSettlementCount}},
                  "latestCheckpointBatchNumber": {{afterTwentySecondRecover.LatestCheckpointBatchNumber}},
                  "consumedDepositCount": {{afterTwentySecondRecover.ConsumedDepositCount}},
                  "messageOutboxL2ToL1Count": {{afterTwentySecondRecover.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentySecondRecover.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentySecondRecover.KnownInboundNonceCount}},
                  "rpcBatch1Status": "{{host.GetRpcBatchStatus(1)}}",
                  "rpcBatch2Status": "{{host.GetRpcBatchStatus(2)}}",
                  "twentySecondWithdrawalProofPresent": true,
                  "twentySecondMessageProofPresent": true,
                  "isSettlementRetrying": true,
                  "isSettlementPoisoned": false
                }
                """);
            Assert.IsTrue(File.Exists(twentySecondPoisonPath));
            StringAssert.Contains(File.ReadAllText(twentySecondPoisonPath), "\"rpcBatch2Status\": \"Finalized\"");
            StringAssert.Contains(File.ReadAllText(twentySecondPoisonPath), "\"consumedDepositCount\": 22");
            StringAssert.Contains(File.ReadAllText(twentySecondPoisonPath), "\"messageOutboxL2ToL1Count\": 22");
            StringAssert.Contains(File.ReadAllText(twentySecondPoisonPath), "\"isSettlementRetrying\": true");

            // After twenty-second recover: re-publish local DA + twenty-third offline deposit while Retrying.
            Assert.IsTrue(host.SupportsLocalDaReader);
            var twentySecondRecoverDa1Payload = new byte[] { 0xDA, 0xA1, 0x01 };
            var twentySecondRecoverDa1 = host.PublishDaAsync(new DAPublishRequest
            {
                ChainId = 20260716u,
                BatchNumber = 1,
                Payload = twentySecondRecoverDa1Payload,
            }).AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(DAMode.Local, twentySecondRecoverDa1.Layer);
            Assert.IsTrue(host.IsDaAvailableAsync(twentySecondRecoverDa1).AsTask().GetAwaiter().GetResult());
            var twentySecondRecoverDa1Read = host.CreateLocalDaReader().ReadAsync(twentySecondRecoverDa1).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(twentySecondRecoverDa1Read is { Length: 3 });
            CollectionAssert.AreEqual(twentySecondRecoverDa1Payload, twentySecondRecoverDa1Read!.Value.ToArray());
            var twentySecondRecoverDa2Payload = new byte[] { 0xDA, 0xA2, 0x02 };
            var twentySecondRecoverDa2 = host.PublishDaAsync(new DAPublishRequest
            {
                ChainId = 20260716u,
                BatchNumber = 2,
                Payload = twentySecondRecoverDa2Payload,
            }).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(host.IsDaAvailableAsync(twentySecondRecoverDa2).AsTask().GetAwaiter().GetResult());
            var twentySecondRecoverDa2Read = host.CreateLocalDaReader().ReadAsync(twentySecondRecoverDa2).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(twentySecondRecoverDa2Read is { Length: 3 });
            CollectionAssert.AreEqual(twentySecondRecoverDa2Payload, twentySecondRecoverDa2Read!.Value.ToArray());
            var twentySecondRecoverL1Asset = UInt160.Parse("0x" + new string('1', 40));
            var twentySecondRecoverL2Asset = UInt160.Parse("0x" + new string('2', 40));
            var deposit23 = new CrossChainMessage
            {
                SourceChainId = 0,
                TargetChainId = 20260716u,
                Nonce = 23,
                ...
                Payload = new DepositPayload
                {
                    L1Asset = twentySecondRecoverL1Asset,
                    L2Recipient = Account(0x55),
                    Amount = new BigInteger(23_000),
                }.Encode(),
                MessageHash = UInt256.Zero,
            };
            var mint23 = host.ProcessDeposit(deposit23);
            Assert.AreEqual(twentySecondRecoverL2Asset, mint23.L2Asset);
            Assert.IsTrue(host.HasConsumedDeposit(0, 1));
            Assert.IsTrue(host.HasConsumedDeposit(0, 2));
            Assert.IsTrue(host.HasConsumedDeposit(0, 3));
            Assert.IsTrue(host.HasConsumedDeposit(0, 4));
            ...
            Assert.IsTrue(host.HasConsumedDeposit(0, 21));
            Assert.IsTrue(host.HasConsumedDeposit(0, 22));
            Assert.IsTrue(host.HasConsumedDeposit(0, 23));
            Assert.AreEqual(23, host.ConsumedDepositCount);
            host.RecordRpcDeposit(new DepositStatus(0, 23, ConsumedOnL2: true, IncludedInBatch: 2));
            Assert.IsTrue(host.GetRpcL1DepositStatus(0, 23) is { ConsumedOnL2: true, IncludedInBatch: 2UL });
            Assert.AreEqual(0, host.ScanSharedBridgeDepositsAsync().AsTask().GetAwaiter().GetResult());
            var afterTwentyThirdDeposit = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(23, afterTwentyThirdDeposit.ConsumedDepositCount);
            Assert.AreEqual(22, afterTwentyThirdDeposit.MessageOutboxL2ToL1Count);
            Assert.AreEqual(22, afterTwentyThirdDeposit.KnownForcedInclusionNonceCount);
            Assert.AreEqual(22, afterTwentyThirdDeposit.KnownInboundNonceCount);
            Assert.IsTrue(afterTwentyThirdDeposit.IsSettlementRetrying);
            Assert.IsTrue(afterTwentyThirdDeposit.PendingSettlementCount >= 2);
            Assert.AreEqual(2UL, afterTwentyThirdDeposit.LatestCheckpointBatchNumber);
            Assert.IsTrue(afterTwentyThirdDeposit.IsOfflinePassportComplete);
            var afterTwentySecondRecoverDaPath = Path.Combine(chainDir, "soft-seal-after-twenty-second-recover-da-deposit.json");
            File.WriteAllText(afterTwentySecondRecoverDaPath, $$"""
                {
                  "daBatch1Layer": "{{twentySecondRecoverDa1.Layer}}",
                  "daBatch2Layer": "{{twentySecondRecoverDa2.Layer}}",
                  "consumedDepositCount": {{afterTwentyThirdDeposit.ConsumedDepositCount}},
                  "deposit23IncludedInBatch": 2,
                  "messageOutboxL2ToL1Count": {{afterTwentyThirdDeposit.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentyThirdDeposit.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentyThirdDeposit.KnownInboundNonceCount}},
                  "latestCheckpointBatchNumber": {{afterTwentyThirdDeposit.LatestCheckpointBatchNumber}},
                  "pendingSettlementCount": {{afterTwentyThirdDeposit.PendingSettlementCount}},
                  "isSettlementRetrying": true,
                  "isOfflinePassportComplete": true
                }
                """);
            Assert.IsTrue(File.Exists(afterTwentySecondRecoverDaPath));
            StringAssert.Contains(File.ReadAllText(afterTwentySecondRecoverDaPath), "\"consumedDepositCount\": 23");
            StringAssert.Contains(File.ReadAllText(afterTwentySecondRecoverDaPath), "\"deposit23IncludedInBatch\": 2");
            StringAssert.Contains(File.ReadAllText(afterTwentySecondRecoverDaPath), "\"daBatch2Layer\": \"Local\"");
            var afterTwentySecondRecoverProm = host.ExportPrometheusMetrics();
            Assert.IsFalse(string.IsNullOrWhiteSpace(afterTwentySecondRecoverProm));
            var afterTwentySecondRecoverPromPath = Path.Combine(chainDir, "soft-seal-after-twenty-second-recover-host.prom");
            host.WritePrometheusMetricsAsync(afterTwentySecondRecoverPromPath).AsTask().GetAwaiter().GetResult();
            Assert.IsTrue(File.Exists(afterTwentySecondRecoverPromPath));
            Assert.AreEqual(afterTwentySecondRecoverProm, File.ReadAllText(afterTwentySecondRecoverPromPath));

            // Twenty-third offline withdrawal/outbox + FI/inbound after twenty-second recover (no L1).
            var twentyThirdSender = Account(0xcf);
            var twentyThirdL2Asset = UInt160.Parse("0x" + new string('2', 40));
            var twentyThirdWdLeaf = host.StageWithdrawal(new WithdrawalRequest
            {
                ChainId = 20260716u,
                EmittingContract = twentyThirdSender,
                L2Sender = twentyThirdSender,
                L1Recipient = twentyThirdSender,
                L2Asset = twentyThirdL2Asset,
                Amount = new BigInteger(100),
                Nonce = 23,
            });
            Assert.AreNotEqual(UInt256.Zero, twentyThirdWdLeaf);
            var twentyThirdSealedWd = host.SealWithdrawalBatch();
            Assert.AreNotEqual(UInt256.Zero, twentyThirdSealedWd.Root);
            Assert.AreEqual(0, host.StagedWithdrawalCount);
            Assert.IsTrue(twentyThirdSealedWd.Tree.Count >= 1);
            var twentyThirdMerkle = twentyThirdSealedWd.Tree.GetProof(twentyThirdSealedWd.Tree.Count - 1);
            Assert.AreEqual(twentyThirdWdLeaf, twentyThirdMerkle.Leaf);
            var twentyThirdWdProofBytes = MerkleProofSerializer.Encode(twentyThirdMerkle);
            Assert.IsTrue(twentyThirdWdProofBytes.Length >= MerkleProofSerializer.HeaderSize);
            host.RecordRpcWithdrawalProof(twentyThirdWdLeaf, twentyThirdWdProofBytes);
            CollectionAssert.AreEqual(
                twentyThirdWdProofBytes,
                host.GetRpcWithdrawalProof(twentyThirdWdLeaf)!.Value.ToArray());
            var twentyThirdOutboundDraft = new CrossChainMessage
            {
                SourceChainId = 20260716u,
                TargetChainId = 0,
                Nonce = 46,
                Sender = twentyThirdSender,
                Receiver = twentyThirdSender,
                MessageType = MessageType.Event,
                Payload = new byte[] { 0x17 },
                MessageHash = UInt256.Zero,
            };
            var twentyThirdOutbound = twentyThirdOutboundDraft with
            {
                MessageHash = MessageHasher.HashMessage(twentyThirdOutboundDraft),
            };
            host.EnqueueOutboundMessagesAsync([twentyThirdOutbound]).AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(23, host.MessageOutbox!.L2ToL1Count);
            var twentyThirdMsgProofBytes = twentyThirdOutbound.MessageHash.GetSpan().ToArray();
            host.RecordRpcMessageProof(twentyThirdOutbound.MessageHash, twentyThirdMsgProofBytes);
            CollectionAssert.AreEqual(
                twentyThirdMsgProofBytes,
                host.GetRpcMessageProof(twentyThirdOutbound.MessageHash)!.Value.ToArray());
            host.RecordMessageRouterFinalizedProof(twentyThirdOutbound.MessageHash, twentyThirdMsgProofBytes);
            CollectionAssert.AreEqual(
                twentyThirdMsgProofBytes,
                host.GetMessageRouterProofAsync(twentyThirdOutbound.MessageHash).AsTask().GetAwaiter().GetResult()!.Value.ToArray());
            Assert.IsTrue(host.RegisterForcedInclusionNonce(48));
            Assert.IsFalse(host.RegisterForcedInclusionNonce(48));
            Assert.AreEqual(23, host.KnownForcedInclusionNonceCount);
            Assert.IsTrue(host.RegisterInboundMessageNonce(48));
            Assert.IsFalse(host.RegisterInboundMessageNonce(48));
            Assert.AreEqual(23, host.KnownInboundNonceCount);
            Assert.AreEqual(0, host.OpenBatchForcedInclusionCount);
            Assert.AreEqual(0, host.OpenBatchL1MessageCount);
            var afterTwentyThirdOutbound = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.AreEqual(23, afterTwentyThirdOutbound.ConsumedDepositCount);
            Assert.AreEqual(23, afterTwentyThirdOutbound.MessageOutboxL2ToL1Count);
            Assert.AreEqual(23, afterTwentyThirdOutbound.KnownForcedInclusionNonceCount);
            Assert.AreEqual(23, afterTwentyThirdOutbound.KnownInboundNonceCount);
            Assert.IsTrue(afterTwentyThirdOutbound.IsSettlementRetrying);
            Assert.IsTrue(afterTwentyThirdOutbound.PendingSettlementCount >= 2);
            Assert.AreEqual(2UL, afterTwentyThirdOutbound.LatestCheckpointBatchNumber);
            var twentyThirdOutboundPath = Path.Combine(chainDir, "soft-seal-after-twenty-second-recover-twenty-third-outbound.json");
            File.WriteAllText(twentyThirdOutboundPath, $$"""
                {
                  "consumedDepositCount": {{afterTwentyThirdOutbound.ConsumedDepositCount}},
                  "messageOutboxL2ToL1Count": {{afterTwentyThirdOutbound.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentyThirdOutbound.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentyThirdOutbound.KnownInboundNonceCount}},
                  "withdrawalLeaf": "{{twentyThirdWdLeaf}}",
                  "outboundMessageHash": "{{twentyThirdOutbound.MessageHash}}",
                  "withdrawalProofBytes": {{twentyThirdWdProofBytes.Length}},
                  "messageProofBytes": {{twentyThirdMsgProofBytes.Length}},
                  "latestCheckpointBatchNumber": {{afterTwentyThirdOutbound.LatestCheckpointBatchNumber}},
                  "pendingSettlementCount": {{afterTwentyThirdOutbound.PendingSettlementCount}},
                  "isSettlementRetrying": true
                }
                """);
            Assert.IsTrue(File.Exists(twentyThirdOutboundPath));
            StringAssert.Contains(File.ReadAllText(twentyThirdOutboundPath), "\"messageOutboxL2ToL1Count\": 23");
            StringAssert.Contains(File.ReadAllText(twentyThirdOutboundPath), "\"knownForcedInclusionNonceCount\": 23");
            StringAssert.Contains(File.ReadAllText(twentyThirdOutboundPath), "\"consumedDepositCount\": 23");
            var twentyThirdOutboundRpcPath = Path.Combine(chainDir, "soft-seal-after-twenty-second-recover-twenty-third-outbound-rpc.json");
            File.WriteAllText(twentyThirdOutboundRpcPath, $$"""
                {
                  "withdrawalLeaf": "{{twentyThirdWdLeaf}}",
                  "outboundMessageHash": "{{twentyThirdOutbound.MessageHash}}",
                  "withdrawalProofBytes": {{twentyThirdWdProofBytes.Length}},
                  "messageProofBytes": {{twentyThirdMsgProofBytes.Length}},
                  "messageOutboxL2ToL1Count": 23,
                  "knownForcedInclusionNonceCount": 23,
                  "knownInboundNonceCount": 23,
                  "consumedDepositCount": 23,
                  "isSettlementRetrying": true
                }
                """);
            Assert.IsTrue(File.Exists(twentyThirdOutboundRpcPath));
            StringAssert.Contains(File.ReadAllText(twentyThirdOutboundRpcPath), "\"outboundMessageHash\": \"" + twentyThirdOutbound.MessageHash + "\"");

            // Twenty-third poison→recover after trevigintuple soft multi-batch path retains soft state.
            LocalHostOperatorStatus afterTwentyThirdPoison = afterTwentyThirdOutbound;
            for (var attempt = 0; attempt < 16; attempt++)
            {
                try
                {
                ...
                host.SubmitNextAsync().GetAwaiter().GetResult();
                afterTwentyThirdPoison = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
                if (afterTwentyThirdPoison.IsSettlementPoisoned)
                    break;
            }

            Assert.IsTrue(afterTwentyThirdPoison.IsSettlementPoisoned);
            Assert.IsFalse(afterTwentyThirdPoison.IsSettlementRetrying);
            Assert.IsTrue(afterTwentyThirdPoison.PendingSettlementCount >= 2);
            Assert.AreEqual(23, afterTwentyThirdPoison.ConsumedDepositCount);
            Assert.AreEqual(23, afterTwentyThirdPoison.MessageOutboxL2ToL1Count);
            Assert.AreEqual(23, afterTwentyThirdPoison.KnownForcedInclusionNonceCount);
            Assert.AreEqual(23, afterTwentyThirdPoison.KnownInboundNonceCount);
            Assert.IsNotNull(afterTwentyThirdPoison.Recovery.BlockedBatchNumber);
            Assert.IsNotNull(afterTwentyThirdPoison.Recovery.ArtifactContentHash);
            var twentyThirdBlocked = afterTwentyThirdPoison.Recovery.BlockedBatchNumber!.Value;
            var twentyThirdHash = afterTwentyThirdPoison.Recovery.ArtifactContentHash!;
            Assert.ThrowsExactly<InvalidOperationException>(
                () => host.RecoverPoisonedBatchAsync(twentyThirdBlocked, UInt256.Zero)
                    .GetAwaiter().GetResult());
            host.RecoverPoisonedBatchAsync(twentyThirdBlocked, twentyThirdHash).GetAwaiter().GetResult();
            var afterTwentyThirdRecover = host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult();
            Assert.IsFalse(afterTwentyThirdRecover.IsSettlementPoisoned);
            Assert.IsTrue(afterTwentyThirdRecover.IsSettlementRetrying);
            Assert.AreEqual(SettlementRecoveryState.Retrying, afterTwentyThirdRecover.Recovery.State);
            Assert.AreEqual(0, afterTwentyThirdRecover.Recovery.RetryCount);
            Assert.IsTrue(host.GetPendingCountAsync().AsTask().GetAwaiter().GetResult() >= 2);
            Assert.AreEqual(2UL, afterTwentyThirdRecover.LatestCheckpointBatchNumber);
            Assert.AreEqual(23, afterTwentyThirdRecover.ConsumedDepositCount);
            Assert.AreEqual(23, afterTwentyThirdRecover.MessageOutboxL2ToL1Count);
            Assert.AreEqual(23, afterTwentyThirdRecover.KnownForcedInclusionNonceCount);
            Assert.AreEqual(23, afterTwentyThirdRecover.KnownInboundNonceCount);
            Assert.AreEqual(BatchStatus.Finalized, host.GetRpcBatchStatus(1));
            Assert.AreEqual(BatchStatus.Finalized, host.GetRpcBatchStatus(2));
            Assert.IsTrue(host.GetRpcWithdrawalProof(twentyThirdWdLeaf) is { Length: > 0 });
            Assert.IsTrue(host.GetRpcMessageProof(twentyThirdOutbound.MessageHash) is { Length: > 0 });
            Assert.IsTrue(afterTwentyThirdRecover.IsOfflinePassportComplete);
            Assert.IsTrue(afterTwentyThirdRecover.IsOperatorReady);
            var twentyThirdPoisonPath = Path.Combine(chainDir, "soft-seal-twenty-third-poison-recover.json");
            File.WriteAllText(twentyThirdPoisonPath, $$"""
                {
                  "twentyThirdPoisonBlockedBatch": {{twentyThirdBlocked}},
                  "pendingSettlementCount": {{afterTwentyThirdRecover.PendingSettlementCount}},
                  "latestCheckpointBatchNumber": {{afterTwentyThirdRecover.LatestCheckpointBatchNumber}},
                  "consumedDepositCount": {{afterTwentyThirdRecover.ConsumedDepositCount}},
                  "messageOutboxL2ToL1Count": {{afterTwentyThirdRecover.MessageOutboxL2ToL1Count}},
                  "knownForcedInclusionNonceCount": {{afterTwentyThirdRecover.KnownForcedInclusionNonceCount}},
                  "knownInboundNonceCount": {{afterTwentyThirdRecover.KnownInboundNonceCount}},
                  "rpcBatch1Status": "{{host.GetRpcBatchStatus(1)}}",
                  "rpcBatch2Status": "{{host.GetRpcBatchStatus(2)}}",
                  "twentyThirdWithdrawalProofPresent": true,
                  "twentyThirdMessageProofPresent": true,
                  "isSettlementRetrying": true,
                  "isSettlementPoisoned": false
                }
                """);
            Assert.IsTrue(File.Exists(twentyThirdPoisonPath));
            StringAssert.Contains(File.ReadAllText(twentyThirdPoisonPath), "\"rpcBatch2Status\": \"Finalized\"");
            StringAssert.Contains(File.ReadAllText(twentyThirdPoisonPath), "\"consumedDepositCount\": 23");
            StringAssert.Contains(File.ReadAllText(twentyThirdPoisonPath), "\"messageOutboxL2ToL1Count\": 23");
            StringAssert.Contains(File.ReadAllText(twentyThirdPoisonPath), "\"isSettlementRetrying\": true");

            // SubmitNext after twenty-third recover remains best-effort; does not clear pending without funded L1.
            host.SubmitNextAsync().GetAwaiter().GetResult();
            Assert.IsTrue(host.GetPendingCountAsync().AsTask().GetAwaiter().GetResult() >= 2);
            Assert.IsFalse(host.GetOperatorStatusAsync().AsTask().GetAwaiter().GetResult().IsSettlementIdle);
            // Gateway multi-batch backlog still independent after twenty-third recover / SubmitNext.
            Assert.IsTrue(gatewayHost.AggregatorPendingCount >= 4);
        }
        finally
        {
